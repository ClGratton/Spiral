...
namespace Engine::RHI
{
    ...
    namespace
    {
        ...
        public:
            ...
            ~NVRHID3D12Texture() override
            {
                if (m_OwnershipTracker && !m_OwnershipTracker->Unregister(*this))
                {
                    Log::Error("D3D12 RHI texture destruction rejected while queue ownership is pending: ", m_Description.DebugName);
                    std::terminate();
                }
            }
        ...
        public:
            ...
            bool TransitionTexture(Texture& texture, ResourceState expectedBefore, ResourceState destinationState) override
            {
                auto* nativeTexture = dynamic_cast<NVRHID3D12Texture*>(&texture);
                if ((m_OwnedCommandList && m_State != State::Recording) || !m_CommandList || !nativeTexture || !nativeTexture->GetResource()
                    || expectedBefore == ResourceState::Unknown || destinationState == ResourceState::Unknown
                    || (m_TextureOwnershipTracker && !m_AllowPendingTexture && !m_TextureOwnershipTracker->CanUse(&texture))) return false;
                const D3D12_RESOURCE_STATES source = ConvertResourceState(expectedBefore);
                const D3D12_RESOURCE_STATES destination = ConvertResourceState(destinationState);
                if (source != destination)
                {
                    D3D12_RESOURCE_BARRIER barrier {};
            ...
            bool ReleaseTextureOwnership(const TextureOwnershipRelease& release) override
            {
                ...
                if (!m_TextureOwnershipTracker->RecordRelease(release, m_EffectiveQueueType, m_ResolveQueue(release.SourceQueue),
                    m_ResolveQueue(release.DestinationQueue), baseline, operation)
                    || GetNVRHID3D12TextureOwnershipBarrier(operation.Type) != NVRHID3D12TextureOwnershipBarrier::None) return false;
                // A Copy list cannot transition from a graphics-only state;
                // move to the destination portable state on the source list
                // before publishing the accepted release.
                if (!TransitionTexture(*release.Resource, release.After)) return false;
                m_TextureOwnershipOperations.push_back(operation); return true;
            }
            bool AcquireTextureOwnership(const TextureOwnershipAcquire& acquire) override
            {
                ...
                RecordedTextureOwnershipOperation operation;
                if (!m_TextureOwnershipTracker->RecordAcquire(acquire, m_EffectiveQueueType, m_ResolveQueue(acquire.SourceQueue),
                    m_ResolveQueue(acquire.DestinationQueue), operation)
                    || GetNVRHID3D12TextureOwnershipBarrier(operation.Type) != NVRHID3D12TextureOwnershipBarrier::PortableStateTransition) return false;
                m_AllowPendingTexture = true;
                const bool transitioned = TransitionTexture(*acquire.Resource, acquire.After);
                m_AllowPendingTexture = false;
            ...
            bool ReleaseBufferOwnership(const BufferOwnershipRelease& release) override
            {
                ...
                if (!m_OwnershipTracker->RecordRelease(release, m_EffectiveQueueType, m_ResolveQueue(release.SourceQueue),
                    ...
                    return false;
                if (!TransitionBufferInternal(*release.Resource, release.After, false)) return false;
                m_OwnershipOperations.push_back(operation);
                return true;
            }
            bool TransitionBuffer(Buffer& buffer, ResourceState expectedBefore, ResourceState destinationState) override
            {
                auto* nativeBuffer = dynamic_cast<NVRHID3D12Buffer*>(&buffer);
                if (m_State != State::Recording || !nativeBuffer || !nativeBuffer->GetResource()
                    || expectedBefore == ResourceState::Unknown || !IsBufferStateCompatible(buffer.GetDescription().Usage, buffer.GetDescription().CpuAccess, destinationState)
                    || (m_OwnershipTracker && !m_OwnershipTracker->CanUse(&buffer))) return false;
                const D3D12_RESOURCE_STATES source = ConvertResourceState(expectedBefore);
                const D3D12_RESOURCE_STATES destination = ConvertResourceState(destinationState);
                if (source != destination)
                {
                ...
                StageBufferState(*nativeBuffer, destination, destinationState, expectedBefore);
                return true;
            }
        ...
        private:
            ...
            void StageTextureState(NVRHID3D12Texture& resource, D3D12_RESOURCE_STATES native, ResourceState tracked, ResourceState expected = ResourceState::Unknown)
            {
                for (TextureState& state : m_TextureStates)
                    if (state.Resource == &resource) { state.Native = native; state.Tracked = tracked; if (state.Expected == ResourceState::Unknown) state.Expected = expected; return; }
                m_TextureStates.push_back({ &resource, native, tracked, expected });
                m_UsedTextures.push_back(&resource);
            }

            void StageBufferState(NVRHID3D12Buffer& resource, D3D12_RESOURCE_STATES native, ResourceState tracked, ResourceState expected = ResourceState::Unknown)
            {
                for (BufferState& state : m_BufferStates)
                    if (state.Resource == &resource) { state.Native = native; state.Tracked = tracked; if (state.Expected == ResourceState::Unknown) state.Expected = expected; return; }
                m_BufferStates.push_back({ &resource, native, tracked, expected });
            }
            ...
            void CommitTrackedStates()
            {
                for (const TextureState& state : m_TextureStates)
                {
                    ...
                    if (m_TextureOwnershipTracker) m_TextureOwnershipTracker->PublishOrdinaryState(*state.Resource, state.Tracked);
                }
                for (const BufferState& state : m_BufferStates)
                {
                    state.Resource->SetCurrentState(state.Native);
        ...
        public:
            ...
            Scope<Texture> CreateTexture(const TextureDescription& description) override
            {
                ...
                if (!texture->Initialize(m_Device.Get()) || !m_TextureOwnership.Register(*texture, QueueType::Graphics, texture->GetTrackedState()))
                    return nullptr;
                texture->SetOwnershipTracker(&m_TextureOwnership);
            ...
            CompletionToken Submit(CommandList& commandList, const std::vector<CompletionToken>& dependencies) override
            {
                ...
                if (std::any_of(nativeCommandList->GetOwnershipOperations().begin(), nativeCommandList->GetOwnershipOperations().end(),
                    [&](const auto& operation) { return !m_BufferOwnership.ValidateSubmission(operation, dependencies); }))
                {
                    Log::Error("D3D12 RHI submission rejected buffer ownership validation");
                    return {};
                }
                if (std::any_of(nativeCommandList->GetTextureOwnershipOperations().begin(), nativeCommandList->GetTextureOwnershipOperations().end(),
                    [&](const auto& operation) { return !m_TextureOwnership.ValidateSubmission(operation, dependencies); }))
                {
                    Log::Error("D3D12 RHI submission rejected texture ownership validation");
                    return {};
                }
                ...
                for (const auto& ownership : nativeCommandList->GetOwnershipOperations())
                {
                    ...
                    if (!published)
                    {
                        Log::Error("D3D12 RHI submission could not publish buffer ownership");
                        return {};
                    }
                }
                for (const auto& ownership : nativeCommandList->GetTextureOwnershipOperations())
                {
                    ...
                    if (!published) { Log::Error("D3D12 RHI submission could not publish texture ownership"); return {}; }
                }
                return token;
            }
