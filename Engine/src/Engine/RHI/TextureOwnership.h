...
namespace Engine::RHI
{
    ...
    // Same accepted-submission authority as BufferOwnershipTracker. Ordinary
    // state is whole-resource and may cover a mip chain. Cross-queue transfer
    // deliberately remains restricted to the original single-subresource
    // contract until a later contract widens its range.
    class TextureOwnershipTracker
    {
    public:
        bool Register(Texture& resource, QueueType owner, ResourceState state)
        {
            return state != ResourceState::Unknown && SupportsTrackedResource(resource) && Compatible(resource, state)
        ...
        { const auto i=m_Records.find(acquire.Resource); if (i==m_Records.end() || !SupportsOwnershipTransfer(*acquire.Resource) || !i->second.Pending || !acquire.ReleaseToken.IsValid() || source==destination || list!=destination || !SamePair(*i->second.Pending,source,destination,acquire.Before,acquire.After) || !SameToken(i->second.Pending->Token,acquire.ReleaseToken)) return false; operation={TextureOwnershipOperationType::Acquire,acquire.Resource,source,destination,acquire.Before,acquire.After,acquire.ReleaseToken}; return true; }
    ...
    private:
        ...
        static bool SupportsTrackedResource(const Texture& texture)
        {
            const TextureDescription& description = texture.GetDescription();
            return description.Extent.Width != 0 && description.Extent.Height != 0
                && description.MipLevels != 0 && description.ArrayLayers == 1 && description.SampleCount == 1;
        }
        static bool SupportsOwnershipTransfer(const Texture& texture)
        {
            return SupportsTrackedResource(texture) && texture.GetDescription().MipLevels == 1;
        }
