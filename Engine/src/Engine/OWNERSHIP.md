...
| `RHI` | Backend-neutral GPU contracts and backend adapters, including the explicit single-slot vertex-layout/stride contract, the fixed graphics read-only structured-buffer binding, device-lifetime completion-token query/dependency authority, and backend-private bounded native completion history. | Scene/material/light policy, editor UI, backend-native vertex-layout types, or backend-native synchronization handles in public contracts. |
...
| `Renderer` | High-level render passes, presentation bridges, shaders, scene rendering, render diagnostics and debug-view publication/overlays, exact pipeline strides derived from renderer-owned vertex structures, and frame-local material-row/normal-transform publication from immutable Scene and Assets inputs. | Gameplay entity ownership, persistent material identity, asset import, or backend-native layout policy. |
| `Scene` | Entities/components, authoring facade, serialization, cameras, validated atomic full-transform mutation with canonical world position and camera unit-scale invariants, and backend-neutral immutable render extraction retaining stable mesh/material handles. | Resolved material rows, editor panels, or backend-native GPU objects. |
