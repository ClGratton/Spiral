...
| Prefer randomized stress/property tests with invariants or a simpler reference implementation over many fixed examples. | Accepted where a stable property or independent oracle exists. Random bytes alone are insufficient for structured inputs; generators must reach semantically interesting states. | The graph/direct-recorder exact-byte comparator is a reference oracle, but no shared property/fuzz harness, replay seed, shrinking, or failure corpus exists yet. |
| Run stress under leak/memory diagnostics and do not treat line coverage as proof of state coverage. | Accepted. Sanitizer and coverage results are additional evidence, never replacements for behavioral assertions, real workflows, or platform qualification. | No admitted ASan/UBSan/TSan build or CI lane and no coverage report currently exist. |
...
## Current Gap And Adoption Rule

Spiral already has strong deterministic contract and real-workflow evidence, but it does not yet have selectable test tiers, per-test duration reporting, a reusable property/state-machine runner, saved/minimized generated-failure corpus management, sanitizer configurations, or coverage-guided structure-aware fuzz targets. The unchecked Phase 15 items in [PLAN.md](../PLAN.md) own that shared infrastructure.

Feature work must not wait until Phase 15 to test a current risk. Until shared tooling exists, use small explicit boundary tables, deterministic operation sequences, simple local reference models, and bounded script fixtures through the current test target. When generated testing is warranted, record the seed/input/rerun command and keep the campaign outside the default gate; do not admit a new framework or dependency without the normal architecture and dependency review.
