...
- Filesystem fixtures must own unique asset identities and paths, including every path they clean up. A test must never overwrite or delete a normal Editor project's source, manifest, registry, material, cooked mesh, or cooked texture artifact. Repository-relative ignored `output/` is shared state, not an automatically isolated sandbox; use a unique fixture identity/directory or an explicitly scoped temporary root, and restore only state the fixture itself created. Do not run builds or tests concurrently with a user-visible headed Editor because legacy fixtures may still touch shared ignored outputs and trigger file watching.
...
| **Headed/platform** | Real editor interaction, graphics backend/device, capture, presentation, or external tool | 15 minutes per bounded script excluding a separately reported build | Launch the current Debug artifact, confirm the native backend/device and requested display, inspect the live result, exercise the claimed controls, preserve diagnostics, and state exact hardware/platform scope. |
...
Headed evidence separates launch, visual, and interaction claims. A running window proves only launch; a capture proves only the captured visual state; an automated state transition proves only the modeled callback path. When the feature claim includes mouse/keyboard behavior, the real current Editor must receive the documented input sequence in its live viewport and expose an observable camera/entity/UI result. Persisted workspace settings and navigation presets are test inputs: record them, and do not mistake a preset-specific no-op for a backend or renderer failure.
