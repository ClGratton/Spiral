...
P2A2 must extend that deterministic case or add one focused adjacent case proving the transaction itself retains the logical pool when the public wrapper is destroyed after recording but before submit; incomplete/poisoned/stale/wrong-device/unreserved state fails a non-mutating publication preflight before the simulated native submit callback; and an accepted preflight followed by a newly issued exact token publishes without a new logical failure path, then retires normally. Backend query methods and capability state must remain unavailable.
...
The planned benchmark-capture prerequisite must add deterministic tests for bounded retention/eviction, frame-ID continuity, percentile and 1%/0.1% fixtures, deadline-miss/overshoot aggregation, raw-spike preservation, unavailable-field preservation, and stable CSV/JSON schema/order. Bounded D3D12/Vulkan benchmark launches at maintainable target inputs must qualify engine trace completeness and condition metadata only; they cannot claim display cadence, VRR state, GPU headroom, or input latency unless a separate measurement source actually supplies those fields.

`Scripts/TestFramePacingBenchmark.ps1 -Backend D3D12` and `-Backend Vulkan` run bounded 60/120 FPS Responsive, InterFrame, and SubmissionGate conditions and write ignored `output/frame-pacing-benchmark/` schema-2 CSV/JSON artifacts. Optional `-PresentationMode`, `-SyncMode`, `-VrrMode`, and `-TearingMode` parameters publish runner-observed condition strings; their honest default is `unknown`. Each condition discards a declared 30-frame startup warm-up before retaining exactly 512 raw measurement frames; spikes inside that window are preserved. The engine retains lifecycle/wait/control-point rows plus deterministic nearest-rank p50/p95/p99 start-to-start, CPU-active, and intentional-wait summaries; 1% and 0.1% lows are `1000 / p99 frametime` and `1000 / p99.9` frametime, respectively. Requested and effective policy targets are recorded separately so a manifest cannot silently claim a cadence the frame policy did not use. The artifacts retain raw spikes and identify display, replacement/drop, input latency, and GPU headroom as unavailable. They prove capture completeness only, not displayed smoothness, external RTSS behavior, VRR state, input latency, or a selected candidate.

The external-capture attachment prerequisite runs independently of a collector. `Scripts/TestFramePacingBenchmark.ps1 -Backend D3D12 -ExternalAttachment` and `-Backend Vulkan -ExternalAttachment` own readiness/release files and run one released condition plus stale-run and timeout failures. After renderer/window initialization, the editor publishes schema-1 readiness with a unique run ID, exact PID/canonical executable path, QPC frequency/current tick, artifact location, and requested conditions, then waits on a bounded exact run/PID release before warm-up/capture. Released schema-2 artifacts carry the same identity plus nonzero Windows QPC ticks for every lifecycle marker; normal/default benchmark lifecycle rows retain `qpc=0` and make no Windows QPC call. The runner proves readiness before release and before artifact publication, exact identity, 512 released rows, stale-release rejection, timeout failure without a capture artifact, and no surviving owned process tree. Normal benchmark launches without attachment preserve existing behavior.

The implemented deterministic parser is `Scripts/JoinPresentMonCorrelation.ps1`. It accepts separate schema-2 engine JSON and PresentMon CSV paths, hashes them before and after parsing to reject changing raw inputs, requires the exact observed official 1.10.0 header order, one exact process ID, nonzero engine QPC frequency, continuous retained engine frames with one `PresentBegin`, and strictly monotonic unique QPC values. Every non-final engine present owns exactly one unused causal PresentMon row in its half-open interval; the final present owns exactly one row within the caller-declared bounded QPC tolerance. It atomically writes a schema-1 report that retains paths/hashes, identity, headers, counts, deltas, and original PresentMon present/display fields while leaving both raw streams and engine unavailable fields untouched. Warm-up and trailing rows remain counted rather than discarded.

`Scripts/CapturePresentMonCorrelation.ps1` is the Windows live supervisor. The caller supplies an explicit PresentMon executable and expected SHA-256; the script requires the tool to report exactly version 1.10.0, creates a new collision-safe capture directory and unique ETW session name, launches the exact Editor attachment gate, validates readiness PID/path/run/QPC/conditions, and starts PresentMon with exact `-process_id`, `-output_file`, `-qpc_time`, unique `-session_name`, `-terminate_on_proc_exit`, and `-no_top`. After the read-only exact `logman query -ets` readiness probe, both exact children must remain alive for a receipt-recorded 1500 ms settle interval and the exact session is revalidated before release. After Editor exit 0 and stable 512-frame engine input, the supervisor accepts only a stable exact-header/PID collector CSV and successful deterministic join; then, if PresentMon still lingers, it closes only its owned job, verifies no survivor, and re-hashes the raw CSV to require the joined hash unchanged. Process-tree cleanup and exact ETW-session cleanup are separate receipt boundaries: if the unique session survives its collector, a bounded official `-session_name <owned> -terminate_existing` helper targets only that invocation, and a helper failure or surviving exact session makes the capture fail closed. The receipt distinguishes `natural` from `owned-job-after-complete-capture`; a collector exit code is nullable only for the latter. This is complete raw/join evidence before cleanup, not a natural-exit requirement.
...
`EngineTests` verifies frame-pacing policy default resolution, runtime override/inherit resolution, target validation, transactional rejection of an invalid Smooth Frametime override, deterministic candidate deadlines/overruns/resets, and value-snapshot behavior across runtime changes. The editor `--frame-pacing-policy-smoke` writes and reads a version-1 legacy manifest (defaulting to Responsive), rejects an invalid version-2 target without mutating the caller's manifest value, then saves/reopens an opt-in 144 FPS project policy and emits `FramePacingPolicySmokeV1 ... candidate=InterFrame, behavior=experimental-inter-frame ... result=pass`. This smoke verifies settings persistence and resolved diagnostics; the backend candidate launches above own native timing/control-point evidence.
