...
`EngineTests --test "Texture artifacts cook the deterministic RGBA fallback transactionally"` is the focused CPU/Assets prerequisite evidence before KTX2 ingestion or GPU texture work. It proves normalized two-level RGBA8 fallback cooking, exact mip payload/range resolution, role/color-space and invalid-enum rejection, explicit unavailable `DesktopBC`/`Astc` rejection, atomic restoration after corruption, trailing-byte rejection, wrong-type and mismatched-provenance rejection, and caller-output preservation. The final MSVC Debug build completed with zero warnings/errors and the full suite passed 82/82. It does not decode KTX2 or another file format, admit libktx, transcode compressed targets, generate mips, upload a GPU texture, bind a sampler/descriptor, or qualify a renderer backend.
