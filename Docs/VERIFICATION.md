...
The revision must be `4d6fc70eaf62ad0558e63e8d97eb9766118327a6`, the checkout must be clean, and `LICENSE.md`, `LICENSES/`, and `NOTICE.md` must exist. Normal `Scripts/Setup.ps1` and `Scripts/Setup.sh` invoke this verification before Premake generation; an already exact clean checkout is accepted locally without another network fetch. The equivalent non-Windows direct acquisition is `bash Scripts/FetchDependencies.sh --include-ktx-software`. This verifies source identity and notice retention only; it does not prove private build exclusions, importer behavior, artifact cooking, staging, redistribution closure, or runtime qualification.
...
`EngineTests --test "RHI sampled texture-table binding validates pipeline space offsets and exact ownership"` is the deterministic prerequisite for native descriptor realization. Its deliberate fake device/command list accepts only the fixed `t0,s0,space1` declaration with Vulkan offsets 1/2 and an existing `b0,space0` constant buffer, rejects altered registers plus colliding space/offset declarations at pipeline creation, and rejects missing-pipeline or foreign-device table binding before its test-facing draw. Final local acceptance produced a zero-warning/error MSVC Debug build, the focused pass, and 85/85 Integration tests. It proves portable declaration and validation semantics only. D3D12/Vulkan adapters must reject the declaration until a native descriptor path and actual shader-visible sampled-resource consumer are separately exercised.
