...
Before transient allocation, deterministic tests must prove `Phase3TransientResourcesV1` selects `PlacedAliasedTransient` only when both RHI `PlacedResources` and `AliasingBarriers` lifecycle states are usable, and otherwise selects `NonAliasedGpuRetiredPool` with the GPU-retired reuse requirement retained in fallback diagnostics. Both headed renderer capability smokes must validate that the active backend publishes a valid, exercised group and emits the selected path. Current D3D12 and Vulkan/NVRHI evidence is the truthful non-aliased fallback on the named local RTX 3080 Ti; it does not prove a native aliasing path, any physical allocation, memory-cost accounting, token-gated resource reuse, or hosted fallback regression. The later allocation item must add real resource/lifetime evidence and prove attempted reuse remains unavailable until the exact completion token reports retired.
