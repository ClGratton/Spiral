...
The current prototype's visible shader demonstration is an object-space procedural checker on the cube. A headed capture must show alternating light/dark cells inside the non-background prototype geometry; the 2026-07-14 local `editor-viewport.bmp` capture was visually inspected after both `TestRender.ps1` and `TestVulkan.ps1` passed. This demonstrates interpolator and pixel-shader execution through the portable package, not sampled textures, samplers, material descriptors, or texture-asset upload.
