...
The planned Windows PresentMon prerequisite must start a uniquely named PresentMon session before the benchmark editor, retain the exact editor PID/path plus PresentMon version/arguments, and bound both process trees independently. Engine events must export QueryPerformanceCounter ticks and frequency. Verification preserves raw engine and exact-child-PID PresentMon streams, asserts the installed CSV header set, and accepts only a deterministic ordered one-to-one join between engine `PresentBegin` and PresentMon presents in the common QPC domain. Missing, ambiguous, non-monotonic, wrong-PID, stale-session, timeout, or incomplete-cleanup cases fail closed. PresentMon-derived present/display cadence or displayed/dropped classification must remain distinct from engine cadence, and monitor identity, VRR, RTSS/FES state, click-to-photon input latency, replacement semantics, and GPU headroom stay unavailable unless separately measured.
