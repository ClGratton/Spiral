...
The settings smoke must prove a missing settings file defaults to Fusion, a valid Unreal selection persists/reopens, malformed/unknown preset input is rejected without mutating the caller's value, and `.spiralproject` contains no navigation field. `ViewportNavigationSmokeV4` must prove at nonzero pitch/yaw that Fusion LMB/RMB are no-ops; MMB translates camera and persistent pivot by the same camera-plane vector; cursor wheel zoom preserves its ray/plane anchor, applies its multiplicative scale without crossing the near-plane safety distance, and remains active while MMB is held; and, after off-center zoom, Shift+MMB has exact zero-delta continuity, bounded small-delta displacement, stable pivot/distance/upright state, and approximate reverse-delta pose restoration. Unreal must retain alternate LMB/fly behavior. It must also prove public Window cursor capture/restore, double-precision Scene main-camera movement, camera/snapshot agreement, and one valid discontinuity epoch after F explicitly focuses an existing selected entity origin. A bounded headed Windows D3D12 launch must exercise the real viewport and inspect its post-navigation capture; no portable native OS gesture injector exists yet, so manual native Fusion/Unreal gestures remain a required acceptance gate. This is editor-perspective behavior only; it does not prove picking, multi-select, real geometry bounds, Autodesk custom/reset-pivot UI, bounds-aware Fit/Zoom-to-Fit, MMB-double-click fit, gizmos, orthographic navigation, camera piloting, or configurable bindings.

`Scripts/TestPresentMonCorrelation.ps1` provides focused synthetic success/failure coverage for wrong PID, missing/mismatched headers, missing rows, duplicate/reordered QPC, missing/ambiguous intervals, noncausal final input, and the successful report's distinct paired-row invariant. The saved ignored non-elevated D3D12 diagnostic capture passes the parser with 512 pairs, 16 warm-up rows, zero trailing rows, and unchanged raw engine/PresentMon files. The remaining Windows PresentMon production-pacing prerequisite launches the benchmark editor into the readiness gate, attaches a uniquely named bounded session to the exact PID, records tool version/arguments, then releases warm-up/capture and verifies both process trees. Its live failures (stale session, timeout, incomplete cleanup) remain separate from parser tests. PresentMon-derived present/display cadence or displayed/dropped classification must remain distinct from engine cadence, and monitor identity, VRR, RTSS/FES state, click-to-photon input latency, replacement semantics, and GPU headroom stay unavailable unless separately measured.
