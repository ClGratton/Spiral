...
The same scripts also run same-process `--smooth-frametime-target-change-smoke` transitions on InterFrame. Both 5-to-10 and 5-to-1000 FPS cases require the published old/new targets, the `next-valid-frame-boundary` contract, a completed frame with the new requested target, GPU-completion observation, bounded exit, and process-tree cleanup; this is accepted live target-propagation evidence. The current category assertions are not accepted source evidence: exact-head CI run `29652359689` observed the WARP 5-to-10 transition at 110.84 ms and classified it as synchronized presentation, while a local 5-to-1000 retry compared current-frame CPU work against the preceding cadence interval and classified `CpuActiveWork`. The source-aligned effective-limiter prerequisite must replace those single-record comparisons with explicit `N` cadence/`N-1` source associations and portable assertions before the scripts may qualify limiter categories. Neither current marker proves monitor refresh, actual displayed cadence, or a production pacing choice.
