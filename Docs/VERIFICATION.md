...
```bash
./bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --test "Renderer color pipeline settings validate manual exposure"
./bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --tier integration
bash Scripts/TestVulkan.sh Debug gmake --skip-build
```

The focused test covers default `0.0`, finite `[-16,+16]` bounds, nonfinite rejection, transactional Renderer publication, and the `scale=exp2(-EV100)` sign convention. The Editor smoke marker `ColorPipelineSettingsSmokeV1 default=pass bounds=pass nonfinite=pass v3Migration=pass saveReopen=pass rendererPublication=pass` covers project schema migration, invalid-load transactionality, persistence, reload, and live Renderer publication. The native Vulkan harness must also emit `SceneExposureControlV1 backend=Vulkan ev100=-2,0,+2 graph=exact-byte-pass monotonic=pass constants=immutable-retained-cached`, proving that graph and direct-reference recordings used the same retained immutable constants, unchanged EV reuses the current immutable constants, changed EV allocates a distinct immutable generation, and higher EV darkens the rendered output. These commands qualify only the native Vulkan run actually performed; D3D12 shares the source path and baseline zero-EV script expectation, but D3D12 multi-EV shader-visible monotonic execution still needs Windows evidence before it can be claimed.
