...
Corrected hosted CI run `35330957687` / Windows job `105555027314` is rejected pre-test evidence: `Engine` and `Editor` compiled, but MSVC reported `C1128` for the monolithic `EngineTests.cpp` object exceeding the standard section limit (with an accompanying `D8040` compiler-child-process diagnostic). The correction adds `/bigobj` only under EngineTests' Windows Visual Studio Premake filter; it changes neither Engine targets nor non-MSVC projects/actions. The log's two `LNK4006` duplicate import-descriptor warnings arise while linking `Engine` (`Shell32`/`Advapi32` versus `slang.lib`) and are not a new test result. No test ran, no checkbox changes, and the required Windows rerun remains pending.

Hosted CI run `35331589680` / Windows job `105557020138` proves the full MSVC compile/link and `/bigobj` emission, but is rejected as Fab qualification evidence. The prior renderer-first workflow then failed the unrelated D3D12 `RenderGraphExecutionSmokeV1 ... result=fail`, encountered an ImGui OpenGL2-shutdown assertion, and timed out before its CPU `EngineTests` step. The workflow now builds once, runs the complete Windows EngineTests integration registry and runner contract plus the existing CPU headless smokes, then invokes the still-required D3D12 `TestRender.ps1` gate with `-SkipBuild`; it neither suppresses nor continues past render failure. The complete registry executes the declared URL-policy, receipt-atomicity, immutable rooted-resolver/registry-migration, and three secure-directory snapshot boundaries before renderer. A reordered hosted run must execute and pass those CPU boundaries before Windows Fab parity can be qualified; it still does not repair or qualify the D3D12 gate.

Local generation proved all three Debug/Release/Dist `EngineTests.vcxproj` configurations contain `/bigobj` and that neither `Engine.vcxproj` nor `EngineFuzzTests.vcxproj` receives it. After restoring the GMake graph, the warning-free Debug `Editor`/`EngineTests` build, all three snapshot focused tests, 122/122 Integration tests, code style, and diff checks passed. Raw logs are retained under ignored `output/verification/fab-windows-parity-ci-bigobj-correction-20260918/`; native MSVC execution remains the qualification gate.
...
Corrected run `35362360458` / Windows job `105656686770` reached the staging privacy gate and reported that the runner-created parent was not owned by `TokenUser`. Windows may assign the current access token's distinct `TokenOwner` SID to newly created objects, especially for an elevated token. The predicate now accepts either exact current-token SID while preserving its protected-DACL and trusted-ACE checks. Because this change is wholly inside the Windows preprocessor branch, the local gate is code style, diff validation, and targeted control-flow review; native hosted execution remains the only behavioral qualification.
...
The current harness requires the one-shot `VulkanCompletedSubmissionCollectionV1 collections=8 result=pass` aggregate marker together with two ready seven-scope RenderGraph timestamp records. The marker proves eight first-terminal collection requests occurred on that device; the timestamp records independently prove two completed seven-pass graph frames. Together they do not attribute any individual collection to a specific graph context. The 2026-09-03 RTX 3080 Ti acceptance used the then-four-pass graph and also kept a guarded 21-second actual Debug Editor run between 1,098 and 1,183 MiB after a 1,077 MiB baseline, returned to 1,068 MiB after exit, and completed a debugger-owned RMB/W/A/S/D/mouse/release/F sequence plus normal close near 1.2 GiB after 25,640 frames. This bounded observation does not prove an exact driver-level reclaimed-byte quantity, a long soak, other devices, or other Vulkan implementations.
...
`EngineTests --test "Cooked mesh artifacts validate and resolve transactionally"` is the focused Integration boundary after the shared build. It proves schema-2 `SpiralMeshArtifact` round-trip and atomic replacement for `PositionNormalColorUV32F` vertices, explicit primitive byte ranges, and `UInt32` triangle indices; schema-1 migration to deterministic geometric normals; finite normalized default-cube normals; and invalid-normal and malformed/range rejection without mutating a destination artifact. Direct public completion-helper cases cover NaN, infinity, non-unit, and mixed authored/missing normals with bytewise failure atomicity. Registry resolution rejects missing artifacts, wrong asset types, and mismatched source provenance without partial publication; Renderer publication uses an immutable copied registry catalog. The resolver test proves a published catalog remains valid after the mutable registry changes, a replacement snapshot rejects a removed mesh handle, and that failure preserves the caller's prior artifact.

`EngineTests --test "glTF geometric normals preserve authored data and derive deterministic fallback"` exercises fixed authored, both-winding missing-normal, malformed-count/type, nonfinite-normal, and degenerate triangle fixtures. Deterministic wire-format fixtures also prove legal normalized signed `BYTE`/`SHORT` conversion under `KHR_mesh_quantization` and reject unsigned, unnormalized, undeclared, optional-only, and required-only integer NORMAL cases transactionally. It independently checks authored direction preservation, winding-derived fallback, and all-or-nothing registry/artifact publication. `EngineTests --test "Scene surface basis and material rows publish deterministically"` checks stable numeric material-row assignment across snapshot order, duplicate reuse, invalid/missing/registered-wrong-type/missing-library routing to row zero, the exact row-zero constant state, distinction from persistent handles, nonuniform row-vector `S^-1 * R` constants, complete-frame singular-transform rejection, and the exact shared 256-byte constant layout. `EngineTests --test "Submitted render graph frames retire exact tokens without waiting"` includes the shared Scene constants in exact completion-token retirement evidence.

The native Vulkan acceptance command remains `Scripts/TestVulkan.sh Debug gmake --skip-build`. The script preserves the comprehensive Scene invocation and its production `PSMain` material-texture coverage, then starts a second independently bounded `--scene-surface-basis-material-id-smoke` process. That process must require `SceneSurfaceBasisMaterialIdV1 backend=Vulkan invocation=dedicated productionPSMain=preserved`, the authored oblique `(1,2,3)` normal with nonuniform `(1.3,0.7,2)` scale and nonzero Z rotation, independently computed expected direction/color versus numeric readback within tolerance, `materialId=1`, `normalTransform=S^-1*R`, `interface=production-scene`, `retention=exact-graph-token`, and `result=pass`. The diagnostic pixel entry shares the production vertex layout and constants but is compiled only for that separate invocation; it does not replace ordinary Editor or comprehensive Scene shading. This evidence does not qualify D3D12/MoltenVK execution, PBR, tangents, normal-map basis correctness, light payloads, photometric attenuation, shadows, or visibility-buffer IDs.

Final Linux acceptance is retained under ignored `output/verification/scene-surface-material-20260904-repair` and `output/verification/scene-surface-material-20260904-headed`. The incremental Debug GMake build emitted no compiler warning/error, the post-repair Integration registry passed 106/106, style and cached-diff checks passed, and the full native RTX 3080 Ti Vulkan harness emitted the required `SceneSurfaceBasisMaterialIdV1` marker with readback `175,215,199` matching the independently calculated expected value. The first dedicated run usefully failed: it attempted to replace the 48x36 output with 64x48 while the first graph token was still pending, and its initialization-failure path then destroyed the Vulkan context before a surviving Scene renderer dereferenced its device during shutdown. The accepted repair performs an explicit first-output readback/retirement before resize and resets the Scene renderer before context shutdown; the complete rerun passed and created no new core. The actual Debug Vulkan Editor then visibly rendered the textured native Scene on DP-3/workspace 2. Compositor-native Wayland pointer input selected Directional Light and exposed its lux/light controls, reselected Prototype Mesh and restored its material Inspector, and window-manager close produced normal Application, job-system, renderer, and GLFW teardown after 123,198 frames with no new core dump.
...
The current valid-Scene viewport consumer builds seven imported-resource Graphics passes per render on D3D12 and Vulkan: `Scene Light Payload Copy`, `Scene Primary Directional Shadow Map`, `Scene Viewport Graph Clear`, `Scene Sky Atmosphere`, `Scene Viewport Graph Raster`, `Scene Viewport Graph Tone Map`, and `Scene Viewport Graph Output Handoff`. An active selected-bounds overlay redirects tone mapping into a distinct RGBA8 intermediate and inserts `Scene Debug Overlay` before handoff as pass eight. Zero visible meshes remain a valid seven-pass Scene: only geometry/shadow draw lists become empty, while clear, sky, tone map, and handoff still execute. The executor records those stable pass names as RHI debug markers around the restricted callbacks; the output handoff finishes the color target in `ShaderResource` for the unchanged native presentation/ImGui bridges. Under `--scene-viewport-render-graph-smoke`, both backends also create separate renderer-owned reference color/depth/shadow outputs, render the independent shadow and sky references, explicitly rebind color plus depth after the sky's color-only pass, replay the main direct recorder with the same immutable prepared frame, pipelines/resources, constants, retained artifact-resolver snapshot, and draw order, and compare graph/reference RHI readbacks for equal extent, row pitch, and exact bytes. The conditional reference path owns its own tone-mapped intermediate and overlay. The reference raster samples only its independently rendered shadow depth, so the comparator cannot inherit graph-produced shadow contents. The graph output is restored to `ShaderResource`; reference outputs are never exported or presented. `SceneViewportRenderGraphV1` validates its actual compiled ordered labels and accepted-pass count before reporting success. The ordinary harness requires `passes=7 labels=light-payload-copy,primary-directional-shadow-map,clear,sky-atmosphere,raster,tone-map,output-handoff execution=pass reference=direct comparator=exact-byte-pass`; the dedicated debug harness additionally requires the exact eight-pass sequence containing `debug-overlay` and a later seven-pass fallback. Exact-head CI run `29430304670` passed the earlier three-pass comparator markers on Windows D3D12, Ubuntu Vulkan/lavapipe, and macOS Intel/MoltenVK; dependency submission `29430304809` passed. Those runs are historical rather than evidence for the expanded graph.

Production graph lifetime is asynchronous. `SubmittedRenderGraphFrameOwner` retains a bounded frame's graph contexts, exact accepted tokens, frame/pass identity, and external per-frame constants, light-payload, sky, shadow, optional debug-overlay, and artifact-resolver resources until nonblocking completion; ordinary frames do not call `WaitForCompletion` between graph submission and presentation. Deterministic `EngineTests` must prove payload retention, stable labels and exact tokens, accepted-prefix failure retention, explicit four-frame capacity, truthful failed-token state, and zero calls to the fake device's wait method. Current `TestRender.ps1`, `TestVulkan.ps1`, and `TestVulkan.sh` require at least two `ProductionRenderGraphRetirementV1 backend=<backend> frame=<id> passes=7 cpuWaitBetween=no pending=<n> result=pass` markers plus the exact-byte comparator. The dedicated selected-bounds path must independently retire eight accepted passes. The 2026-07-16 RTX 3080 Ti three-pass runs and 67-test build remain historical evidence only; the expanded Linux/Vulkan graph passed on 2026-09-04. Device-idle release is a shutdown/device-replacement rule, not a normal-frame pacing mechanism.

P3A instruments each accepted effective-Graphics production RenderGraph pass with a separate two-query transaction and retains its pool/generation through the same asynchronous owner. Deterministic `EngineTests` prove reset/start/work/end/resolve ordering in caller and worker lanes, exact frame/pass/token/generation identity, failed-token disjoint publication without release, independent-queue rejection before recording, the current 32-state retirement bound for four maximum-shape eight-pass frames, and zero wait calls. `TestRender.ps1`, `TestVulkan.ps1`, and `TestVulkan.sh` require at least two ordinary `RenderGraphTimestampScopesV1 backend=<backend> frame=<id> scopes=7 raw=ready cpuWaitBetween=no result=pass` markers; the dedicated selected-bounds path must also produce an eight-scope retirement. The 2026-07-16 D3D12/Vulkan three-scope runs and 68-test build are the historical initial acceptance; current seven/eight-scope evidence belongs to their respective Linux/Vulkan gates. These are raw endpoints only: P3B owns tick conversion, exact-frame Profiler publication, capability-path promotion, and CPU fallback evidence.

P3B converts only a complete exact-frame set of P3A scopes on one effective Graphics clock. Deterministic `EngineTests` prove named-pass end-minus-start conversion, the non-summed first-start-to-last-end whole-frame interval, exact retained-frame amendment, cross-queue/generation rejection, and explicit ready/pending/unavailable/disjoint states. They also launch 32 simultaneous per-pass reservations against one retirement queue, require all distinct reservations to survive, reject the thirty-third, and prove destruction releases the bound; this is regression coverage for the worker-recording race exposed by the Vulkan headed smoke. `TestRender.ps1`, `TestVulkan.ps1`, and `TestVulkan.sh` require ordinary `RendererGpuTimingV1 backend=<NVRHI D3D12|NVRHI Vulkan> frame=<id> passes=7 wholeMs=<positive> status=Ready capability=GpuTimestamps result=pass`; the PowerShell D3D12 harness requires at least two ready publications and Vulkan requires at least one before its close gate. A selected-bounds frame reports eight named passes instead. An initialization-smoke result with no retained application timing frame emits `RendererGpuTimingDropV1 ... status=Unavailable ... result=ignored` and must not fail rendering or amend another frame. The 2026-07-16 three-pass RTX 3080 Ti build and 70-test evidence remain historical; the current seven-pass Linux/Vulkan result is recorded in the 2026-09-04 sky acceptance.
...
```text
SceneBasicPbrMaterialIdV2 backend=Vulkan productionPSMain=exercised brdf=GGX-Smith-Schlick-Burley materialIds=stable rowZero=error view=per-pixel-view-space lighting=typed-directional-lux sceneLights=consumed hdr=float32-before-pre-exposed-finite-storage retention=exact-graph-token result=pass
```

The V1 neutral-preview marker remains historical evidence for the original material-only closure. Current source and the full harness require V2, which preserves the same five-panel independent BRDF oracle while supplying its established 4-unit fixture through a typed directional-lux Scene record. This is Linux/Vulkan execution only. Shared D3D12 reflection and 512-byte aligned constant allocation compile from the same source, but D3D12 PBR execution remains unqualified. The marker does not claim tangent-space normal mapping, Callisto/Proxima control consumption, two-sided material policy, shadows, or indirect lighting.
...
## Phase 3E Typed Scene-Mutation Authority

After one coherent Debug build, run the focused Scene mutation and legacy migration regressions, bounded color/history smoke, complete mailbox/helper smoke, Integration registry, and native Vulkan harness:

```bash
bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --test "Scene full-transform mutation validates and preserves prior state"
bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --test "Scene loads legacy absolute transforms"
bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --test "Photometric light schema publication and diagnostics are transactional"
bin/Debug-linux-x86_64-gmake/Editor/Editor --renderer-vulkan --color-pipeline-settings-smoke
bash Scripts/TestEditorMaterialControl.sh
bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --tier integration
bash Scripts/TestVulkan.sh Debug gmake --skip-build
```
...
## Typed Editor Scene Control V3

After one coherent Debug build, run the material-control regression and the schema-3 scene-control smoke headlessly. The script retains its historical `V2` filename, but its request, session, receipt, target, and terminal-marker contracts are all schema 3. Do not use that test script to launch a user-visible Editor; the canonical headed transaction is separate:
...
bash Scripts/LaunchHeadedEditor.sh
```

The helper executes 30 ordered requests covering entity inspection/selection, typed light mutation/restoration, complete mesh transform mutation/restoration, complete project color-pipeline mutation/restoration, dedicated main-camera pose mutation/restoration, Scene debug-view/selected-bounds publication, and MeshRenderer visibility/shadow-flag mutation/restoration. It must report 19 successes, 11 rejections, exactly 10 successful document mutations/history entries, two injected post-mutation failures with verified full rollback (Transform and debug publication), and final state equality. Stale compare-and-swap requests cover selection, light, transform, color pipeline, camera pose, debug state, debug selected-entity identity, and mesh flags. Debug receipts must verify the immutable Renderer publication and a later restoration generation without adding document history. A generic transform targeting the main camera must fail with `main_camera_pose_action_required`; an invalid non-unit camera pose must be rejected by the client without creating a request. Four raw owner-only fixtures separately prove server rejection of wrong project, unexpected field mask, duplicate entity field, and stale schema 2. Persistent project, scene, asset-registry, and material bytes must remain unchanged, and every receipt reports `SessionOnly` plus `Saved=no`.

Use the `controlDir` printed in the `HeadedEditorLaunchV3` receipt as the target for `Scripts/EditorMaterialControl.py`; write captures to a fresh ignored evidence directory. Headed acceptance requires that one exact live process/window, stable DP-3/workspace-2 placement, native `NVRHI Vulkan` Scene-output/handoff markers, and typed inspection/mutation receipts. The mesh-hidden capture must retain the analytic sky instead of blanking the complete viewport. No mouse, keyboard, virtual pointer, or UI widget mutation is permitted. The accepted PID 632356 V3 run visibly covered all four debug views, selected bounds, mesh-hidden sky, restored mesh, and exact final state. The earlier V2 run remains historical evidence for light, transform, color, and camera actions. Compositor transparency differs between captures, so receipt/state equality and persistent-byte fingerprints—not whole-window PNG equality—are the exact restoration oracle.

The required terminal marker is:

```text
EditorSceneControlV3Test helper=typed schema=3 requests=30 succeeded=19 rejected=11 headless=pass vulkan=skipped security=project-mask-duplicate-rejected cas=selection-transform-light-color-camera-debug-mesh stale=rejected selectedIdentity=bound mainCameraAuthority=dedicated invalid-camera=client-rejected rollbacks=2-verified history=one-per-document-action restore=exact save=not-invoked persistentBytes=unchanged input=no-ui-synthesis result=pass
```

This qualifies only the fixed Editor-owned schema-3 repository-verification surface on Linux/Vulkan. It does not qualify implicit save, arbitrary dispatch, entity/component lifecycle, secondary-camera view control, provider/model integration, generic Automation infrastructure, D3D12, or MoltenVK.

## Phase 3E Shader-Visible Scene Light Payload

After one coherent Debug build, run the focused transactional payload oracle, complete Integration registry, the dedicated native diagnostic, and the comprehensive Vulkan harness:
...
bin/Debug-linux-x86_64-gmake/Editor/Editor \
  --vulkan-render-smoke \
  --vulkan-scene-viewport-raster-smoke \
...
The focused oracle constructs its expected versioned header, complete directional/point/spot records, compact global list, every CSR offset, and local-light list independently of the production packer and compares the complete word sequence. It corrupts copied color, derived view direction, global/local type membership, CSR monotonicity, tile dimensions, projection clip metadata, source identity, photometric unit, checked cluster arithmetic, view index, and generation; every rejection must retain the caller's prior generation and words. The publisher must retain at most four device-affine slots, reuse a free equal-size allocation, and keep the last accepted output unavailable for overwrite until a newer graph publication commits.
...
```text
SceneLightPayloadV3 backend=Vulkan layout=versioned-uint4 records=directional-point-spot-prepared tables=global-csr-local preExposure=header-scale cpuGpu=exact-pass copy=graph staging=cpu-write gpu=structured-copydest slots=4 allocations=2 reuses=1 retention=exact-graph-token productionPSMain=separate lightingEvaluation=not-exercised result=pass
```

The comprehensive harness additionally requires the seven-pass `light-payload-copy,primary-directional-shadow-map,clear,sky-atmosphere,raster,tone-map,output-handoff` graph, seven ready timestamp scopes, exact graph/direct viewport bytes, resize, ImGui handoff, presentation, and cleanup. The 2026-09-04 V2 Linux acceptance passed the warning-free Debug GMake build, focused oracle, 111/111 Integration registry, style/diff/script checks, and complete native RTX 3080 Ti Vulkan harness. Current V3 verification extends each light to seven words and independently checks the prepared RGB coefficient, reciprocal spot-cosine span, and reciprocal range while preserving the same GPU-only upload, graph, and lifetime requirements. Shared D3D12 production source is present but has not been executed by this Linux evidence; the payload diagnostic itself does not exercise lighting or shadows, which are covered by their separate production oracles, nor a GPU-built cluster grid, D3D12, or MoltenVK.
...
The payload oracle independently derives Rec.709 luminance normalization, point `lumens/(4*pi)`, the exact squared-cosine spot solid-angle integral, hard-cone behavior, and reciprocal range. It covers black and zero-flux output, range-zero locals, zero-flux zero-solid-angle spots, positive-flux zero-solid-angle rejection, float-collapsed cones, nonrepresentable reciprocal range, and the 64-local shader bound without replacing a prior valid payload.

The dedicated native entry renders zero-light, directional, point, and spot cases through ordinary production `PSMain`. It compares raw RGBA16F samples against an independent double-precision BRDF/light oracle, then independently tone-maps and sRGB-encodes the quantized half values for RGBA8 comparison. Its fixture must reject reversed directional sign, omitted `4*pi`, omitted inverse-square/range attenuation, unnormalized spot flux, and linear, cubic-smoothstep, or hard falloff in place of the accepted squared ramp. The sampled point and spot must exist in the independently built CPU cluster with zero overflow. It must emit:

```text
ScenePhotometricDirectLightingV1 backend=Vulkan productionPSMain=exercised types=directional-point-spot units=lux-lumens color=Rec709-luminance-normalized point=lm-over-4pi spot=flux-normalized-squared-cosine distance=inverse-square-smooth-range clusters=bounded-csr direction=emission-forward hdr=independent-half-pass ldr=independent-pass overflow=0 retention=exact-graph-token result=pass
```

The larger production pixel shader initially produced a valid SPIR-V module but triggered NVIDIA's internal `NVVM compilation failed: 3`. Production Scene shader requests now use `-O3`, and payload validation uses explicitly bounded arithmetic under the four-million-word ABI cap instead of expanding overflow-helper call graphs. `spirv-val` and real pipeline creation are both required; successful offline compilation alone is insufficient.

The oracle also exposed a pre-existing Vulkan orientation defect: NVRHI converted the viewport to negative height while the portable SPIR-V compiler already applied `VulkanInvertY`, producing two Y flips. The Vulkan NVRHI adapter now reverses the input Y interval so native conversion and shader compilation produce exactly one flip. The light fixture is intentionally asymmetric in Y and its LDR readback fails under the former orientation. The complete native harness and a headed Editor inspection must remain the regression gates for this adapter-wide correction.

The 2026-09-04 acceptance passed the warning-free full Debug GMake build, both focused transactional tests, 111/111 Integration tests, code-style/bash/diff gates, dedicated payload V3/PBR V2/direct-light processes, and the complete native RTX 3080 Ti Vulkan harness. The direct diagnostic read `zeroHalf=0,0,0`, `directionalHalf=12963,12963,12963`, `pointHalf=12757,12757,12757`, `spotHalf=12757,12757,12757`, `q=0.5`, and exact first-channel LDR `0,114,105,105`; every named counterfactual was rejected. PowerShell syntax was not locally parsed because `pwsh` is unavailable, but `TestVulkan.ps1` carries the same marker updates.

For the headed check, activate DP-3/workspace 2 before process creation with Hyprland 0.56's Lua dispatcher, then verify the new exact Editor address reports monitor 1/workspace 2 and that monitor 1 maps to DP-3. A query after an operator manually moved the window is invalid placement evidence. The actual Debug Vulkan Editor exposed an authored-content calibration issue: the existing saved sample's 30,000-lux directional at legacy EV100 0 clips to white. A typed `SetProjectColorPipeline` request changed only the live session to EV100 11, after which the textured blue/orange cube and directional response were visible. This proves the renderer responds coherently to the project exposure authority; it does not make EV11 a global default or persist a demo-scene decision.

This evidence qualifies production directional/point/spot photometric direct lighting only on Linux/Vulkan. It does not qualify shadow sampling, indirect lighting, volumetrics, IES profiles, GPU-built clusters, calibrated spectral radiometry, D3D12 execution, or MoltenVK execution.

## Phase 3E Stable Primary-Directional Shadow Map

Use focused CPU and native visual checks while editing, then run the complete harness only at the coherent slice boundary:

```bash
bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --test "Scene shadow map preparation is stabilized and classifies caster modes"
bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --test "RHI timestamp query retirement serializes concurrent pass reservations"
bin/Debug-linux-x86_64-gmake/Editor/Editor \
  --vulkan-render-smoke \
  --vulkan-scene-viewport-raster-smoke \
  --scene-shadow-map-smoke \
  --scene-viewport-render-graph-smoke \
  --frame-task-single-thread
bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --tier integration
bash Scripts/TestVulkan.sh Debug gmake --skip-build
bash Scripts/CheckCodeStyle.sh
git diff --check
...
The preparation oracle must prove deterministic first-eligible directional selection; a finite camera-relative zero-to-one light transform; quantized extent and 1024-map texel snapping stable under sub-texel camera movement; explicit constant/slope bias; and transactional retention on malformed bounds or light state. It classifies opaque and masked casters, excludes blend and component-disabled casters, and preserves a conservative error-caster path with counted diagnostics. The timestamp oracle reserves all 24 states required by four retained six-pass frames, rejects the twenty-fifth, and releases the complete bound without a wait.

The native fixture must execute the production depth-only pass and ordinary `PSMain`, not a diagnostic shadow shader. It compares final color readbacks with component casting enabled and disabled, requires a nonzero set of darker target pixels and a nonzero maximum difference, and requires the independent control region to remain identical. It must emit:

```text
SceneShadowMapVisualV1 backend=Vulkan fixture=receiver-plus-offset-caster light=directional-30deg resolution=1024 stabilization=texel-snapped filter=3x3-pcf casterToggle=component target=32,24 targetBrightness=<enabled>,<disabled> darkerPixels=<positive> maximumDelta=<positive> controlDelta=0 depthOnly=production-pass finalColor=readback-differential result=pass
```

The ordinary graph comparator must build and render a distinct reference D32 shadow map before its reference raster. Reusing graph-produced shadow depth is invalid evidence even if the final color bytes happen to match. The 2026-09-04 Linux/Vulkan acceptance passed the warning-free Debug build, both focused oracles, 112/112 Integration tests, the complete boundary harness, style/Bash/diff gates, exact six-pass comparison, six ready timestamp scopes, resize, ImGui handoff, presentation, and clean shutdown. The canonical headed launcher then proved the exact current-worktree Debug executable/cwd, explicit NVRHI Vulkan, and DP-3 monitor 1/workspace 2 before a typed session-only EV11 adjustment made the existing cube's texture and directional response visible. Because that saved Scene contains only one cube and no separate receiver, the native differential fixture owns the visual cast-shadow claim; the headed image owns real-Editor/backend/presentation inspection. D3D12 implements the shared depth-only contract, typeless R32/D32 view pairing, colorless pipeline, material-table-plus-fixed-SRV binding, and static fixed sampler, but remains source-only until executed on Windows. Cascades, atlases, receiver-aware caster exclusion, proxy/cached shadows, transparent colored transmission, sparse ray residuals, and MoltenVK execution are not claimed.

## Phase 3E Basic Analytic Sky And Live Catalog Coherence

Use the focused CPU/material-lifetime checks while editing, then the dedicated native sky process and complete Vulkan harness at the coherent slice boundary:

```bash
bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --test "Scene sky atmosphere preparation is deterministic and bounded"
bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --test "Scene surface basis and material rows publish deterministically"
bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --test "Material texture binding resolves semantic slots samplers and fallbacks"
bin/Debug-linux-x86_64-gmake/Editor/Editor \
  --vulkan-render-smoke \
...
The sky CPU oracle independently checks first-directional sun selection, fixed turbidity/albedo/solar radius, finite Preetham/Perez coefficients and zenith values, linear authored-lux scaling, deterministic 512-sample upper irradiance, lower ground bounce, disabled below-horizon state, exact 160-byte sky constants, exact 512-byte surface constants, and transactional rejection. The native oracle executes the production sky and surface shaders, distinguishes upper dome, ground, solar disk, enabled/disabled indirect surface response, scans the complete RGBA16F image for nonfinite values, and repeats after resize. Its required terminal marker is:

```text
SceneSkyAtmosphereVisualV1 backend=Vulkan model=Preetham1999 turbidity=3 groundAlbedo=0.1 sun=first-directional angularRadiusDegrees=0.266 exposureEV100=11 skyDome=production-pass diffuseIrradiance=first-order-zonal ormOcclusion=indirect-only upper=<rgb> upperBrightness=<positive> ground=<rgb> groundBrightness=<positive> sunPeak=<positive> sunPixel=<xy> sunHdrSaturatedPixels=<positive> totalHdrSaturatedPixels=<positive> surfaceAmbientBrightness=<positive>,0 hdrFinite=pass resize=pass result=pass
```

Live publication is a frame-coherence test, not an input-throttling test. The two focused material tests capture one exact artifact-resolver snapshot, publish a changed material catalog after preparation, prove the old and new generations remain independently resolvable, and require unchanged texture content to reuse the same binding handles. The ordinary native Vulkan smoke then performs that publication after `PrepareCurrentSceneRasterFrame` but before `RenderCurrentSnapshot`; the already-prepared frame must render and read back successfully, and the next preparation must adopt the new generation. The harness requires:

```text
SceneArtifactSnapshotContinuityV1 backend=Vulkan publication=after-prepare inFlight=retained-generation-rendered nextFrame=new-generation-prepared retainedGeneration=<old> liveGeneration=<new> result=pass
```

This regression applies to every publication path sharing the catalog—live material values/samplers, import/reimport, and file watching. Moving edits to widget release, debouncing them, retaining only material values while rereading current texture/mesh resolvers, or accepting a blank/error frame is a failure. The real Debug Editor must still be launched only through `Scripts/LaunchHeadedEditor.sh`, verified on DP-3/workspace 2 after native handoff, and visually inspected at a usable session-only exposure. D3D12 shares the retained-snapshot implementation and graph source but remains unqualified until the corresponding Windows runtime is executed; the sky gate does not claim authorable atmosphere/weather, aerial perspective, volumetrics, clouds, night sky, or specular environment lighting.

## Phase 3E Scene Debug Views And Selected Bounds

After the coherent Debug build, run both focused CPU contracts, the schema-3 controls, the dedicated native Vulkan diagnostic, the complete Integration registry, and the boundary harness:

```bash
bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --test "Scene debug visualization publication is monotonic and frame coherent"
bin/Debug-linux-x86_64-gmake/EngineTests/EngineTests --test "Scene debug visualization overlay clips and packs transactionally"
bash Scripts/TestEditorMaterialControl.sh
bash Scripts/TestEditorSceneControlV2.sh
bin/Debug-linux-x86_64-gmake/Editor/Editor \
  --vulkan-render-smoke \
...
The publication oracle must prove valid enum parsing, no-op suppression, serialized monotonic multi-writer generation, and immutable prepared-frame retention when a newer live setting is published. The overlay oracle computes the selected resolved mesh AABB, clips all twelve edges in zero-to-one homogeneous clip space before division, emits finite normalized top-left coordinates, packs exact 224-byte GPU constants, and rejects malformed input transactionally. Hidden, missing, disabled, invalid, or fully clipped selections emit no overlay.

The native fixture must use ordinary production `PSMain` and the post-tone-map overlay. It reads back distinct exact diagnostic colors for `Lit`, `MaterialId`, `GeometricNormal`, and `ShadowCaster`; Material ID must derive from the persistent 64-bit material handle rather than the frame-local row or selection state. Publishing a new live debug setting after preparation must not alter that prepared frame. Selected bounds must change real output pixels through the conditional eighth pass without changing an interior control pixel; disabling bounds must restore the exact seven-pass graph. Finally, hiding every mesh must retain a nonuniform analytic sky and valid output. The required terminal marker is:

```text
SceneDebugVisualizationV1 backend=Vulkan modes=Lit,MaterialId,GeometricNormal,ShadowCaster settings=immutable-prepared readback=exact-diagnostic selectedBounds=post-tone-map overlayPixels=<positive> fullOpacityBlendPixels=<positive> graphPasses=8-on,7-off emptyVisibleMeshes=sky-retained result=pass
```
