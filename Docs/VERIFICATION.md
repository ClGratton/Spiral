...
The planned Windows PresentMon prerequisite launches the benchmark editor into that gate, attaches a uniquely named session to the exact published PID, then releases warm-up/capture. It retains PresentMon version/arguments and bounds both process trees independently. Verification preserves raw engine and exact-child-PID PresentMon streams, asserts the actual collector CSV header set, and accepts only a deterministic ordered one-to-one join between engine `PresentBegin` and PresentMon presents in the common QPC domain. Missing, ambiguous, non-monotonic, wrong-PID, stale-session, timeout, or incomplete-cleanup cases fail closed. PresentMon-derived present/display cadence or displayed/dropped classification must remain distinct from engine cadence, and monitor identity, VRR, RTSS/FES state, click-to-photon input latency, replacement semantics, and GPU headroom stay unavailable unless separately measured. The local FrameView `1.7.12119.0` binary exits without CSV, while the official portable Intel/GameTechDev `1.10.0` console asset produced the exact header set and PID-targeted rows in an ignored diagnostic capture; live supervision and correlation are still unimplemented.

Before live supervision, a deterministic parser/join helper must accept separate schema-2 engine JSON and PresentMon CSV paths plus an output report path. It requires the exact official 1.10.0 console header set, one process ID, monotonic QPC values, the engine QPC frequency, and one `PresentBegin` per retained frame. Warm-up rows before the first engine present and trailing rows remain explicitly counted. For every engine present except the last, exactly one unused PresentMon row must be causal and earlier than the next engine present; the last requires exactly one row within a declared tick/millisecond tolerance. Synthetic fixtures must reject wrong PID, missing headers/rows, duplicate or reordered QPC, ambiguous intervals, row reuse, and noncausal input. A real ignored diagnostic capture must produce 512 pairs while retaining the original engine JSON and PresentMon CSV unchanged.
