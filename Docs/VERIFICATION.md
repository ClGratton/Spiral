...
`EngineTests --test "Cooked mesh artifacts validate and resolve transactionally"` is the focused Integration boundary after the shared build. It proves schema-1 `SpiralMeshArtifact` round-trip and atomic replacement for `PositionColorUV32F` vertices, explicit primitive byte ranges, and `UInt32` triangle indices; malformed/unsupported headers and out-of-range-index rejection without mutating a destination artifact; and registry resolution that rejects missing artifacts, wrong asset types, and mismatched source provenance without partial publication. The bounded `Editor.exe --smoke-test --gltf-import-smoke` additionally writes a minimal glTF triangle, imports it through the real editor workflow, resolves the registered cooked artifact, and requires three vertices and three indices. The headless form currently fails an unrelated frame-input lifecycle invariant before import and is not accepted evidence for this slice. This validates cooked artifact publication/resolution only; it does not detect changed source dependencies, upload buffers, replace the viewport prototype, create descriptors/materials/textures, or qualify any renderer backend/device.
