...
`FrameLifecycleTelemetryV1` now requires `frame-start,input-sample,input-simulation,render-submission,present-begin,present-end` for one exact application frame. The active GLFW poll completes after `Renderer::BeginFrame` and before immutable frame-input publication; minimized windows poll without inventing a timing sample. Deterministic validation accepts only one finite nonnegative same-frame sample after FrameStart and before InputSimulation and proves every rejection leaves timing unchanged. `TestRender.ps1` and `TestVulkan.ps1` require the ordered marker while still requiring DXGI latency or Vulkan acquire/fence waits separately and `inputLatency=unavailable`. The D3D12/Vulkan benchmark scripts require the same ordered lifecycle in every retained schema-4 frame; attachment QPC checks include the sample row, while normal runs may truthfully retain `qpc=0`.
