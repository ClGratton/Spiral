...
The same scripts also run same-process `--smooth-frametime-target-change-smoke` transitions on InterFrame. The below-ceiling case changes 5 to 10 FPS and requires the first completed new-policy frame to retain a nonzero intentional wait and report `effectiveLimiter=RequestedTargetCadence`. The above-ceiling case changes 5 to 1000 FPS and requires `D3D12SynchronizedPresent` with synchronized `Present(1)` or `VulkanFifoPresent` with FIFO; a late frame may correctly contain no intentional pacer wait. Both cases require the published old/new targets, the `next-valid-frame-boundary` contract, the completed frame's new requested target, GPU-completion observation, bounded exit, and process-tree cleanup. These markers prove live policy propagation and engine/presentation diagnosis on the named device, not monitor refresh, actual displayed cadence, or a production pacing choice.
