...
The focused editor navigation test must prove public Window cursor capture/restore, image hover/focus and ImGui-widget gating, the exact plain-LMB/RMB/LMB+RMB/MMB/wheel/RMB-wheel/RMB-fly mappings, timestep-scaled double-precision Scene main-camera movement, stable-ID snapshot canonical-position agreement, and one valid discontinuity epoch after F focuses an existing selected entity origin. A bounded headed Windows D3D12 launch must exercise the real viewport and inspect its post-navigation capture. This is editor-perspective behavior only; it does not prove picking, multi-select, pivot orbit/dolly/track, bounds-aware framing, gizmos, orthographic navigation, camera piloting, or configurable bindings.
