...
`EngineTests --test "RHI read-only texture binding table preserves error and GPU-retired identities"` is the focused deterministic Phase 3D descriptor-table contract check. Its fake exact-device RHI proves the named minimum/maximum capacity bound including the reserved error slot, deterministic exhaustion, foreign and writable (`RenderTarget`) rejection without mutation, invalid sampler rejection at creation/allocation/replacement, retained error/old/replacement lifetime after caller references are dropped, invalid-handle error resolution, capability-selected bounded fallback, queued replacement invisibility before the exact token retires, foreign-token rejection, and `Complete`/`Failed` exact-token retirement before generation-safe slot reuse. Final local acceptance produced a zero-warning/error MSVC Debug build, the focused pass, and 84/84 Integration tests. It creates no native descriptor heap/set, sampler, shader binding, texture upload, material path, or Scene draw, so it is not D3D12/Vulkan/backend qualification.
...
On 2026-09-02, `ViewportNavigationSmokeV7` passed on Linux after adding two regressions that the earlier forced-focus smoke missed. First, an unfocused hovered viewport retains the initiating MMB or wheel event across the GLFW-before-ImGui boundary; the user confirmed the repaired Fusion gesture path in the live Debug Vulkan Editor. Second, selecting the non-camera Prototype Mesh replaces a deliberately stale orbit pivot with that entity's double-precision transform origin without moving the camera. The rebuilt Debug Editor was visually inspected on DP-3/workspace 2 with NVRHI Vulkan active on the NVIDIA GeForce RTX 3080 Ti; the selected-pivot interaction still requires the user's live confirmation. The ordinary MMB pan and Shift+MMB orbit math were not replaced by either repair.
