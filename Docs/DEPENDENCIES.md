...
| KTX-Software / libktx v4.4.2 | `Vendor/KTX-Software` via the opt-in `Scripts/FetchDependencies.*` flag | Future private KTX2 container validation and BasisLZ/UASTC transcoding for `Engine::TextureImporter`. | The complete fetched source preserves upstream's broader per-file inventory in `LICENSE.md`, `LICENSES/`, and `NOTICE.md`, including the non-open Ericsson terms for optional `etcdec.cxx`; the dependency graph therefore records `NOASSERTION` for the untrimmed source checkout. The intended compiled subset is Apache-2.0 (KTX-Software, Basis Universal, DFD utilities), BSD-1-Clause (uthash), and the BSD-3-Clause option for zstd. | Exact source commit `4d6fc70eaf62ad0558e63e8d97eb9766118327a6`; the official annotated `v4.4.2` tag resolves to that commit. The admitted eventual build is a private static read/transcode subset only: KTX2 only, ETC software unpack disabled, no GL/Vulkan upload helpers, ASTC decode, writers/encoders, tools, tests, or KTX1. In particular, the build must exclude `external/etcdec/etcdec.cxx`, which upstream identifies as non-open. It is source-admitted but not linked, staged, or a runtime dependency until the following importer/artifact slice supplies the engine-owned boundary and focused behavior evidence. Normal setup does not fetch it until that optional importer boundary is requested. |
