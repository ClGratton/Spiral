...
| NVRHI | `Vendor/NVRHI` | Renderer abstraction vendor layer. Common, validation, and Windows/MSVC D3D12 backend sources are built by Premake and used through `Engine::RHI`. | MIT, see `Vendor/NVRHI/LICENSE.txt`. | NVRHI types stay out of gameplay/editor APIs. D3D12/Vulkan details remain behind `Engine::RHI`. |
| Vulkan-Headers | `Vendor/Vulkan-Headers` | Platform headers for the future NVRHI Vulkan backend. | Apache-2.0 OR MIT, see `Vendor/Vulkan-Headers/LICENSE.md`. | Pinned from NVRHI's expected tag. Not linked into the build until Vulkan backend enablement. |
