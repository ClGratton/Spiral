...
RenderDoc is likewise an optional diagnostic tool, not an admitted build or runtime dependency. A user-supplied Linux package may remain under ignored `output/tools/renderdoc-*/`; it is never fetched, linked, staged, shipped, registered, or required for ordinary Editor execution. `Scripts/VerifyRenderDocSceneLabels.sh` requires explicit `qrenderdoc`, Editor, repository, and artifact paths; it accepts optional explicit RenderDoc library, official implicit-layer manifest, and runtime-library-directory paths and otherwise derives the ordinary installed/package layout. It preflights the selected dynamic-library closure, copies and validates the official version-45 manifest into the artifact, rewrites only that copy to the selected absolute `librenderdoc.so`, and supplies its directory through the launched child's `VK_IMPLICIT_LAYER_PATH`. It never changes user/system Vulkan configuration. The verifier isolates qrenderdoc data/config/cache with analytics and update checks disabled and records the reported RenderDoc version/commit plus qrenderdoc, library, source-manifest, effective-manifest, and Editor paths/hashes in its schema-versioned receipt. Its embedded-Python path accepts exactly the public RenderDoc 1.45 replay/target-control API used by the checked script, preserves the `.rdc`, and inspects marker actions without opening the UI. The upstream project is MIT-licensed, but a local package's origin, binary closure, and redistribution status remain the operator's responsibility; this diagnostic version gate is not a repository dependency pin or redistribution approval.
