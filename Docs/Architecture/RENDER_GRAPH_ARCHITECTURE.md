...
Physical graph binding also requires a backend-neutral resource/device ownership check before command recording. Resource descriptions prove shape and usage compatibility but cannot prove that a supplied texture or buffer belongs to the executor's device. `Engine::RHI::Device` therefore owns a narrow compatibility query for textures and buffers; adapters answer from their opaque implementation identity without leaking native handles or backend types. Null, foreign-backend, and same-backend cross-device resources fail binding before any pass callback or transition is recorded. Description-only checks and native downcasts above RHI are rejected because they either admit invalid cross-device work or break backend neutrality.
