...
GPU-retired frame-context reuse depends on a backend-neutral completion boundary that did not exist when construction was completed: the original device contract exposed only synchronous `SubmitAndWait`, with no opaque submission token, completion query, or reusable recording lifecycle. Phase 3 therefore requires an explicit RHI prerequisite before the executor. Graphics submission returns a device-owned monotonic completion token; callers may query it without blocking or wait for it, and may reset/reuse a command-list or enclosing frame context only after completion. D3D12 and Vulkan/NVRHI translate this contract to their native completion mechanisms. Tokens are opaque outside `Engine::RHI`; invalid, stale, or cross-device tokens fail explicitly. The existing synchronous helper remains a wrapper over submit-plus-wait so bootstrap consumers do not fork submission policy. CPU frame numbers and synchronous destroy-and-recreate are not acceptable substitutes because neither proves GPU retirement before reuse.
