...
The accepted enabling foundation for the next consumer is not itself a new mailbox action: the public Scene facade now provides a validated atomic complete-transform authority, and Editor history captures, validates, restores, and republishes the complete project color-pipeline value. Failed transform and history-restore requests preserve their prior state. The next concrete consumer remains the ordered closed version-2 extension for entity inspection/selection, complete Transform, typed Light, complete project color-pipeline, and main-camera viewport-pose actions. Each action must have a fixed schema, complete-value compare-and-swap preconditions, domain validation, main-thread commit, action-specific postconditions, rollback-capable history where document state changes, and an explicit session-only/not-saved result. Repository agents use these actions for headed scene verification and never inject mouse, keyboard, or virtual-pointer input to mutate parameters. This extension still provides no arbitrary method dispatch, entity/component lifecycle, save authority, provider/model integration, or general Automation module; later concrete consumers must justify any additional action independently.
...
There is no in-product AI runtime, provider adapter, generic workflow executor, cross-domain action schema, or Automation module. The implemented Editor material-control mailbox is a session-local, explicitly enabled verification tool with two fixed actions. Its accepted Scene-transform and color-history foundation does not expand that schema. The ordered version-2 extension remains pending and, once implemented, remains a fixed Editor-owned repository-verification surface rather than generic Phase 13 infrastructure. Phase 2 project creation and undo/redo are ordinary editor features, not proof of Phase 13 automation.
