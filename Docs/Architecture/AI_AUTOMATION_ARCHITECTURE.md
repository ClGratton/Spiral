...
The first deliberately reordered concrete consumer is narrower than the Phase 13 workflow framework: an implemented opt-in Editor control mailbox used by repository agents to inspect and transactionally patch authored state without mouse/UI injection. Schema 3 retains the two fixed material actions and the prior six fixed scene actions for entity inspection/selection, complete Transform, typed Light, complete project color pipeline, and main-camera viewport pose. It adds only complete Scene-debug view/selected-bounds publication bound to the expected selected entity and complete MeshRenderer visibility/shadow-flag mutation. It proves main-thread execution, exact session/project/entity identity and field masks, complete-value preconditions, validation, one history transaction per successful document mutation, rollback, renderer republication where applicable, and bounded terminal receipts with at most 32 sampled affected stable IDs. It remains session-local and does not save assets, accept arbitrary commands or request-chosen paths, introduce a model/provider dependency, or justify generalization before another real consumer proves the shared boundary. This user-directed prerequisite makes actual headed renderer verification use the same deterministic command authority intended for later in-product agents; it is not evidence that the Phase 13 project-template workflow or product AI exists.

The accepted enabling foundation and its first consumer now form one closed authority: the public Scene facade provides validated atomic complete-transform and complete-light mutation, Editor history captures and restores the complete project color-pipeline and MeshRenderer flag values, and schema-3 mailbox actions expose only those named operations plus selection, inspection, a dedicated main-camera pose path, and the bounded debug-visualization publication. Each action has a fixed schema, complete-value compare-and-swap preconditions, domain validation, main-thread commit, action-specific postconditions, rollback-capable history where document state changes, and an explicit session-only/not-saved result. The debug action also binds the expected selected entity and verifies the resulting Renderer publication generation; generic transform requests cannot mutate the main camera. Repository agents use these actions for headed scene verification and never inject mouse, keyboard, or virtual-pointer input to mutate parameters. This extension provides no arbitrary method dispatch, entity/component lifecycle, save authority, provider/model integration, or general Automation module; later concrete consumers must justify any additional action independently.
...
There is no in-product AI runtime, provider adapter, generic workflow executor, cross-domain action schema, or Automation module. The implemented schema-3 Editor control mailbox is a session-local, explicitly enabled repository-verification tool with two fixed material actions and eight fixed scene/debug actions. It uses the accepted Scene transform/light/MeshRenderer, Editor color/history/selection, and Renderer debug-publication authorities, cannot save, and does not generalize beyond those named operations. Phase 2 project creation and undo/redo are ordinary editor features, not proof of Phase 13 automation.
