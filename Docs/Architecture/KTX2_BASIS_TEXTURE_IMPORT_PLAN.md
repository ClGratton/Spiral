...
`TextureImporter::CookNormalizedRgba8` remains the decoder-to-importer fallback boundary. Its explicit mip policy defaults to `PreserveAuthored`; `CompleteMissing` preserves all supplied RGBA8 levels and generates only the absent suffix before cooking, with fixed-point linear-light color filtering, vector-normal filtering, and component-linear data filtering through deterministic odd-extent bounds. `TextureImporter::CookKtx2Basis` is the private libktx-backed KTX2/Basis boundary: it accepts only caller-owned bytes and explicit role/color-space settings, validates 2D LDR single-face BasisLZ/ETC1S or UASTC containers and their transfer/primaries metadata, then copies every transcoded mip into a schema-2 Spiral artifact before destroying the libktx object. Its default also preserves the authored chain; requesting completion of an incomplete compressed source fails visibly because the admitted read/transcode-only subset has no encoder and may not reinterpret canonical compressed content. No ktx type crosses a public Engine API. Schema 2 adds `HasAlpha` and block-aware mip validation; it writes only engine-owned `DesktopBC`, `Astc`, or qualified `RGBAFallback` pages. Schema-1 RGBA fallback artifacts remain readable and conservatively report `HasAlpha=true`, because that schema did not retain the source alpha state. A rejected source does not register a new asset, replace a cooked file, or mutate the caller artifact.

Each target uses a deterministic target-qualified derived path (`desktop-bc`, `astc`, or `rgba-fallback`) under the same stable source asset, so cooking one target cannot replace another. Resolver callers that know the device preference request that target explicitly and receive its optional separately cooked RGBA fallback as one transactional variant set. The preferred and fallback must match asset/source provenance, role, color space, alpha state, base extent, and mip count. Missing fallback is a successful set with no fallback value; a present corrupt or mismatched variant rejects without replacing caller output. The old unsuffixed schema path remains a read-only migration source and can satisfy only the target profile stored in that artifact.
...
Normal `Scripts/Setup.*` fetches the exact ignored source checkout and verifies its commit/notices before generation, but no libktx binary is staged at runtime. The shell path does not require Git LFS for upstream assets excluded from the private build: a fresh checkout retains their pointers, while a reused materialized checkout is admitted only after every apparent LFS change matches the committed payload SHA-256 and byte count and no staged, untracked, mode, deleted, or non-LFS change exists. The private static subset compiles only KTX2 read/container, DFD, Basis transcoding, and bundled zstd sources. Its explicit compile-time exclusions are KTX1, GL/Vulkan upload helpers, writers/encoders, tools, upstream tests, `external/etcdec/etcdec.cxx`, software ETC unpack, and ASTC decode. KTX-Software v4.4.2's common texture object retains one link reference to its KTX1 constructor even when `texture1.c` is excluded; the pin-specific private `Ktx2OnlyLinkShim.cpp` satisfies only that symbol by returning `KTX_UNSUPPORTED_TEXTURE_TYPE`, rather than compiling KTX1 support. Basis output targets are deterministic: BaseColor/Emissive select BC7 sRGB or ASTC 4x4 sRGB; Normal selects BC5 UNORM or ASTC 4x4 UNORM; ORM/Mask select BC7 UNORM or ASTC 4x4 UNORM; `RGBAFallback` remains RGBA8 matching the explicit color space. The first implementation deliberately does not infer role or color space from a path, extension, or container name.

The Linux ASan/UBSan oracle instruments project-owned Engine/test/fuzz code but not pinned vendor code. Because the KTX subset is compiled into the Engine static library instead of a separate project, its Premake source filter explicitly appends `-fno-sanitize=address,undefined` after Engine's sanitizer flags. This keeps importer calls and every project-owned validation/copy boundary under the oracle without treating an upstream C function-pointer signature mismatch as an engine defect or claiming vendor memory-safety qualification.

Pinned libktx v4.4.2's memory constructor allocates its private read-only stream before reading the fixed 80-byte KTX2 header, but its pre-header read-failure path returns a null texture without destroying that allocation. `TextureImporter` therefore rejects inputs shorter than `KTX2_HEADER_SIZE` before entering libktx. Once the complete header is readable, ownership transfers into libktx's texture construction and its later success/failure cleanup paths remain authoritative. The existing malformed-input transactional test is also the leak regression under the Linux ASan lane; Spiral does not patch vendored source or suppress leak detection.
