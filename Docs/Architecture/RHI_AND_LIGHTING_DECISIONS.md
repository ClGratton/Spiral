...
Renderer now packs one immutable Scene-light payload as explicit `uint32x4` words rather than a compiler-dependent C++ overlay. A versioned header carries checked section offsets/counts for complete light records, the compact directional index list, complete cluster CSR offsets, and local-light indices. Packing validates one selected immutable snapshot view and its matching CPU grid as one identity-preserving publication, including copied and derived type/unit/value/transform data, bounded products and byte size, monotonic CSR offsets, exact final offset, index bounds, directional-only globals, and local-only cluster membership. Failure builds no partial replacement. The exact double-precision photometric value remains recoverable from raw IEEE-754 words; the float consumption value is separately checked and is explicitly not the persistence authority.

GPU publication uses the existing submitted-frame capacity as its bound. A slot pairs a CPU-write `CopySource` staging buffer with a GPU-only `Structured|CopyDest` buffer and is writable only when no accepted frame retains it. Publication maps/copies/unmaps the staging member, while the same frame graph records the copy, transition, and later raster read without a per-frame synchronous `Device::UploadBuffer` wait. Allocation, mapping, graph-recording, copy, or capacity failure preserves the prior accepted generation/output. A successful publication returns an immutable shared payload retaining both buffers and is retained with the graph until every exact accepted token for that frame is terminal. Production `PSMain` continues using the neutral preview light in this prerequisite. The separate bounded Vulkan diagnostic entry loads discriminating words from every payload section and matches native RGBA16F readback against an independent CPU oracle with `productionPSMain=preserved lightingEvaluation=no`; D3D12/MoltenVK execution and light evaluation remain separate.
