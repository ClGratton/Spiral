...
The portable graphics-pipeline contract currently admits one interleaved per-vertex stream at input slot zero. `PipelineDescription::VertexStrideBytes` is the exact stride of that stream: it must be zero only when the vertex-input list is empty, and every nonempty attribute must use an admitted portable format, avoid offset-plus-width overflow, and end within the declared stride. The current cross-backend format set is `R32G32Float` and `R32G32B32Float`; multi-slot and instanced layouts remain unsupported until a real consumer and capability contract require them. Vulkan forwards the explicit stride to every NVRHI attribute and does not constrain the attribute count; D3D12 keeps its native input layout and buffer-view stride, but an exact-stride mismatch at slot-zero binding invalidates the binding and prevents the draw. This bootstrap interleaved layout does not replace the longer-term separate-stream geometry direction in [LOW_LEVEL_ARCHITECTURE.md](LOW_LEVEL_ARCHITECTURE.md).

Vulkan completion tokens are device-local monotonic submission identities. The adapter retains native queue/submission/timestamp state only for the live suffix after one scalar compacted terminal prefix. Before issuing later work, and after an explicit device-idle wait, it observes the next contiguous prefix entry in order; normal compaction is nonblocking, and an incomplete or invalid entry stops it even when later queues have completed out of order. Timestamp queries are completed or failed and retired before terminal publication. The first terminal publication performs the existing one-shot NVRHI garbage collection, after which successful entries are implicit in the prefix and failed IDs remain explicit. Consequently every issued same-device token remains queryable/waitable as exactly `Complete` or `Failed` and remains valid as a dependency for the device lifetime, while compacted terminal dependencies require no native queue handle or GPU wait. Future/unissued and foreign-device tokens remain invalid. `VulkanCompletionHistoryV1` reports issued, compacted, live, failed, and incomplete CPU-authority counts at sparse power-of-two compaction milestones; it is history-boundedness evidence, not reclaimed-byte or driver-memory attribution.
...
The current glTF importer validates supported float3-position triangle primitives and writes a schema-2 `.spiralmesh` artifact with source/handle provenance, `PositionNormalColorUV32F` vertices, `UInt32` indices, and source primitive byte ranges. Schema 1 remains a read-only migration input: its old position/color/UV payload receives deterministically derived geometric normals before validation and is never republished as schema 1. The D3D12 and Vulkan viewport renderers resolve `SceneRenderSnapshot` mesh handles and upload the validated artifact through the shared resource cache.
...
The normal default Editor scene cannot remain a private renderer cube once viewport integration begins. New scenes, loaded projects, and the bounded Vulkan Scene viewport fixture therefore reuse the original engine-owned `Engine/Generated/PrototypeCube.mesh` source identity and one shared registration/store/rollback helper before publishing the copied resolver. The helper invokes the schema-2 constructor, validation, and atomic publication path; its artifact retains explicit normalized face normals in the supported `PositionNormalColorUV32F`/`UInt32` cube payload and exact primitive byte ranges. This is not a second mesh format, renderer fallback, or implicit source-freshness/hot-reload mechanism. On first publication failure it removes only the asset it just registered; replacement failure leaves the previously accepted artifact in place. D3D12 origin raster setup uses the normal Editor registry publication rather than a synthetic unresolvable handle.

### Phase 3E Surface Basis And Frame-Local Material Identity

Assets owns the object-space geometric-normal persistence boundary. A schema-2 artifact accepts only finite unit-length normals within the artifact contract's admitted tolerance, and the public completion helper validates already-authored normals instead of silently regenerating or renormalizing invalid values. A supported glTF triangle primitive either supplies a complete `NORMAL` accessor (`VEC3`, count equal to positions, core `FLOAT`, or `KHR_mesh_quantization` normalized signed `BYTE`/`SHORT`) or supplies none. Quantized normals are admitted only when `KHR_mesh_quantization` appears in both `extensionsUsed` and `extensionsRequired`; unsigned, unnormalized, or optional/undeclared integer normals reject transactionally. Valid imported values are normalized and preserved by direction. Missing normals are derived deterministically from indexed winding with area-weighted accumulation. A degenerate triangle, an unreferenced or cancelling vertex, mixed zero/nonzero artifact normals, malformed accessor/count/type, nonfinite/zero authored value, or non-unit authored artifact value rejects the complete import or migration before registration/artifact replacement; no partial primitive publication is allowed. Tangents, normal-map basis generation, smoothing policy, and non-triangle fallback remain outside this prerequisite.

Renderer acquires one opaque immutable resolver snapshot while preparing one raster frame and retains that exact snapshot in the frame. Row zero is the explicit default/error material. All distinct valid material asset handles referenced by visible mesh instances are sorted by numeric stable handle and copied into rows `1..N`; duplicates reuse the same row. Invalid or missing handles, registered wrong-type handles, and registered Material handles absent from the retained material library map to row zero, whose constants publish `MaterialState = {0, 1, 0, 0}`. Later material-texture and mesh-artifact resolution for that frame must use the retained snapshot, never a fresh read of the globally published catalog. Thus a live Inspector edit, import/reimport, or file-watch event may publish the next catalog after CPU preparation without invalidating or blanking the in-flight frame; the next prepared frame adopts the new generation. These IDs are deterministic only for the prepared frame/catalog generation and must never be persisted or confused with asset handles, Scene entity IDs, or future visibility-buffer IDs.

The shared Scene vertex contract consumes the cooked object-space normal and a row-vector inverse-transpose of the model linear transform, represented by the project convention `S^-1 * R`. This preserves direction under nonuniform scale; a nonfinite or singular transform on any mesh rejects the complete prepared frame rather than publishing partial or corrupt constants. The production D3D12/Vulkan pipeline descriptions use the explicit `sizeof(MeshArtifactVertex)` stride for the schema-2 position/normal/color/UV layout. One shared 384-byte per-instance constant layout carries model-view-projection, normal transform, texture/material state, and appended model-view plus normal-view matrices to both backends; native allocations are 512-byte aligned. Each accepted graph frame retains its exact mesh bundle, material texture resources, and constant allocation until the final accepted graph token is terminal. The Vulkan-only `--scene-surface-basis-material-id-smoke` runs a separate bounded pipeline invocation whose diagnostic pixel entry shares the exact production vertex inputs and constants; normal comprehensive and Editor rendering continue to compile and use `PSMain`. Its independent readback oracle uses an authored oblique `(1,2,3)` normal, nonuniform `(1.3,0.7,2)` scale, and nonzero Z rotation so incorrect inverse/transpose/order fails. This does not establish tangent-space normal mapping, Scene-light evaluation, photometric attenuation, shadows, visibility-buffer identity, D3D12 runtime execution, or MoltenVK execution.

### Phase 3E Fixed Structured SRV And Light-Payload Boundary

The shader-visible light payload was split at the missing RHI authority instead of being implemented as one monolith. The accepted prerequisite admits one optional pixel-stage read-only `StructuredBuffer<uint4>` at `t0,space3`. Reflection, pipeline description, and command binding must agree exactly on that register/space, one resource, the `uint4` element shape, and 16-byte stride. Binding accepts only an exact-device, GPU-only, nonempty buffer with exactly `Structured` plus optional copy usage whose size is divisible by the stride. Every bind attempt and pipeline change clears the active binding, it is required before a draw only when the active pipeline declares it, and native objects used by earlier draws remain retained for the whole recording. D3D12 uses a root SRV without replacing the sampled-table descriptor heaps; Vulkan uses a set-3 NVRHI structured-buffer SRV. The fixed path does not generalize register selection, resource arrays, writable or host-visible binding, descriptor indexing, or arbitrary structured layouts.

Renderer now packs one immutable Scene-light payload as explicit `uint32x4` words rather than a compiler-dependent C++ overlay. The V3 header carries checked section offsets/counts for seven-word light records, the compact directional index list, complete cluster CSR offsets, and local-light indices. Packing validates one selected immutable snapshot view and its matching CPU grid as one identity-preserving publication, including copied and derived type/unit/value/transform data, bounded products and byte size, monotonic CSR offsets, exact final offset, index bounds, directional-only globals, and local-only cluster membership. Failure builds no partial replacement. The exact double-precision photometric value remains recoverable from raw IEEE-754 words; the checked float value and prepared RGB coefficient, reciprocal spot span, and reciprocal range are shader-consumption data rather than persistence authority.

GPU publication uses the existing submitted-frame capacity as its bound. A slot pairs a CPU-write `CopySource` staging buffer with a GPU-only `Structured|CopyDest` buffer and is writable only when no accepted frame retains it. Publication maps/copies/unmaps the staging member, while the same frame graph records the copy, transition, and later raster read without a per-frame synchronous `Device::UploadBuffer` wait. Allocation, mapping, graph-recording, copy, or capacity failure preserves the prior accepted generation/output. A successful publication returns an immutable shared payload retaining both buffers and is retained with the graph until every exact accepted token for that frame is terminal. Production `PSMain` now validates and consumes V3 global/CSR sections for photometric directional, point, and spot evaluation. The separate bounded Vulkan payload diagnostic still proves exact section bytes without conflating that with the independent production-lighting oracle. D3D12 and MoltenVK execution remain separate runtime qualifications.
...
### Exact-Device Immutable Texture Cache

`TextureGpuResourceCache` is the narrow T4B construction and ownership boundary. It receives one validated preferred `TextureArtifact`, an optional separately cooked semantic RGBA fallback, and one exact live `Engine::RHI::Device`. Existing plan selection chooses only a format supported for sampled and copy-destination use by that device. The cache creates the selected full-mip texture, uses the synchronous `Device::UploadTexture` authority, verifies the resulting whole-resource state is `ShaderResource`, and only then publishes a Ref-owned bundle retaining the exact upload plan, payload, resource, selection diagnostic, and monotonic generation.

The key is full value equality over the exact device address, lexically normalized source provenance, asset and texture semantics, complete RHI description, every mip range, and every payload byte; no hash collision can alias a generation. A hit returns the same bundle. Changed content creates a new generation. Capacity uses deterministic least-recent eviction with the generation as tie-break, but eviction and `Clear` drop only cache ownership, so accepted frames can retain an external bundle. Selection, creation, upload, or final-state failure does not mutate accepted entries or the caller's prior output. Device addresses are only live-instance identities: callers must clear the cache and release all external bundles before device destruction or address reuse. This boundary allocates no sampled-table slot, infers no filename semantics, changes no material, and provides no streaming or residency policy; T4C owns stable-handle table publication and exact GPU-retired replacement/removal.

### Stable Texture-Table Publication And Retirement

`TextureTablePublication` is the sole T4C mutation coordinator over one owned `RHI::TextureBindingTable`. A stable `(texture asset, sampler)` view can allocate a real handle only from a nonzero-generation T4B bundle whose asset, immutable upload plan, exact-device texture description/ownership, and published `ShaderResource` state agree. Multiple sampler views may retain the same immutable GPU resource, but each has a distinct generation-safe handle. Any invalid, foreign, incomplete, mismatched, duplicate-changing, or capacity-exhausted request returns the table's retained error handle without altering accepted views or slots. Sampler meaning is supplied explicitly by the caller; this layer never derives it from a filename or role.

After a submission is accepted, the caller records the exact completion token and every bound handle in submission-acceptance order. The coordinator retains those bundles within the same four-frame bound as the current submitted-frame owner and records the latest accepted use per asset/sampler view. Queueing replacement or removal freezes only that stable view to the error handle for new recordings, while `TextureBindingTable` deliberately keeps its old published slot visible to already accepted native work. The view owns its sampler for its lifetime; content replacement cannot mutate it into another view. The pending record retains the old bundle, replacement bundle when present, operation kind, and exact latest-use token. Earlier, foreign, untracked, and incomplete tokens cannot publish the operation. Exact `Complete` or `Failed` retirement releases the matching accepted-frame references and either exposes the replacement through the same handle or removes the slot and advances its generation before reuse. Sibling views retire independently, including when their operations share one terminal token. Calls must be serialized in actual submission-acceptance order. Device-idle shutdown releases frames, operations, views, and the owned table before device destruction. Native binding remains the already accepted bounded-table implementation; T4 parent integration owns catalog resolution, cache acquisition, and real Scene workflow use.

`TextureRuntimePublication` is the accepted T4 composition authority. Its convenience entry point acquires the atomically published resolver snapshot and monotonic generation together, while Scene rendering supplies the exact snapshot already retained by raster preparation. Material lookup, selected target plus optional matching fallback, texture cache acquisition, and T4C mapping all remain within that one generation; a resolution sequence must never reload global publication state between those steps. Its logical key is `(texture asset, sampler)`: compatible views share the cache bundle/resource but not the table handle or lifetime state. Each accepted frame supplies the exact handles actually recorded and its exact submission token; error and duplicate handles add no lifetime reference. A later catalog generation queues one replacement/removal for each resolved view against that view's latest accepted use and returns the declared error handle to new recordings while the operation is pending. Scalar material-only edits whose texture bundles are unchanged advance the entry's catalog generation while preserving the same handles. If the latest accepted use was already retired normally, the coordinator remembers its terminal state and commits a newly discovered content operation immediately against the same token. If the handle never appeared in accepted GPU work, the publication layer may synchronously replace or remove it through the narrowly named unsubmitted table operations; it must not fabricate a token or retain a dead slot until shutdown. Resolution, artifact validation, target selection, upload, table allocation, stale-handle, and pending-operation failure all preserve the error-resource rule.

The D3D12 and Vulkan Scene viewport renderers create the coordinator only after their device and pipeline are valid and release it only after device idle, before device destruction. The T4 native smoke composes catalog storage, preferred upload, table publication, simultaneous `LinearWrap` and `PointClamp` views over one resource, accepted submissions, exact-token replacement/removal of both views, failure-to-error behavior, and idle release on the selected backend. This lifecycle integration deliberately does not bind a Scene material yet: T5 owns material texture/sampler semantics, render-snapshot consumption, and the exact point where accepted draw handles are handed back to this coordinator.

T5A extends the same immutable publication state with a copied `MaterialLibrary`; registry, mesh/texture resolvers, and material values now advance under one monotonic generation. Material lookup first requires that generation's registry to classify the stable handle as `Material`, then requires a value in that generation's library, and copies the result only after both checks succeed. Editor startup/default creation, project load, history restoration, imports that mutate the registry, explicit material save/smoke, and live Inspector edits all publish the registry and material library together. Mutable authoring changes after publication cannot alter an acquired snapshot. This prerequisite does not yet declare sampler state, validate texture roles against material slots, resolve a texture, bind the sampled table, or change shader output.

T5S gives the authoring asset an explicit backend-neutral sampling declaration before T5 consumes it. Every BaseColor, Normal, ORM, Emissive, Opacity, and CallistoControl slot independently selects `LinearWrap`, `LinearClamp`, `PointWrap`, or `PointClamp`; schema 2 requires exactly one declaration per slot and rejects missing, duplicate, unknown-slot, or unknown-mode data transactionally. Schema 1 has no sampler records and therefore migrates all six slots to `LinearWrap`, matching the native table's former effective default. The Material Inspector edits and republishes those declarations through the accepted T5A catalog boundary. Assets owns only persisted sampling intent: T5 remains responsible for mapping it to the existing RHI sampler enum, validating texture role/color-space compatibility, resolving table handles, and proving shader-visible Scene consumption. Anisotropy and broader sampler LOD policy remain Phase 4 work.

T5B accepts the first textured Scene draw on native Linux Vulkan without overstating the full T5 gate. The default prototype material references a deterministic seven-level sRGB base-color artifact produced through the ordinary normalized RGBA cooking path. Scene rendering resolves material and texture content from one immutable catalog generation, validates slot semantics, maps the declared sampler, binds the shared sampled table, and retains the exact texture bundle through the raster completion token. The shared shader has semantic defaults for all six absent slots and samples the declared base/normal/ORM/emissive/opacity/control handles when present; its normal-texture contribution remains a bounded detail modulation rather than tangent-space lighting because tangents and a tangent-space basis are not yet admitted. Vulkan's unchanged table realization and mapped constant-buffer frame ring are cached under the existing GPU-retirement bounds. The table capacity is selected once from the exact adapter's sampled-image/sampler limits and then shared by shader compilation, reflection, logical publication, native realization, and cache construction; the local RTX device retains 4096 while smaller portability devices use their truthful limit. D3D12 contains the mirrored consumer but remains unqualified in this Linux slice, and full T5 still requires exact native readback for auxiliary semantics and mip choice.

T5C makes that remaining shader evidence exact without creating a second material implementation. `EditorViewport.hlsl` retains its production entry and adds a narrowly test-only probe entry compiled with a bounded table capacity; both use the same constants, texture indices, declaration/error masks, register spaces, and sampler table. Three immutable material sets rendered in one real RHI submission expose eight exact cells: BaseColor mip 0 and mip 1, Normal, ORM, Emissive, Opacity, CallistoControl, and error state. Deliberate byte patterns distinguish sRGB decode from linear sampling, declared point/linear and clamp/wrap behavior, semantic defaults, and error-resource routing. The runtime retains the resolved handles under the accepted submission and retires only its exact completion token. This oracle is implemented once through the backend-neutral RHI and passes on Linux Vulkan; D3D12 qualification remains the Windows harness gate.
...
- Dynamic exposure pipeline. The current executable prerequisite is the renderer-owned project exposure control: either manual `EV100` or calibrated camera-style aperture/shutter/ISO resolving as `EV100 = log2((N * N) / t * (100 / ISO))`, then `scale=exp2(-effectiveEV100)`. It remains project-owned rather than Scene camera-owned until a per-camera lighting-validation consumer requires that authority; automatic metering/zones remain future work.
...
- Filament direct and punctual lighting: https://google.github.io/filament/main/filament.html#lighting/directlighting
