...
`TextureTablePublication` is the sole T4C mutation coordinator over one owned `RHI::TextureBindingTable`. A stable asset can allocate a real handle only from a nonzero-generation T4B bundle whose asset, immutable upload plan, exact-device texture description/ownership, and published `ShaderResource` state agree. Any invalid, foreign, incomplete, mismatched, duplicate-changing, or capacity-exhausted request returns the table's retained error handle without altering accepted assets or slots. Sampler meaning is supplied explicitly by the caller; this layer never derives it from a filename or role.

After a submission is accepted, the caller records the exact completion token and every bound handle in submission-acceptance order. The coordinator retains those bundles within the same four-frame bound as the current submitted-frame owner and records the latest accepted use per asset. Queueing replacement or removal freezes that stable asset to the error handle for new recordings, while `TextureBindingTable` deliberately keeps the old published slot visible to already accepted native work. The pending record retains the old bundle, replacement bundle when present, sampler, operation kind, and exact latest-use token. Earlier, foreign, untracked, and incomplete tokens cannot publish the operation. Exact `Complete` or `Failed` retirement releases the matching accepted-frame references and either exposes the replacement through the same handle or removes the slot and advances its generation before reuse. Calls must be serialized in actual submission-acceptance order. Device-idle shutdown releases frames, operations, assets, and the owned table before device destruction. Native binding remains the already accepted bounded-table implementation; T4 parent integration owns catalog resolution, cache acquisition, and real Scene workflow use.

`TextureRuntimePublication` is the accepted T4 composition authority. It reads the atomically published resolver state and its monotonic catalog generation together, resolves the selected target plus optional matching fallback, acquires a complete T4B bundle on the exact live device, and is the only caller that maps that result through T4C. Each accepted frame supplies the exact handles actually recorded and its exact submission token; error and duplicate handles add no lifetime reference. A later catalog generation queues one replacement/removal against the latest accepted use and returns the declared error handle to new recordings while the operation is pending. If that use was already retired normally, the coordinator remembers its terminal state and commits the newly discovered operation immediately against the same token. If the handle never appeared in accepted GPU work, the publication layer may synchronously replace or remove it through the narrowly named unsubmitted table operations; it must not fabricate a token or retain a dead slot until shutdown. Resolution, artifact validation, target selection, upload, table allocation, stale-handle, and pending-operation failure all preserve the error-resource rule.

The D3D12 and Vulkan Scene viewport renderers create the coordinator only after their device and pipeline are valid and release it only after device idle, before device destruction. The T4 native smoke composes catalog storage, preferred upload, table publication, accepted submissions, exact-token replacement/removal, failure-to-error behavior, and idle release on the selected backend. This lifecycle integration deliberately does not bind a Scene material yet: T5 owns material texture/sampler semantics, render-snapshot consumption, and the exact point where accepted draw handles are handed back to this coordinator.
