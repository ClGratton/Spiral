...
Before the prototype draw is replaced, the renderer must consume that versioned, validated cooked mesh payload with supported primitive topology, explicit vertex/index layout and byte ranges, and asset-handle/source-path provenance. Renderer resolves snapshot handles through the Engine-owned resolver and creates/uses buffers only through `Engine::RHI`. The payload contract rejects malformed, unsupported, mismatched, wrong-type, and missing artifacts deterministically and publishes replacements atomically. Accepted RenderGraph frames retain exact mesh resources and per-draw constants through GPU retirement; a failed replacement retains the last valid GPU generation. It must not expose cgltf source objects or native backend resource types to Scene, and it does not define source-dependency freshness, material/texture descriptors, GPU residency/streaming, or a generic asset hot-reload framework. No structured buffer is admitted until an actual current consumer needs it.

The shared `MeshGpuResourceCache` prerequisite is the narrow resource construction boundary before viewport integration. It accepts one already validated `MeshArtifact` and an exact live `Engine::RHI::Device`, creates immutable `Vertex|CopyDest` and `Index|CopyDest` buffers with `BufferCpuAccess::None`, and uploads through `Device::UploadBuffer`. Its key is the exact live device instance plus a collision-free value identity over the asset handle, lexically normalized source provenance, primitive byte ranges, vertex values, and index values. It safely converts byte ranges to portable per-primitive element ranges; because cooked indices already address the combined vertex buffer, every portable draw range keeps `BaseVertex = 0`. The cache mutates only after both buffers upload. It has a deterministic LRU/generation tie-break; eviction drops cache ownership but does not invalidate an external `Ref` bundle. Because `Device` currently exposes no lifetime-stable cache token, this cache is explicitly cleared before its caller destroys/replaces a device; neither the cache nor an external bundle may outlive that device or cross an address-reused device lifetime. Snapshot resolution, per-draw constants, RenderGraph-frame retention, source freshness, residency, hot reload, descriptors/materials/textures, and viewport binding remain later consumers.
