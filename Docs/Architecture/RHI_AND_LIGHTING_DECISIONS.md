...
Before the prototype draw is replaced, Assets must define a versioned, validated cooked mesh payload with supported primitive topology, explicit vertex/index layout and byte ranges, and asset-handle/source-path provenance. `Scene` continues to publish only stable asset handles and immutable transforms; Renderer resolves those handles through an Engine-owned asset boundary and creates/uses buffers only through `Engine::RHI`. The payload contract must reject malformed, unsupported, mismatched, wrong-type, and missing artifacts deterministically and publish replacements atomically. It must not expose cgltf source objects or native backend resource types to Scene, and it does not define source-dependency freshness, material/texture descriptors, GPU residency/streaming, or a generic asset hot-reload framework.
