...
The next prerequisite fixes the portable shader-facing declaration without realizing descriptors: a graphics pipeline may declare exactly one sampled texture table at `t0,space1` with its matching sampler table at `s0,space1`; Vulkan reserves texture binding offset 1 and sampler binding offset 2, while the current per-draw constant buffer remains `b0,space0`/offset 0. A declaration colliding with any declared constant-buffer space, with offset zero, or with equal texture/sampler offsets is invalid. `CommandList::BindGraphicsSampledTextureTable` must reject before recording unless an active pipeline carries that valid declaration and the table belongs to the exact device; setting another pipeline clears the binding. Backends without native descriptor realization reject such pipeline creation and binding explicitly. This contract preserves the logical table's already-published view during pending exact-token update/removal, but creates no heap/set, sampler, table write, shader compilation, or shader-visible native consumption. Native D3D12/Vulkan realization may only enable/report `DescriptorIndexing` after the declared layout, descriptor lifetime, and real shader consumption are exercised together.
...
Before native realization, the fixed sampled-table declaration carries one capacity in `[2, 4096]`, including error slot zero. The portable compiler's existing reflection must report one `t0,space1` texture array and one `s0,space1` sampler array, each with exactly that compile-time count; pipeline creation rejects zero/out-of-range capacity and missing or mismatched arrays. `BindGraphicsSampledTextureTable` additionally rejects an exact-device table whose declared capacity differs. This is the capacity handshake required for a future bounded descriptor array; it adds no source parser, descriptor allocation, native sampler, shader-visible backend consumption, or `DescriptorIndexing` enablement. Current D3D12/Vulkan adapters remain fail-closed for any sampled-table pipeline.
