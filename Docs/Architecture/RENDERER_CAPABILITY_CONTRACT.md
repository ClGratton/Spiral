...
Delivery is split at real authority boundaries. P1 establishes the backend-neutral query-pool lifecycle and deterministic failure/reuse semantics without claiming native execution. P2A adds the shared recording/submission transaction and native-allocation retirement seam: logical validation precedes native mutation, accepted submission owns exact-token publication, and backend-private shared state outlives public-wrapper destruction until GPU retirement. P2B implements and qualifies D3D12 query heaps/readback/frequency; P2C independently implements and qualifies Vulkan query pools/valid bits/period. NVRHI's paired timer-query API is not substituted for the indexed P1 reset/write/resolve contract; any required raw query calls stay inside these documented RHI adapter escape hatches and retain NVRHI device/command ownership. P3 associates asynchronous results with frame/RenderGraph-pass identity and publishes Profiler durations plus target-budget headroom. A checked foundation never makes a later stage implicitly complete.
