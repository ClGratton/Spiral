...
The first consumer group is `Phase3FrameTimingV1`. Device timestamp usability and renderer timing-consumer readiness are separate gates: P2B makes the D3D12 RHI feature usable, but P3 has not yet associated native queries with whole-frame/RenderGraph-pass identity. The group therefore continues to select the portable CPU steady-clock timing already integrated into the frame and pass workflow on both backends, retaining an explicit “native RHI usable, renderer consumer pending” reason on D3D12. Headed presentation smokes mark that selected path exercised and qualify the group at Presentation independently from the device's Bootstrap level. P3 alone may enable the GPU consumer gate after real asynchronous frame/pass publication exists. This does not complete Vulkan timestamps, GPU timing, Scene qualification, later consumer groups, dedicated Vulkan queue enablement, physical-device breadth, or Production qualification.
...
Delivery is split at real authority boundaries. P1 establishes the backend-neutral query-pool lifecycle and deterministic failure/reuse semantics without claiming native execution. P2A1 adds logical-validation-before-callback, exact-token publication, and bounded post-submit native-state retirement. P2A2 retains the logical pool through public-wrapper destruction before submit and adds a non-mutating publication preflight before any native submission, so an accepted native submit cannot discover an incomplete/stale query transaction only afterward. P2B is now implemented and locally qualified on D3D12: a private wrapper owns `ID3D12QueryHeap`, readback storage, and the graphics-queue frequency; command-list operations use P2A; fence-terminal nonblocking queries map results and retire the exact token; a portable nanoseconds-per-tick accessor preserves the scale P3 will consume. P2C independently implements and qualifies Vulkan query pools/valid bits/period. NVRHI's paired timer-query API is not substituted for the indexed P1 reset/write/resolve contract; required raw D3D12 query calls stay inside this RHI adapter escape hatch and retain the NVRHI-created device/queue/command ownership. P3 associates asynchronous results with frame/RenderGraph-pass identity and publishes Profiler durations plus target-budget headroom. A checked foundation never makes a later stage implicitly complete.
