...
The first consumer group is `Phase3FrameTimingV1`. Device timestamp usability and renderer timing-consumer readiness are separate gates: P2B/P2C make both native RHI features usable, but P3 has not yet associated queries with whole-frame/RenderGraph-pass identity. The group therefore continues to select portable CPU steady-clock timing on both backends, retaining an explicit “native RHI usable, renderer consumer pending” reason. Headed presentation smokes mark that selected path exercised and qualify the group at Presentation independently from the device's Bootstrap level. P3 alone may enable the GPU consumer gate after real asynchronous frame/pass publication exists. This does not complete GPU timing, Scene qualification, later consumer groups, physical-device breadth, or Production qualification.
