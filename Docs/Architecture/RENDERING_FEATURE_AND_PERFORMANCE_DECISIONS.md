...
The implemented capture publishes immutable renderer snapshots and schema-4 CSV/JSON. Percentiles use sorted nearest-rank samples; the 1% and 0.1% low FPS summaries are `1000 / p99` and `1000 / p99.9` start-to-start frametime. Requested and effective policy targets are separate manifest fields. Schema 2 added run ID, process ID, canonical executable path, QPC frequency, and lifecycle QPC ticks. Schema 3 added explicit GPU timing status/duration/headroom: a ready P3B duration may amend only its exact retained frame, and headroom is `1000 / that frame's effective Smooth target FPS - that frame's GPU interval`. Schema 4 adds `cadencePreviousFrame`, `limitingSource`, and `limitingSourceFrame`, including exact late-GPU amendment of a retained dependent cadence row. Responsive, missing/nonpositive target, stale/evicted/mismatched, non-ready, and disjoint cases stay unavailable. Presentation/sync/VRR/tearing strings remain runner inputs defaulting to `unknown`; display/replacement/input stay unavailable and are not inferred from GPU timing. PresentMon join/supervision remains backward compatible with schema 2 and 3 and accepts schema 4.
