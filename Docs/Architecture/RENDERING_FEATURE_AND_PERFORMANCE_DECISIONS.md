...
Effective-limiter diagnostics are source-qualified heuristics over an actual completed engine interval, not display feedback or a causal proof. With a valid Smooth target and consecutive `FrameStart` interval, a ready exact-frame GPU interval that fills the observed cadence reports `GpuWork`; CPU active work that fills it reports `CpuActiveWork`; otherwise a successful D3D12 synchronized present or Vulkan FIFO present whose engine cadence materially exceeds the requested period reports the corresponding presentation source. An interval within `max(0.5 ms, 10% of the requested period)` reports `RequestedTargetCadence`; insufficient or contradictory evidence reports `Unresolved`. Profiler shows the requested FPS, observed engine FPS, and this source separately and states that synchronized presentation does not reveal physical refresh or display cadence. Local same-process smokes change 5 to 10 FPS below the active ceiling and 5 to 1000 FPS above it; they require requested-cadence and backend presentation classifications respectively while preserving the existing native control-point evidence.
