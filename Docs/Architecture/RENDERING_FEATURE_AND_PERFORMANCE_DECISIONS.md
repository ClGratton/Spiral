...
Effective-limiter diagnostics require causal frame alignment and are not yet accepted. `StartToStartMilliseconds` on record `N` is `FrameStart[N] - FrameStart[N-1]`; the InterFrame release on `N` is inside that interval, while SubmissionGate wait, CPU work, GPU work, and `Present` that can occupy it belong to `N-1`. The current provisional classifier instead compares the interval on `N` with CPU/GPU fields on `N` and a mutable latest-backend presentation snapshot. Hosted WARP and a local retry exposed observable misclassification, so target rebasing remains complete but limiter categories are reopened. The required classifier consumes an explicit `N` cadence/`N-1` source association, retains the policy at each actual control point, amends cadence `N` when delayed exact GPU evidence for `N-1` arrives, requires immutable frame-keyed D3D12/Vulkan present publication, and reports `Unresolved` for first/gapped/evicted/missing/contradictory evidence. Profiler may keep showing the provisional field during implementation, but no roadmap or verification claim may treat it as source-qualified until that prerequisite passes. None of these engine intervals are display cadence.
...
The implemented capture publishes immutable renderer snapshots and schema-3 CSV/JSON. Percentiles use sorted nearest-rank samples; the 1% and 0.1% low FPS summaries are `1000 / p99` and `1000 / p99.9` start-to-start frametime. Requested and effective policy targets are separate manifest fields. Schema 2 added run ID, process ID, canonical executable path, QPC frequency, and lifecycle QPC ticks. Schema 3 adds explicit GPU timing status/duration/headroom: a ready P3B duration may amend only its exact retained frame, and headroom is `1000 / that frame's effective Smooth target FPS - that frame's GPU interval`. Responsive, missing/nonpositive target, stale/evicted/mismatched, non-ready, and disjoint cases stay unavailable. Presentation/sync/VRR/tearing strings remain runner inputs defaulting to `unknown`; display/replacement/input stay unavailable and are not inferred from GPU timing. PresentMon join/supervision accepts both schema 2 and 3 raw engine inputs.
