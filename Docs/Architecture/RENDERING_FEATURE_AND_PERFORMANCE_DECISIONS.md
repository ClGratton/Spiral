...
The implemented capture publishes immutable renderer snapshots and schema-8 CSV/JSON. Percentiles use sorted nearest-rank samples; the 1% and 0.1% low FPS summaries are `1000 / p99` and `1000 / p99.9` start-to-start frametime. Requested and effective pacing targets are separate manifest fields. Schema 2 added run/process/QPC identity, schema 3 exact-frame GPU timing/headroom, schema 4 cadence predecessor and limiter-source identity, schema 5 exact input-sample source plus input-to-simulation/submit/`PresentEnd`, schema 6 deadline-wait telemetry, and schema 7 engine-owned requested presentation policy, capability, actual native mode, fallback, native generation, semantic synchronization, and tearing-present state. Schema 8 adds bounded per-frame key, mouse-button, and mouse-motion annotations with event-relative timing, plus an explicit benchmark sample-count option whose default remains 512. These annotations correlate interaction with the already source-aligned cadence; they are not peripheral timestamps or input-to-photon evidence. PresentMon join/supervision accepts schemas 2-8 and never converts engine-stage input or API/present capability into display evidence. VRR-active remains `unavailable`.
