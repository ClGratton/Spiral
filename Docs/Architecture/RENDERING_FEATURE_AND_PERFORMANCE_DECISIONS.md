...
Shipped `Smooth Frametime` is wholly engine-owned: it must not require RTSS, MSI Afterburner, their installation, runtime process, configuration/profile mutation, or shared-memory access. The existing opt-in `InterFrame` path and engine Platform deadline clock already provide the internal-cap mechanism: release occurs after the prior `Present` and before the next frame's timestep, `FrameStart`, input, and simulation. It does not depend on an unproven story that RTSS holds completed CPU instructions. RTSS/Afterburner remain optional, separately named laboratory/reference conditions under the bounded manual gate, never a runtime dependency or selected default. No production candidate has been selected and this decision changes no renderer behavior.
...
The implemented capture publishes immutable renderer snapshots and schema-5 CSV/JSON. Percentiles use sorted nearest-rank samples; the 1% and 0.1% low FPS summaries are `1000 / p99` and `1000 / p99.9` start-to-start frametime. Requested and effective policy targets are separate manifest fields. Schema 2 added run/process/QPC identity, schema 3 added exact-frame GPU timing/headroom, and schema 4 added cadence predecessor and limiter-source identity. Schema 5 adds exact input-sample source frame plus input-to-simulation, input-to-submit, and input-to-`PresentEnd`; input-to-display and click-to-photon stay unavailable. Presentation/sync/VRR/tearing strings remain runner inputs defaulting to `unknown`. PresentMon join/supervision accepts schemas 2-5 and never converts engine-stage input fields into display evidence.
