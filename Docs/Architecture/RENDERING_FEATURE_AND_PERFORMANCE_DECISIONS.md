...
Windows production-pacing measurement requires a separate PresentMon source stream rather than treating engine `Present` timestamps as display feedback. The engine first needs an external-capture attachment boundary: after renderer/window initialization it publishes a unique run ID, exact process ID/canonical executable path, QueryPerformanceCounter frequency/current tick, artifact location, and requested condition metadata, then waits on a bounded explicit supervisor release before its 30-frame warm-up and retained window begin. Timeout or cancellation fails and cleans up; running without the attachment option preserves the existing benchmark path.
