...
- Persistent Scene integration, stable-ID view-origin tracking, and canonical snapshot/raster propagation are complete for the current built-in D3D12 prototype-geometry workflow. The MSVC Debug build passes with zero warnings/errors, the focused deterministic suite passes, and the comparative D3D12 capture passes. Real mesh/material GPU resources, Vulkan scene raster, culling, coordinate error views, physics islands, and ray/TLAS/query consumers remain future work and receive no qualification from this evidence.
