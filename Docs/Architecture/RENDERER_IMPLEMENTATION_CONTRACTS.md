...
- `PrepareSceneRasterFrame` keeps Scene extraction handle-only and resolves material identity against one immutable Renderer catalog generation. Row zero is the default/error row. Distinct valid material handles are sorted numerically into rows `1..N`; duplicates reuse a row, while invalid/missing handles, registered wrong-type handles, and registered Material handles missing from the published material library select row zero. That row publishes `MaterialState = {0, 1, 0, 0}`. The resulting ID is frame-local and is not an asset, entity, or visibility ID.
...
Renderer packs the accepted immutable Scene snapshot and its matching `ClusteredLightGrid` into a bounded versioned sequence of explicit `uint32x4` words. The header contains checked offsets and counts for complete light records, global directional indices, all CSR cluster offsets, and local-light indices. Each light retains stable source identity, type, explicit photometric unit, raw double-value bits, a separately checked float consumption value, view position, world/view directions, color, range, cone cosines, and shadow flag. The builder validates snapshot/grid one-to-one agreement, all finite/range constraints, multiplication/addition overflow, maximum byte size, monotonic CSR, exact terminal offset, index bounds, the 16-directional hard limit, global-directional membership, and local-only clustered membership before replacing caller output.
...
The production PBR entry declares and binds the payload but continues to evaluate the existing neutral non-photometric preview light for this prerequisite. A mutually exclusive diagnostic entry must read varied header, directional, point, spot, global-index, cluster-offset, and local-index words and produce discriminating HDR/readback cells. Its oracle derives expectations from the fixture snapshot/grid without calling the packer or comparing the packer to itself. The required marker reports exact CPU/GPU agreement and graph-token retention while stating `productionPSMain=preserved lightingEvaluation=no`. Directional/point/spot BRDF evaluation, attenuation/cone normalization, shadows, a GPU-built cluster grid, general structured descriptors, D3D12 execution, and MoltenVK execution remain separate claims.
...
- Both manual and calibrated modes use the current shader sign convention `scale = exp2(-effectiveEV100)`, so increasing the effective value darkens the scene and `0.0` preserves unit scale. The current paper-white scale is fixed at `1.0`.
- Scene viewport renderers must snapshot the accepted color-pipeline settings before building the render graph, record the graph tone-map pass and smoke-only direct-reference path with the same immutable constant bytes, and retain those constants through the exact accepted graph submission token. Repeated renders at unchanged complete settings must reuse the current immutable constants; any accepted settings change publishes a distinct immutable generation even when the resolved EV is unchanged. Prior values remain alive only through graph callback/local ownership and exact-token payload retention. A mutable per-pass constant buffer must not be overwritten while in-flight work may still reference it; arbitrary rings are not authority unless their lifetime is tied to accepted completion.
