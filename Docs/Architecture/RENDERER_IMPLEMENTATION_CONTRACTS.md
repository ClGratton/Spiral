...
- Raster preparation publishes the row-vector `S^-1 * R` inverse-transpose of each model linear transform. Nonuniform scale is supported; zero, nonfinite, or singular scale on any mesh rejects the complete prepared frame transactionally. The shared 256-byte Scene constants and explicit `sizeof(MeshArtifactVertex)` position/normal/color/UV pipeline stride expose the normal plus frame-local material ID to both D3D12 and Vulkan paths. Accepted submissions retain those exact constants and resource bundles through their final graph token.
