...
- Persistent Scene integration, stable-ID view-origin tracking, and canonical snapshot/raster propagation are complete for the current built-in D3D12 prototype-geometry workflow. The MSVC Debug build passes with zero warnings/errors, the focused deterministic suite passes, and the comparative D3D12 capture passes. The D3D12 prototype still uses a temporary native presentation command-list bridge because the current RHI command contract has no render-target bind/clear operation. Before any Vulkan scene implementation, a backend-neutral viewport-output recording contract must bind/clear renderer-owned color/depth outputs through `Engine::RHI`, then hand the completed NVRHI output to native WSI/ImGui only at presentation. It must not turn the presentation owner into a scene renderer or expose Vulkan handles to Scene, Editor, or gameplay. Real mesh/material GPU resources, Vulkan scene raster, culling, coordinate error views, physics islands, and ray/TLAS/query consumers remain future work and receive no qualification from this evidence.
