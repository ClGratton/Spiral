...
- Sector/local representation, origin-transition policy, propagation of the per-view origin through the immutable render snapshot, actual scene-raster integration, culling, debug error views, physics islands, and ray/TLAS/query consumers remain future work. Until those consumers exist and share the origin, this is a coordinate foundation rather than completion of the full large-world rendering contract.
