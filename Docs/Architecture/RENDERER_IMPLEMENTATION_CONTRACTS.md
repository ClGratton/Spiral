...
- Persistent Scene integration is complete for version-4 serialization and legacy migration. The stable-ID stateful origin tracker, sector-snapped runtime behavior, multi-view publication tests, sector/local snapshot/raster propagation, real mesh/material GPU resources, Vulkan scene raster, culling, coordinate error views, physics islands, and ray/TLAS/query consumers remain future work. In particular, declaring or serializing `SectorSnapped` does not claim that view-origin tracking or renderer propagation is implemented.
