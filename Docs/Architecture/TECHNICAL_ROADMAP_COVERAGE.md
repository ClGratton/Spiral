...
| Layered water | Cascaded spectral deep water, camera-relative continuous-LOD surface mesh, bounded terrain/obstacle-aware 2.5D zones, authored breakers/waterfalls, coherent optical lighting and underwater rendering, deterministic CPU surface queries, and optional fixed-step physics coupling | Phase 8W after Phase 7 terrain inputs and Phase 8 lighting/volumetrics. Phase 9 adds optional sparse RT misses; Phase 11 consumes water queries without waiting on GPU visuals. |
...
- Water bodies consume versioned terrain and lighting inputs but retain their own body, simulation, query, rendering, and physics boundaries; a bounded heightfield zone is not treated as a universal fluid solver.
...
- Water now has an accepted planning contract in [WATER_ARCHITECTURE_AND_RESEARCH.md](WATER_ARCHITECTURE_AND_RESEARCH.md) and dependency-ordered Phase 7/8/8W/9/11 coverage for terrain inputs, shared lighting, layered simulation and rendering, sparse ray enhancements, deterministic queries, and optional physics coupling.
