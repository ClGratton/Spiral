...
cp "$ROOT"/Tests/Corpus/Fuzz/*.case "$FUZZ_CORPUS/"
...
"$ENGINE_FUZZ" -runs=512 -artifact_prefix="$ROOT/output/fuzz-failures/" "$FUZZ_CORPUS"
echo "SanitizerLaneV1 mode=asan-ubsan engineTests=pass structuredFuzz=pass runs=512 vendorInstrumentation=excluded mutableCorpus=output result=pass"
