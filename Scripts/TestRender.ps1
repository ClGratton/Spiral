...
$RequiredMarkers = @(
    ...
    "SceneSkyAtmosphereV1 backend=D3D12 model=Preetham1999"
    "skyDome=procedural-fullscreen diffuseIrradiance=first-order-zonal graphLabel=Scene-Sky-Atmosphere result=pass"
    "SceneViewportRenderGraphV1 backend=D3D12 passes=7 labels=light-payload-copy,primary-directional-shadow-map,clear,sky-atmosphere,raster,tone-map,output-handoff execution=pass reference=direct comparator=exact-byte-pass"
...
if ($JoinedLog -notmatch 'RenderGraphRecordingV1 backend=D3D12 mode=worker workerPasses=2 overlap=(yes|no) submitted=7 result=pass') {
    throw "D3D12 render smoke did not prove the parallel RenderGraph recording marker."
}
$RetirementMatches = [regex]::Matches($JoinedLog, 'ProductionRenderGraphRetirementV1 backend=D3D12 frame=\d+ passes=7 cpuWaitBetween=no pending=\d+ result=pass')
...
$TimestampScopeMatches = [regex]::Matches($JoinedLog, 'RenderGraphTimestampScopesV1 backend=D3D12 frame=\d+ scopes=7 raw=ready cpuWaitBetween=no result=pass')
...
$GpuTimingMatches = [regex]::Matches($JoinedLog, 'RendererGpuTimingV1 backend=NVRHI D3D12 frame=\d+ passes=7 wholeMs=[0-9]+(?:\.[0-9]+)? status=Ready capability=GpuTimestamps result=pass')
...
if (($InlineRecordingJoinedLog -notmatch 'SceneRasterPreparationV1 mode=single-thread task=Frame.PrepareSceneRaster worker=caller') -or ($InlineRecordingJoinedLog -notmatch 'RenderGraphRecordingV1 backend=D3D12 mode=inline workerPasses=2 overlap=no submitted=7 result=pass') -or ($InlineRecordingJoinedLog -notmatch 'SceneViewportRenderGraphV1 backend=D3D12 passes=7 labels=light-payload-copy,primary-directional-shadow-map,clear,sky-atmosphere,raster,tone-map,output-handoff execution=pass reference=direct comparator=exact-byte-pass') -or ($InlineRecordingJoinedLog -notmatch 'ProductionRenderGraphRetirementV1 backend=D3D12 frame=\d+ passes=7 cpuWaitBetween=no pending=\d+ result=pass')) {
