...
- Use the `terra_medium` custom agent for substantive delegated repository work. Its local definition is `$CODEX_HOME/agents/terra_medium.toml` (normally `~/.codex/agents/terra_medium.toml`; alternate local accounts may use another `CODEX_HOME` such as `~/.codex_1`), whose `name = "terra_medium"` pins `model = "gpt-5.6-terra"`, medium reasoning, and the implementation-owner contract. Never use generic, default, worker, or inherited subagents when `terra_medium` is available; spawn it with fresh scoped context instead of the complete parent history. Use as many Terra workers concurrently as there are concrete, independent, non-overlapping tasks that justify them; minimize token waste, redundant research or review, and idle agents, and never use concurrency as a quota. If neither the direct custom-agent route nor the persistent fallback below is available, continue in the root session and report that routing is unavailable.
...
- Prefer first-class subagent orchestration when it exposes custom-agent selection, and select the agent by the exact role name `terra_medium`; selecting only the `gpt-5.6-terra` model is not equivalent because it omits the custom agent's developer instructions. When the original app root's direct spawn wrapper cannot specify an agent role, use a **persistent CLI orchestrator**, not a generic child or direct model invocation: start one bounded low-cost root with `codex exec -m gpt-5.6-luna -c 'model_reasoning_effort="low"' -s <least-privilege-mode> -C <workspace> -`, and instruct it to use its native SpawnAgent/collaboration tool to spawn exactly one configured `terra_medium`, wait for that child, do no duplicate work, and return only the distilled result. Once inside that persistent CLI root, never invoke another `codex exec` or reapply this fallback recursively. Omit `--ephemeral`; optionally use `-o` with a temporary path to keep streamed logs out of the parent transcript. A successful run must persist both the `source="exec"` parent rollout and a child rollout whose source names `agent_role="terra_medium"` and the parent thread ID; record those sessions when auditing usage. Never use a user-owned app task/thread as a substitute for a subagent unless the user explicitly asks for a separate task.
