...
- Do not use repeated CI-status or unchanged-state polling as a progress loop. At a coherent integration milestone, batch push and hosted CI when local evidence suffices, preserve separate slice evidence and commits, look up CI by full commit SHA, and retain only compact final status/log excerpts in root context. After one unchanged check, use an event-driven/process-level monitor, continue independent useful work, or record the run/agent as pending in `HANDOFF.md`.
