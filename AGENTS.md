...
- Prefer first-class subagent orchestration when it exposes custom-agent selection, and select the agent by the exact role name `terra_medium`; selecting only the `gpt-5.6-terra` model is not equivalent because it omits the custom agent's developer instructions. When the app's direct spawn wrapper cannot specify an agent role, use a **persistent CLI orchestrator**, not a generic child or direct model invocation: start a bounded low-cost root with `codex exec -m gpt-5.6-luna -c 'model_reasoning_effort="low"' -s <least-privilege-mode> -C <workspace> -`, and instruct it to spawn exactly one configured `terra_medium`, wait for that child, do no duplicate work, and return only the distilled result. Omit `--ephemeral`; optionally use `-o` with a temporary path to keep streamed logs out of the parent transcript. A successful run must persist both the `source="exec"` parent rollout and a child rollout whose source names `agent_role="terra_medium"` and the parent thread ID; record those sessions when auditing usage. Never use a user-owned app task/thread as a substitute for a subagent unless the user explicitly asks for a separate task.
