...
- Use the `terra_medium` custom agent for substantive delegated repository work. Never use generic, default, worker, or inherited subagents when `terra_medium` is available; spawn it with fresh scoped context instead of the complete parent history. Use at most two Terra workers concurrently. If `terra_medium` cannot be selected, continue in the root session and report that routing is unavailable.
