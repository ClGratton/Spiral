...
- The default delegation pattern is one Terra Medium owner per coherent vertical slice. That owner implements the slice, runs focused verification, investigates failures, and reports the concrete diff plus evidence. Do not independently redo the same research, implementation analysis, or code audit assigned to that agent merely to feel informed.
...
- At a safe milestone in a long task, if the root transcript has accumulated multiple completed slices or large diagnostic output, refresh `HANDOFF.md` with the exact objective, decisions, diff state, evidence, and next ordered action, then use supported context compaction or begin a fresh task from `HANDOFF.md`. Do not restart mid-edit, mid-test, or while an agent owns unreported work, and do not carry a long transcript merely to preserve facts that already exist in durable repository files.
- After launching a long-running owner or process, send at most one user-visible start update and then remain silent until it exits, fails, or produces a decision-relevant event. Do not emit periodic “still running,” elapsed-time, healthy-process, or “no final report” messages. Configure the launch/tool call to remain pending until completion or use one event-driven/process-level monitor with the longest supported wait; do not re-enter the model every minute through repeated `wait`, `wait_agent`, terminal-read, or CI-status calls. If the available surface cannot await completion without repeated model turns, detach the persistent process, record its session/result path and exact pending state in `HANDOFF.md`, and return control instead of polling.
- Do not use repeated CI-status or unchanged-state polling as a progress loop. After one unchanged check, use an event-driven or single process-level monitor when available, continue independent useful work, or record the run/agent as pending in `HANDOFF.md` and return control. Never spend model turns merely to keep an active turn open.
