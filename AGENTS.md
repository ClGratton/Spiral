...
- Prefer first-class subagent orchestration when it exposes custom-agent selection, and select the exact role name `terra_medium`; selecting only the `gpt-5.6-terra` model is not equivalent because it omits the custom agent's developer instructions. When the app root cannot select a custom role, start the owner **directly** as one persistent CLI session with `codex exec -p terra_medium -s <least-privilege-mode> -C <workspace> -`; the `$CODEX_HOME/terra_medium.config.toml` profile loads the same model, medium reasoning, and developer instructions as the native role. Omit `--ephemeral`, use `-o` with a temporary result path when useful, and do not insert a Luna/default wrapper or ask the direct owner to spawn another agent. Verify the startup banner reports `gpt-5.6-terra`, medium reasoning, the intended workspace/permissions, and a persisted session ID. Record that rollout when auditing usage. Never use a user-owned app task/thread as a substitute unless the user explicitly asks for a separate task, except for the supported automatic same-goal context transition described next.
- When reliable telemetry shows root input context at or above 100k tokens, or after two to three completed slices, treat that threshold as a transition trigger rather than a stop condition. In order: (1) finish and commit the current coherent slice; (2) refresh `HANDOFF.md`; (3) if the current interface supports safe compaction, compact, reread the mandatory repository state, and continue the same goal automatically; (4) otherwise, if it supports creating and continuing in a fresh first-class task while preserving the same goal, perform that transition, reread the mandatory state, and continue automatically. A user request to continue the roadmap is standing authorization for the agent to perform these same-goal transitions itself without returning control merely to ask the user to create, open, or resume a task. Stop and return control only when neither transition is available, CI or user input is genuinely required, or the current user-defined allowance floor has been reached. Do not transition mid-edit, mid-test, or while an owner has unreported work, and never use the transition to broaden scope, change branches, or bypass required evidence.
