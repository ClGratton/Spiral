...
- [ ] **Phase 3 pacing latency-source prerequisite:** preserve the authoritative frame ID from the engine input-poll/sample boundary through simulation, submit, `Present`, GPU completion, and any external display observation; publish source-qualified input-to-simulation/submit/present intervals, attach input-to-display only when a measured external/platform source supplies it, and keep click-to-photon `unavailable` unless an admitted measurement path actually observes photons. Extend deterministic artifacts and D3D12/Vulkan native evidence without renaming CPU markers as display or end-to-end latency.
