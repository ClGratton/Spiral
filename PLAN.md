...
- [ ] **Phase 3 test-loop prerequisite T3 — structured fuzz replay and sanitizer lane:** add bounded structure-aware corpus/generator targets for current untrusted/versioned inputs (Scene serialization and portable shader/material/import artifacts as applicable), combining valid generation with field-aware corruption, truncation, version edits, bit flips, and crossover. Provide deterministic checked-in corpus replay in the normal integration gate, ignored failure artifacts plus minimization/replay commands for campaigns, and an admitted ASan/UBSan configuration and hosted lane with symbolized failures and explicit vendor exclusions; separately defer TSan until its concurrency/toolchain boundary is qualified. Raw random bytes and line coverage alone are not completion evidence.
