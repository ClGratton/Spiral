...
- [ ] **Phase 3 GPU timing and headroom prerequisite P3 before pacing selection and lighting/shading:** after P1/P2, instrument whole-frame and named RenderGraph/pass scopes; preserve frame/pass identity through asynchronous readback; publish actual `GpuMilliseconds` plus per-pass GPU duration in Profiler while retaining truthful CPU fallback on unsupported devices; and publish benchmark GPU headroom against the exact target-frame budget only for a valid matching-frame measurement. Qualify D3D12/Vulkan with deterministic ordering/availability tests plus native pacing evidence. This existing parent was moved from after pacing selection because that comparison explicitly requires GPU headroom; it also remains mandatory before Phase 3E Forward+/clustered lighting, PBR shading, lights, shadows, or sky/atmosphere.
