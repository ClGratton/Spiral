...
- Hosted Actions run `29350139365` passed the Windows D3D12 path but exposed two portable-host defects before its Linux/macOS runtime coverage could run: Linux `EngineTests` could not resolve the staged versioned `libslang-compiler.so.0.2026.13.1`, and macOS libc++ rejected the C++20-only `std::atomic<std::shared_ptr<const T>>` specialization. Commit `5df91bb` repaired both, but replacement Actions run `29352633796` then built and launched both portable `EngineTests` jobs only to fail the shared DXIL+SPIR-V integration test because non-Windows hosts have no admitted DXC downstream compiler. Generated Linux executables carry `RUNPATH=$ORIGIN`, generated macOS executables carry `LC_RPATH @loader_path`, and renderer snapshot publication selects the specialization only where `__cpp_lib_atomic_shared_ptr` is available, otherwise using the standard atomic `shared_ptr` free functions with release/acquire ordering. The follow-up makes the host target set explicit; replacement hosted execution remains required before Linux/macOS qualification.
