...
- The viewport prototype mesh uses a disk-backed Slang/HLSL-style shader asset loaded from `Engine/Shaders`, not an embedded shader string. On Windows x86_64/MSVC, pinned Slang v2026.13.1 and its pinned DXC v1.9.2602 downstream compiler produce one validated package per stage containing DXIL and SPIR-V; the D3D12 viewport consumes the DXIL while SPIR-V remains compiled/reflected/convention-validated artifact evidence pending a Vulkan scene consumer.
- Hosted Actions run `29350139365` passed the Windows D3D12 path but exposed two portable-host defects before its Linux/macOS runtime coverage could run: Linux `EngineTests` could not resolve the staged versioned `libslang-compiler.so.0.2026.13.1`, and macOS libc++ rejected the C++20-only `std::atomic<std::shared_ptr<const T>>` specialization. Generated Linux executables now carry `RUNPATH=$ORIGIN`, generated macOS executables carry `LC_RPATH @loader_path`, and renderer snapshot publication selects the specialization only where `__cpp_lib_atomic_shared_ptr` is available, otherwise using the standard atomic `shared_ptr` free functions with release/acquire ordering. A disposable WSL loader probe confirmed the Linux `RUNPATH` resolves the staged compiler without `LD_LIBRARY_PATH`; the replacement hosted run remains required before claiming Linux/macOS qualification.
...
- [x] Shared asynchronous scene-shader portability path with deterministic DXIL and SPIR-V output, reflected RHI layouts, backend convention validation, versioned caching, and diagnostics. The current Windows x86_64/MSVC D3D12 viewport consumes Slang v2026.13.1 DXIL (with pinned DXC v1.9.2602); its paired SPIR-V is compiled, reflected, convention-validated package evidence only until the next Vulkan scene-RHI item consumes it. Normal editor startup uses job-system fire-and-poll, smoke tests use deterministic-inline execution, and redistribution remains blocked until the admitted binary-component/notice audit is resolved. Linux, macOS, MinGW, Windows ARM64, Vulkan scene rendering, and physical-device breadth are not qualified by this completion.
