...
- [ ] **Phase 3 GPU timing prerequisite P2B — D3D12 native query translation:** consume P2A through a private D3D12 RHI bridge backed by timestamp query heaps, resolve readback storage, and the selected graphics queue's timestamp frequency. Translate P1 reset/write/resolve only after logical acceptance, collect results only after the exact completion token without a CPU wait, report failure/disjoint truthfully, retain native state through retirement, and mark D3D12 timestamp capability usable only when this full path exists. Qualify the local D3D12 backend with a bounded native allocation/write/resolve/pending/readback/reuse/destruction smoke. This is D3D12 native RHI evidence only, not frame/pass instrumentation or GPU headroom.
