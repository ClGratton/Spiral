...
- [ ] **Phase 3 selectable editor perspective-viewport navigation prerequisite:** public `Window` cursor capture/restore supports Editor-owned navigation only over the real focused viewport image and only while ImGui is not editing a widget. Navigation capture is a backend-neutral deferred epoch: the press records the visible restore position and pending state, the next update boundary enters disabled cursor mode and arms the post-transition baseline, and pre-transition/synchronous transition motion cannot accumulate a navigation delta; release or focus loss cancels pending capture without changing cursor mode, while an armed capture restores the exact visible position. The workspace-global Engine Settings preset defaults and fails closed to Fusion: the wheel is cursor-anchored bounded multiplicative zoom on the image-rect/FOV/aspect ray and persistent pivot-depth plane; MMB canvas-pans camera and pivot together in camera right/up at FOV/depth/viewport-derived scale; and Shift+MMB rotates the actual camera-to-pivot offset between old/new upright camera bases, preserving distance without assuming the camera is looking through the pivot after an off-center zoom. The persistent default pivot is a deterministic visible-mesh-transform AABB-center approximation (not unavailable geometry bounds), with an explicit camera-forward fallback; F alone replaces it with the selected entity origin. LMB/RMB, fly keys, and RMB-wheel speed control do not navigate in Fusion. Unreal remains the alternate preset with its prior plain LMB move/yaw, RMB look, LMB+RMB/MMB pan, wheel increments, RMB+wheel bounded speed, and RMB+W/S/A/D/E/Q plus arrow/Page aliases fly. Autodesk sources establish the Fusion shortcut/pivot intent; the cursor ray, transform-extent fallback, no-crossing clamp, offset-basis rotation, and deferred capture epoch are the engine implementation approximation. F is not bounds-aware Fit and MMB-double-click Fit remains deferred until real bounds/picking exist. Navigation is never serialized into `.spiralproject`; it uses the documented transactional workspace-global editor settings file. Movement uses the Application timestep and double-precision world coordinates, writes the active Scene main camera through public Scene APIs, preserves canonical sector/local authority, and marks focus relocation discontinuous before immutable snapshot publication. Picking/multi-select, bounds framing/Zoom to Fit, gizmos, orthographic modes, camera piloting, and configurable bindings remain deferred. Focused deterministic settings/navigation and bounded headed D3D12 evidence must prove preset default/persistence/separation, pending-capture cleanup, stale/transition motion rejection, cursor restoration, nonzero-pitch/yaw camera-plane pan, cursor-anchor/no-crossing zoom, zero-delta and reversible post-zoom upright orbit continuity, DVec3 motion, camera/snapshot agreement, and focus discontinuity.
- [ ] **Phase 3 production-pacing measurement prerequisite — Windows PresentMon correlation:** launch the benchmark editor into the readiness gate, attach a uniquely named bounded PresentMon session to the exact published PID, then release warm-up/capture; retain separate raw engine and exact-child-PID PresentMon streams; and publish a deterministic fail-closed join report for engine `PresentBegin` versus PresentMon present/display rows in the explicit QPC domain. Require the actual installed CSV header set, declared condition metadata, and truthful unavailable monitor/VRR/RTSS/FES/input/GPU-headroom fields. This qualifies external present/display correlation only; it neither mutates RTSS/driver state nor selects a production candidate.
