...
- [ ] Vulkan Scene viewport raster integration using the exercised Vulkan RHI primitives: consume the immutable Scene snapshot, render through `Engine::RHI`/NVRHI into renderer-owned color/depth outputs, and prove the result through deterministic offscreen readback without adding a raw-Vulkan scene path.
