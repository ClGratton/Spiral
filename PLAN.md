...
- [ ] Backend-neutral RHI buffer queue-ownership lifecycle implementation and deterministic contract: integrate explicit paired release/acquire for exact-device-owned buffers into production RHI adapters, including portable state/pair/dependency validation, accepted-release pending state, failed-acquire recovery after exact-token retirement, ordinary-operation guards, and owner/state publication only after accepted acquire. D3D12 uses global-resource semantics; Vulkan graphics fallback enters no transfer state. Prove every lifecycle/publication/failure boundary with focused deterministic tests.
