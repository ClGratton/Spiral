...
- [ ] **Phase 3 GPU timing prerequisite P2 — native query translation:** translate the P1 contract to D3D12 timestamp query heaps/readback and Vulkan timestamp query pools with timestamp-period/valid-bit handling, completion-token-gated nonblocking availability, truthful unsupported/disjoint results, and no per-frame CPU stall. Mark timestamp capability usable only when the selected backend path is actually implemented, and qualify both local backends with bounded native write/resolve/readback/reuse smokes. This is native RHI evidence only, not whole-frame/RenderGraph instrumentation or pacing headroom.
