...
- [ ] Backend-neutral RHI buffer queue-ownership lifecycle: implement explicit paired release/acquire for exact-device-owned buffers between resolved queues, including portable state/pair/dependency validation, accepted-release pending state, failed-acquire recovery after exact-token retirement, and owner/state publication only after accepted acquire. Prove the reusable lifecycle deterministically and through real D3D12 independent/fallback evidence; Vulkan graphics fallback must enter no transfer state.
