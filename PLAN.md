...
- GPU timestamp query contracts now include the backend-neutral lifecycle/retirement transaction plus a qualified D3D12 translation backed by native timestamp heaps, resolve readback, and the selected graphics queue's frequency. Vulkan native queries and renderer whole-frame/pass consumption remain pending. The renderer therefore keeps its CPU steady-clock timing fallback until P3 rather than treating the D3D12 device capability as completed frame instrumentation.
...
- [ ] **Phase 3 GPU timing prerequisite P2C — Vulkan native query translation:** consume P2A through a private Vulkan RHI escape hatch backed by a timestamp `VkQueryPool` owned alongside the NVRHI device, using the selected effective queue's nonzero `timestampValidBits`, physical-device `timestampPeriod`, legal reset/write ordering, exact-token completion, and non-waiting result collection. Mask/normalize valid bits as required, report unsupported/not-ready/device-loss results truthfully, retain native state through retirement, and mark Vulkan timestamp capability usable only when this full path exists. Qualify the local Vulkan/NVRHI backend with a bounded native allocation/write/resolve/pending/readback/reuse/destruction smoke. This is Vulkan native RHI evidence only, not frame/pass instrumentation or GPU headroom.
