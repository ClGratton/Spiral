...
- [ ] Vulkan/NVRHI `Engine::RHI::Device` primitives required by the scene path, exercised through a deterministic offscreen conformance indexed draw/readback after real device creation. This covers NVRHI-owned buffers, textures, SPIR-V shaders, graphics binding/pipeline/framebuffer state, constant-buffer updates, transitions, command submission, and GPU-retired lifetime only; it does not consume a Scene snapshot or expose a viewport texture to ImGui.
- [ ] Vulkan scene viewport integration using the exercised `Engine::RHI::Device` primitives, including Scene snapshot consumption and the narrow completed-NVRHI-output-to-native-presentation/ImGui handoff; keep raw Vulkan confined to bootstrap, WSI/presentation, and ImGui.
