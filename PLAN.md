...
- [ ] **Phase 3 external reference-condition prerequisite — RTSS `ASYNC` protocol:** establish a documented supported interface and version for an exact-Editor-profile RTSS reference run, capture the relevant before state, apply and verify the requested condition, and restore the exact prior state with a receipt even after timeout/failure. If the installed RTSS exposes no supported non-UI transactional path, record the candidate unavailable for automation and define the explicit bounded manual evidence gate instead of guessing config formats or using UI automation. Never mutate global profiles, other processes, driver state, or FES/frame-generation settings as an implicit side effect.
