...
- The headed presentation prototype does not deliberately discard completed engine frames, but it still hard-codes synchronized backend behavior: D3D12 uses a waitable swapchain with maximum latency one and `Present(1, 0)`, Vulkan uses FIFO, and the dormant OpenGL path requests swap interval one. The project now persists a versioned `Responsive`/opt-in `Smooth Frametime` policy and exposes developer runtime override resolution, all with `behavior=policy-only`: there is no engine pacing wait, cap, discard, or smoothness/latency claim. These bootstrap choices remain unchanged and are not this policy's implementation; the next presentation-timing item is the implementation and measurement gate.
...
- [x] Project-owned frame-pacing policy with a developer-exposable runtime game-setting override: `Responsive` is the engine/project default and imposes no engine-side smoothness cap, delay, or deliberate render-result discard; opt-in `Smooth Frametime` serializes a validated target cadence as intent only. The version-2 `.spiralproject` manifest migrates version 1 projects to `Responsive`, rejects malformed/invalid policies transactionally, and the editor Project Settings plus public `GameFramePacingSettings` resolve project/override state with `behavior=policy-only` diagnostics. This does not implement an RTSS-ASYNC-inspired pacer or claim smoothness/latency behavior. Keep presentation sync/VRR/tearing, frame-delivery replacement, maximum frames in flight, fixed simulation cadence, and portable/vendor latency modes as independent controls.
- [ ] **Phase 3 presentation-pacing prerequisite — shared frame lifecycle telemetry and policy handoff:** publish the resolved project/runtime frame-pacing policy to the application/presentation path without adding an intentional wait; carry the authoritative application frame ID through frame start, input/simulation, render submission, `Present` begin/end, and GPU-completion observation; report intentional pacing wait separately from mandatory DXGI latency-object and Vulkan acquire/fence correctness waits; and publish truthful unavailable states for display cadence and replacement/drop when the backend has no such feedback. Add backend capability/fallback diagnostics plus deterministic trace tests before using the trace to compare pacing candidates.
