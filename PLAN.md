...
- [ ] Project-owned frame-pacing policy with a developer-exposable runtime game-setting override: `Responsive` is the engine default and imposes no engine-side smoothness cap, delay, or deliberate render-result discard; `Smooth Frametime` is explicitly opt-in, serializable in Project Settings, and exposes its target plus latency/throughput trade-off. Keep presentation sync/VRR/tearing, frame-delivery replacement, maximum frames in flight, fixed simulation cadence, and portable/vendor latency modes as independent controls.
- [ ] Implement and measure the selected presentation policy through DXGI waitable-swapchain and capability-gated Vulkan timing paths. Apply any optional pacing delay before input sampling and simulation rather than after rendering; preserve mandatory acquire/fence correctness waits; and verify `Responsive` and `Smooth Frametime` independently with separate simulation, render, present, GPU-complete, display, replacement/drop, and intentional-delay telemetry.
...
- [ ] Presentation telemetry validation with engine markers plus PresentMon or platform-equivalent evidence that distinguishes configured/effective pacing policy, target cadence, intentional pacing delay, deadline misses, queue depth, sync/delivery mode, and rendered, presented, displayed, replaced, and dropped frames rather than treating one FPS or frametime number as proof.
