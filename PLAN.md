...
- The testing strategy now requires behavior-oriented assertions, implementation-informed boundary selection, explicit invariants or independent oracles, reproducible/minimized generated failures, speed tiers, sanitizer/coverage honesty, and a complete AI test brief. Existing deterministic contract tests, exact reference comparisons, bounded headed scripts, and platform evidence already satisfy important parts; the current roughly 5,000-line/70-case `EngineTests` executable has no tier/filter or per-test timing contract, and the repository has no shared property/state-machine runner, fuzz corpus/minimizer, sanitizer lane, or coverage-guided structured fuzz targets. Phase 15 schedules those shared foundations without deferring feature-specific risk tests.
...
- [ ] Selectable test-tier runner and timing contract: Fast contract, Integration/headless, Headed/platform, and Stress/fuzz/soak selection with per-test/command duration reporting, finite tier budgets, exact rerun commands, isolated output, and no leaked processes; keep the deterministic fast suite runnable without a full rebuild and move only inherently expensive evidence to slower tiers.
- [ ] Reusable deterministic property/state-machine/differential test support for high-risk numeric, serialization, scheduling, resource-lifetime, and state-transition contracts, including boundary-weighted generators, explicit invariants or independent reference models, stable fast seeds, varying-seed campaigns, serialized replay traces, shrinking/minimization, and permanent counterexample regression fixtures.
- [ ] Coverage-guided structure-aware fuzz and sanitizer lanes for untrusted/versioned parsers, importers, shader/material/scene/cooked artifacts, and future save/network formats: valid/invalid seed corpora, grammar/field-aware mutation plus truncation/bit-flip/crossover, ASan/UBSan first and separately qualified TSan where supported, bounded PR corpus replay, scheduled campaigns, symbolized failure artifacts, and no completion claim from line coverage alone.
- [ ] Golden image tests.
