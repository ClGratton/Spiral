...
- [ ] **Phase 3 GPU timing prerequisite P2A2 — pre-submit ownership and publication preflight:** make the command-list transaction retain the logical query pool as well as backend-private shared state when the public wrapper is destroyed after recording but before submission; add a non-mutating close/submit preflight that rejects poisoned, incomplete reset/write/resolve, wrong-device, duplicate-token, stale-generation, or unreserved state before native submission; and guarantee that an accepted preflight plus newly issued exact token cannot fail logical/retirement publication afterward. Deterministic fake-native tests must destroy the public wrapper before preflight/publication, reject an incomplete recording before the simulated native submit callback, preserve zero publication on failed close/submit, and prove accepted preflight/token publication plus terminal retirement. This is shared ownership/preflight evidence only; D3D12/Vulkan paths and capabilities remain unavailable.
