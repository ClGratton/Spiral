...
- [ ] Backend-neutral RHI queue-ownership transfer prerequisite required by Vulkan multi-queue admission: expose explicit paired release/acquire transitions for exact-device-owned textures and buffers between resolved source/destination queues, carrying portable source/destination state and rejecting mismatched owner, state, queue, pairing, or dependency order before submission. Same-family transfers collapse to GPU ordering with no family barrier; D3D12 retains its global-resource semantics; Vulkan emits matched queue-family release/acquire barriers inside the RHI adapter rather than relying on NVRHI's `VK_QUEUE_FAMILY_IGNORED` state tracker. Publish the new owner/state only after the acquire submission is accepted, and prove deterministic failure plus exercised cross-family or truthful fallback evidence.
- [ ] Vulkan/NVRHI multi-queue admission prerequisite: enumerate and create selected compute/copy queues and queue families only when supported, pass them through the existing NVRHI device boundary, and implement queue-class submission/dependency semantics. Preserve the documented graphics-only fallback and distinguish same-family ordering from cross-family release/acquire ownership transfer; do not claim the latter without exercised device support.
