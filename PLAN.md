...
- [x] **Prerequisite 3A — worker-safe Scene viewport preparation:** `Frame.PrepareSceneRaster` now consumes the immutable Scene snapshot on the CPU task graph worker lane and atomically publishes the immutable current-frame raster payload before caller-affine rendering; deterministic-single-thread executes that exact task inline. D3D12/Vulkan graph consumers reject missing/stale prepared payloads rather than silently recomputing them. Local RTX 3080 Ti D3D12 and Vulkan runs each emitted `SceneRasterPreparationV1` with a worker in parallel and `caller` inline, plus `SceneViewportRenderGraphV1 ... comparator=exact-byte-pass` in both modes; MSVC Debug Editor build and `CheckCodeStyle.ps1` passed. This is CPU preparation only, not concurrent command recording or a changed GPU submission policy.
- [ ] **Prerequisite 3B — worker-safe graph recording contract:** make pass-level worker-recording eligibility explicit and safe-by-default, then prove independent eligible render-graph preparation/recording overlaps on CPU workers while callbacks without that declaration remain caller-affine. Preserve graph-derived dependencies, stable labels, callback exception behavior, bounded exact-token context retirement, and deterministic single-thread equivalence. This prerequisite must qualify the same production Scene viewport graph path before parallel recording is adopted.
- [ ] Multithreaded render preparation and command recording driven by the CPU task graph and compiled render graph, with deterministic single-thread validation mode, after prerequisites 3A and 3B: submit accepted recorded work and publish resource/context state only in compiled graph order; worker preparation/recording must not introduce CPU waits between GPU queue submissions.
