...
- [ ] Backend-neutral RHI queue-ownership state-machine foundation: expose explicit paired release/acquire transitions for exact-device-owned textures and buffers between resolved source/destination queues, validate portable source/destination state, pairing, dependency, pending-transfer failure/recovery, and exact-token retirement, and publish the new owner/state only after accepted acquire submission. Same-effective queues are ordered without transfer; D3D12 retains global-resource semantics. Prove the contract deterministically and through real D3D12 independent/fallback evidence without adding Vulkan family barriers yet.
