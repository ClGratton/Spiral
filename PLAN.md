...
- [ ] **Phase 3 production-pacing measurement prerequisite — Windows PresentMon correlation:** export benchmark lifecycle records in an explicit Windows QPC clock domain with process/run identity; supervise a bounded PresentMon capture started before the benchmark child; retain separate raw engine and exact-child-PID PresentMon streams; and publish a deterministic fail-closed join report for engine `PresentBegin` versus PresentMon present/display rows. Require declared condition metadata and truthful unavailable monitor/VRR/RTSS/FES/input/GPU-headroom fields. This qualifies external present/display correlation only; it neither mutates RTSS/driver state nor selects a production candidate.
