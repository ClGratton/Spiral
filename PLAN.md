...
- [x] **Phase 3 GPU timing prerequisite P2A — native-query transaction and retirement seam:** `TimestampQueryTransaction` advances the P1 logical operation before invoking a backend callback, poisons native callback failure, reserves bounded retirement capacity before submit, abandons a fully valid recording without publication when submission fails, and publishes the P1 generation plus opaque backend-private shared state only for an accepted exact-device completion token. The device-owned retirement queue retains that state after public-pool destruction, rejects duplicate-state/cross-device/stale/incomplete retirement and reuse, releases only on the exact terminal token, and caps reserved plus unresolved records at four without waiting. P2B/P2C are the named consumers; no native handle crosses `Engine::RHI`. The zero-warning Debug build and 66/66 `EngineTests` prove validation-before-callback, native/submit rollback, exact-token publication, completion-gated result/state reuse, destruction-before-completion retention, and bounded retirement. Both backend query paths and timestamp capabilities remain unavailable, so this checks shared lifetime/transaction evidence only—not D3D12/Vulkan timestamps, renderer timing, or GPU headroom.
