...
- [ ] **Phase 3 Smooth target application and effective-cap prerequisite:** diagnose the reported headed case where changing Smooth Target FPS does not visibly change cadence (the 2026-07-16 capture requests 180 FPS while Profiler shows about 15.7 ms). Trace the saved/runtime-resolved target through deadline rebasing and both native candidate control points; distinguish requested target, actual engine release cadence, GPU/present limits, and DXGI `Present(1)`/Vulkan FIFO display-refresh ceilings rather than calling every clamp a pacing failure. A live target edit must take effect at the next valid frame boundary without stale candidate deadlines, and diagnostics must expose the effective limiting source when the requested rate is unattainable. Add deterministic target-change/reset tests plus headed D3D12/Vulkan evidence at rates below and above the active presentation ceiling before pacing comparison/selection.
