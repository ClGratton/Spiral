...
- GPU timestamp query contracts now include the backend-neutral lifecycle/retirement transaction plus qualified D3D12 and Vulkan translations. D3D12 uses native heaps/resolve readback and queue frequency; Vulkan uses a private query pool, graphics-family valid-bit masking, physical timestamp period, and non-waiting exact-token result collection. P3A now instruments the real three-pass D3D12/Vulkan viewport RenderGraphs and publishes completion-gated raw endpoints with exact frame/pass/token/generation identity. Duration conversion, Profiler publication, capability-path promotion, and benchmark headroom remain P3B/P3C, so both backends still truthfully select CPU steady-clock timing.
...
- [ ] **Phase 3 GPU timing prerequisite P3B — Renderer/Profiler asynchronous GPU duration publication and CPU fallback:** associate retired P3A results with the exact frame/pass identities; publish each named-pass duration and a same-Graphics-queue whole-frame interval from first-pass start to final-pass end without summing passes or inferring across queues. Amend retained renderer timing by frame ID, select/exercise `GpuTimestamps` only after a real ready consumer result, and preserve explicit unavailable/pending/disjoint states plus portable CPU timing on unsupported devices. Qualify D3D12/Vulkan with deterministic identity/status tests and headed viewport evidence.
