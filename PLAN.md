...
- [ ] RHI queue-topology and GPU-dependency prerequisite: distinguish enabled independent graphics/compute/copy queues from graphics-queue fallback, expose deterministic effective-queue resolution, and allow a submission to wait on prior same-device completion tokens entirely on the GPU without exposing native fences or semaphores. Reject invalid, cross-device, unissued, forward, cyclic, and impossible self waits; same-effective-queue dependencies collapse to normal ordered submission.
