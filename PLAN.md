...
- [x] **Phase 3 Smooth target application and effective-cap prerequisite:** each experimental control point now retains its last actual release, target, and next deadline. Re-publishing an identical policy preserves phase; a target or candidate change invalidates the old deadline, rebases from the last valid release at the next applicable control point, waits there when the new deadline is still ahead, or truthfully records a late frame and rebases from observation. The per-frame resolved-policy snapshot still prevents a mid-frame edit from applying both candidates. Profiler separates requested target, observed released `FrameStart` cadence, and an evidence-qualified effective source: requested cadence, CPU active work, exact-frame GPU work, D3D12 synchronized `Present(1)`, Vulkan FIFO presentation, or unresolved. This never infers monitor refresh or display cadence. The zero-warning Debug build and 75/75 tests prove faster/slower target changes, candidate switch/back, Responsive reset, identical-policy phase retention, and every diagnostic category. Local RTX 3080 Ti `TestRender.ps1` and `TestVulkan.ps1` pass same-process `5 -> 10 FPS` transitions as `RequestedTargetCadence` and `5 -> 1000 FPS` transitions as `D3D12SynchronizedPresent` or `VulkanFifoPresent`, while retaining the original 5 FPS native control-point smokes and bounded cleanup. This diagnoses the reported 180 FPS/~15.7 ms case as a possible synchronized-presentation ceiling when work does not fill the interval; it does not prove physical display cadence, select a production candidate, or change VSync/FIFO policy.
