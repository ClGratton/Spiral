...
- [ ] **Phase 3 production Smooth end-to-end input/physical-display evidence decision prerequisite:** admit an appropriate source and control path for the required input, physical-display, and VRR selection evidence before using any of those signals to choose a candidate; same-frame engine input-to-`PresentEnd` is not photon latency. If an admitted source is unavailable for a condition, retain that field as unavailable and exclude it from the corresponding selection claim rather than substituting engine or present cadence.
