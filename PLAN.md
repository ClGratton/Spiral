...
- [ ] **Phase 3 PresentMon correlation prerequisite — deterministic raw-stream parser and join report:** consume separate schema-2 engine JSON and exact-header PresentMon CSV inputs; require matching QPC domain and exact process ID; retain warm-up/trailing rows; and publish a stable schema-1 report that pairs each retained engine `PresentBegin` with exactly one causal PresentMon row before the next engine present, with a bounded final-row tolerance. Reject wrong PID, missing/reordered/duplicate/non-monotonic rows, missing headers, ambiguous intervals, reuse, and noncausal matches. Preserve both raw streams and PresentMon display/present fields without copying them into engine observations. This qualifies parser/join determinism only, not live collector supervision or a production candidate.
