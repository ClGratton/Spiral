...
- [ ] **Phase 3 production Smooth deadline-precision prerequisite:** replace the production system deadline wait with a Platform-owned monotonic waiter. Windows uses a per-clock `CREATE_WAITABLE_TIMER_HIGH_RESOLUTION` timer plus an at-most-0.5 ms bounded and reported final active tail when available; creation/arm/wait failure falls back to portable steady-clock waiting without changing Renderer policy, candidate control points, phase anchoring, mandatory backend waits, or frame-retention behavior. Deterministic clock traces must prove 60/120 FPS deadlines, target/candidate changes, early wake, bounded tail, fallback, handle cleanup, late rebase, and no frame drop/replacement. Native D3D12 60/120 InterFrame/SubmissionGate evidence must publish wait primitive, release overshoot, and a bounded CPU-time/wall-time cost proxy; portable/Vulkan evidence may report fallback and must not claim Windows timer precision. This establishes measured deadline behavior only, not display cadence or a production winner.
