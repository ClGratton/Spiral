...
- GPU timestamp query contracts now include the backend-neutral lifecycle/retirement transaction plus qualified D3D12 and Vulkan translations. D3D12 uses native heaps/resolve readback and queue frequency; Vulkan uses a private query pool, graphics-family valid-bit masking, physical timestamp period, and non-waiting exact-token result collection. Renderer whole-frame/pass consumption remains pending, so both backends keep CPU steady-clock timing until P3 rather than treating device capability as completed frame instrumentation.
...
- [ ] **Phase 3 GPU timing prerequisite P3A — RenderGraph timestamp scopes and bounded asynchronous query-slot retirement:** for each accepted effective-Graphics production pass, record a distinct two-query pool/transaction with reset, start timestamp, pass work, end timestamp, and resolve in that pass's command list. Retain the query pool/generation with P2D's authoritative frame, ordered pass label, and exact token until nonblocking terminal collection. Size the device retirement bound for four submitted three-pass frames, reject unsupported/cross-queue or capacity cases truthfully, and prove accepted-prefix failure, 12-state capacity, no-wait collection, and reuse. This slice publishes raw completion-gated scope results only; it does not yet make Profiler or benchmark duration/headroom claims.
