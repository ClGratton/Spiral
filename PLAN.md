...
- [ ] **Phase 3 editor perspective-viewport navigation prerequisite:** public `Window` cursor capture/restore supports Editor-owned navigation only over the real focused viewport image and only while ImGui is not editing a widget. Plain LMB drag moves forward/back and yaws; RMB drag looks; LMB+RMB and MMB drag pan; wheel moves by increments; RMB+wheel changes bounded speed; RMB+W/S/A/D/E/Q flies forward/back/left/right/global up/down, with trivial arrow/PageUp/PageDown aliases; and F focuses the existing selected entity origin. Movement uses the Application timestep and double-precision world coordinates, writes the active Scene main camera through public Scene APIs, preserves canonical sector/local authority, and marks focus relocation discontinuous before immutable snapshot publication. Picking/multi-select, Alt pivot navigation, bounds framing, gizmos, orthographic modes, camera piloting, and configurable bindings remain deferred. Focused deterministic and bounded headed D3D12 evidence must prove gating, cursor restoration, DVec3 motion, camera/snapshot agreement, and focus discontinuity.
