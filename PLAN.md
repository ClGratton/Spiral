...
- [ ] **Phase 3 input-sampling identity prerequisite:** replace the tail-of-previous-frame GLFW poll with one explicit engine-owned event-poll/sample boundary after timing frame `N` is opened (and any mandatory pre-frame backend wait has completed) but before `Frame.PublishInput` and `InputSimulation`. Retain exact frame ID plus monotonic sample time/QPC tick where available through the immutable frame-input publication, distinguish the sample from task staging, reject duplicate/wrong-order/wrong-frame records, and expose ordered native trace evidence on D3D12/Vulkan. This establishes source identity only; it does not yet publish latency intervals, reinterpret mandatory waits, inject synthetic input, or claim display/click-to-photon timing.
