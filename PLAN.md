...
- [ ] Define and exercise backend-neutral viewport-output recording (color/depth target binding, clear, and the narrow NVRHI-output-to-native-presentation/ImGui handoff) so scene renderers do not require native command-list access; preserve native ownership only for bootstrap, WSI/presentation, and ImGui.
