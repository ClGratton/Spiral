...
- [ ] **Phase 3 source-aligned effective-limiter prerequisite:** classify each completed released `FrameStart[N] - FrameStart[N-1]` interval only from source-qualified control/work/presentation evidence that can occupy it: the `N` InterFrame release or the `N-1` SubmissionGate/CPU/GPU/present record. Retain source frame IDs and frozen candidate/target policy, amend the dependent cadence record when an exact delayed GPU result for `N-1` arrives, and leave warm-up, gaps, eviction, missing, contradictory, or non-exact presentation evidence `Unresolved`. D3D12/Vulkan presentation timing must be immutable per-frame publication rather than a mutable latest-backend snapshot. Preserve target-rebasing evidence separately and never infer display cadence.
