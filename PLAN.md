...
- Render graph construction validates named pass/resource declarations, explicit read/write/read-write state and stage intent, and optional explicit ordering constraints. It derives RAW/WAR/WAW dependency edges, rejects invalid uses and cycles, emits a stable topological pass order, computes logical lifetimes in that resolved order, and produces abstract state-barrier plus dependent cross-queue transition records. Focused `EngineTests` cover this compiler-only contract. The execution core now binds resources, executes callbacks, emits RHI barriers/submissions, and drives the current D3D12/Vulkan Scene viewport through named Clear, Raster, and Output Handoff passes; imported resource shape matching deliberately excludes current state because the executor separately validates the authoritative observed state. `SubmittedRenderGraphFrameOwner` retains up to four production frames, their accepted exact tokens, graph contexts, and captured external payloads without a per-frame CPU wait. Each accepted effective-Graphics pass owns a distinct reset/start/work/end/resolve timestamp transaction; terminal raw results preserve frame/pass/token/generation identity, convert to named durations on one Graphics clock, and amend only the exact retained renderer frame. The Profiler reads the latest completed timing record, and a real ready result promotes `Phase3FrameTimingV1` from the CPU fallback to exercised GPU timestamps. Missing/evicted records are reported unavailable and never attached to another frame. Smoke-only separate renderer-owned outputs retain the former direct recorder as an exact-byte reference oracle; it never publishes to the viewport. Unbound compatible transient resources now use the selected non-aliased pool with lifetime-based same-queue reuse and exact-token retirement.
...
- GPU timestamp query contracts now include the backend-neutral lifecycle/retirement transaction plus qualified D3D12 and Vulkan translations. D3D12 uses native heaps/resolve readback and queue frequency; Vulkan uses a private query pool, graphics-family valid-bit masking, physical timestamp period, and non-waiting exact-token result collection. P3A instruments the real three-pass D3D12/Vulkan viewport RenderGraphs and publishes completion-gated raw endpoints with exact frame/pass/token/generation identity. P3B converts those endpoints into exact-frame named-pass and whole-frame durations, publishes them to Profiler, and promotes the capability path only after a ready result while retaining CPU fallback/unavailable states. P3C amends only a matching retained benchmark frame and derives target-budget GPU headroom only when that exact frame has both a ready duration and a positive effective Smooth target.
...
- The testing strategy requires behavior-oriented assertions, implementation-informed boundary selection, explicit invariants or independent oracles, reproducible/minimized generated failures, speed tiers, sanitizer/coverage honesty, and a complete AI test brief. Phase 3 now contains the shared loop rather than postponing it to Phase 15: `EngineTests` exposes Fast/complete Integration tiers, exact/filter/list selection, durations, budgets, reruns, and JSON; dependency-free generated support supplies stable seeds, traces, boundary weighting, replay, shrinking, and atomic counterexamples; numeric world-grid and stateful RenderGraph properties use independent models; and a checked Scene/shader-package structured corpus plus `EngineFuzzTests` cover valid generation, corruption, truncation, version edits, bit flips, and crossover. The project-owned Linux Clang ASan+UBSan/libFuzzer configuration excludes vendor instrumentation; replacement hosted CI run `29651182554` passed that lane plus Windows D3D12, Ubuntu Vulkan, macOS MoltenVK, portable EngineTests, and style after its predecessor exposed and the follow-up repaired a GCC/Clang-incompatible nested default argument without changing RenderGraph behavior. TSan remains explicitly deferred. Phase 15 retains broad product-validation suites rather than introducing the basic loop after most engine work.
- Renderer capability state now has separate advertised, enabled, implemented, and exercised stages. D3D12 and Vulkan enumerate real devices before creation, query the versioned Phase 3 Bootstrap profile's API, queue/presentation, synchronization, texture-limit, RGBA8, and D32 requirements, and select through the shared deterministic evaluator. Reports retain every candidate evaluation, stable DXGI LUID/Vulkan UUID, rejection, queue/preference fallback, selected formats, and conservative feature lifecycle state. Exact adapter name or stable-ID preference and strict no-fallback launch are supported; unused D3D12 direct-indexing/enhanced-barrier requests and Vulkan buffer-device-address enablement remain off. The editor Profiler presents the active report through a renderer-owned read-only snapshot, including profile, adapter identity, Bootstrap qualification, queues, formats, feature lifecycle states, selected fallbacks, retained candidate rejection reasons, and versioned consumer groups. Both backends now report timestamp queries advertised/enabled/implemented after their focused P2 native smokes. `Phase3FrameTimingV1` still selects and exercises portable CPU steady-clock timing because native-query capability is distinct from the P3 renderer timing consumer. Headed smokes prove that selected fallback at Presentation without upgrading the device beyond Bootstrap. Hosted CI run `29245709930` for scene-snapshot implementation commit `4d16bb8` started no Code Style, Windows, Ubuntu, or macOS job steps; dependency-submission run `29245709977` also started no steps. GitHub reported failed account payments or an Actions spending-limit increase requirement, so these runs provide no hosted evidence. Future consumer groups, dedicated Vulkan queue enablement, physical-device breadth, and Scene/Production qualification remain pending beside their actual consumers.
- The headed presentation prototype does not deliberately discard completed engine frames. Project format 3 now persists a backend-neutral `Synchronized` or opt-in `TearingAllowed` presentation request independently from `Responsive`/`Smooth Frametime`; a session-only `--presentation-policy` override never rewrites the project. D3D12 uses a latency-one waitable flip swapchain and selects either `Present(1, 0)` or, only when factory/window eligibility permits, an allow-tearing chain with `Present(0, DXGI_PRESENT_ALLOW_TEARING)`. Vulkan selects FIFO or supported IMMEDIATE and explicitly rejects MAILBOX/FIFO_RELAXED as hidden replacement modes. Requested policy, capability, actual mode, fallback, semantic synchronization, native generation, effective frame, and last successful-present generation/frame are distinct diagnostics. Unsupported requests resolve once without recreation churn; transitions occur before mandatory backend waits/acquire and preserve ImGui/present across resize. This never proves driver/display VRR active. Smooth pacing remains engine-owned with no RTSS/MSI Afterburner runtime dependency; physical display/VRR-active/input-to-photon evidence and a production winner remain pending.
...
- [x] **Phase 3 presentation-pacing benchmark-capture prerequisite:** renderer-owned bounded immutable snapshots retain raw frame-ID-keyed lifecycle/wait/completion observations and derive nearest-rank p50/p95/p99 plus `1000 / p99` 1%-low and `1000 / p99.9` 0.1%-low start-to-start metrics, deadline-miss count, and p99 overshoot without replacing raw spikes. Schema-7 CSV/JSON artifacts carry backend/adapter/requested and effective policy target/candidate, run/process/path/QPC identity, engine-owned requested presentation policy/capability/actual mode/fallback/native generation/semantic synchronization/tearing state, exact input-stage identity, deadline-wait telemetry, limiter-source identity, and exact-frame GPU status/duration/headroom; VRR/display/replacement/input-to-display remain explicitly unavailable. Responsive and every non-ready/nonpositive-target frame keep GPU headroom unavailable. `EngineTests` and the local RTX 3080 Ti D3D12/Vulkan matrices cover bounded eviction, continuity, raw spikes, aggregation, escaping, lifecycle/wait/completion QPC fields, exact-frame GPU amendment, and all twelve Responsive/InterFrame/SubmissionGate conditions at 60/120 FPS with a declared 30-frame warm-up and exactly 512 retained frames per artifact. This qualifies capture fidelity and engine-measured GPU budget only; it neither claims display/input evidence nor selects a production candidate.
- [x] **Phase 3 external-capture attachment prerequisite — benchmark readiness and Windows QPC identity:** after renderer/window initialization, attachment mode publishes schema-1 readiness with a unique run ID, exact editor PID/canonical executable path, QueryPerformanceCounter frequency/current tick, artifact path, and requested condition metadata, then waits on an exact run/PID supervisor release before the declared 30-frame warm-up and retained capture begin. Schema-2 engine rows carry lifecycle QPC ticks and the released identity. The focused local RTX 3080 Ti D3D12/Vulkan runner proves release ordering/no pre-release artifact, exact identity, 512 retained released rows, stale-run rejection, bounded timeout failure without a capture artifact, and no surviving editor; normal benchmark launches retain the existing ungated behavior. This qualifies only the attachment boundary, not a working external collector, display cadence, or a selected pacing candidate.
- [x] **Phase 3 PresentMon correlation prerequisite — deterministic raw-stream parser and join report:** `Scripts/JoinPresentMonCorrelation.ps1` consumes separate schema-2 through schema-7 engine JSON and exact official PresentMon 1.10.0-header CSV inputs, hashes both stable raw inputs, requires matching exact process ID/QPC domain, retains warm-up/trailing counts, and atomically publishes a stable schema-1 report. It pairs each retained engine `PresentBegin` with exactly one unused causal PresentMon row before the next engine present, with a declared bounded final-row tolerance; the report retains the original PresentMon present/display fields without copying them into engine observations. `Scripts/TestPresentMonCorrelation.ps1` rejects wrong PID, missing/mismatched headers or rows, duplicate/reordered/non-monotonic QPC, missing/ambiguous intervals, noncausal finals, confirms schema-5/schema-6/schema-7 compatibility, and preserves the one-to-one reuse invariant. The ignored real D3D12 capture produces 512 pairs, 16 warm-up rows, zero trailing rows, and positive 118-1323-QPC-tick deltas. This qualifies parser/join determinism only, not live collector supervision, display claims, or a production candidate.
- [x] **Phase 3 selectable editor perspective-viewport navigation prerequisite:** public `Window` cursor capture/restore supports Editor-owned navigation only over the real focused viewport image and only while ImGui is not editing a widget. Navigation capture is a backend-neutral deferred epoch: the press records the visible restore position and pre-arm state, the next update boundary enters disabled cursor mode and arms the post-transition baseline, and pre-transition/synchronous transition motion cannot accumulate a navigation delta; release or focus loss cancels pre-arm capture without changing cursor mode, while an armed capture restores the exact visible position. The workspace-global Engine Settings preset defaults and fails closed to Fusion: the wheel is cursor-anchored bounded multiplicative zoom on the image-rect/FOV/aspect ray and persistent pivot-depth plane; MMB canvas-pans camera and pivot together in camera right/up at FOV/depth/viewport-derived scale; and Shift+MMB rotates the actual camera-to-pivot offset between old/new upright camera bases, preserving distance without assuming the camera is looking through the pivot after an off-center zoom. The persistent default pivot is a deterministic visible-mesh-transform AABB-center approximation (not unavailable geometry bounds), with an explicit camera-forward fallback; F alone replaces it with the selected entity origin. LMB/RMB, fly keys, and RMB-wheel speed control do not navigate in Fusion. Unreal remains the alternate preset with its prior plain LMB move/yaw, RMB look, LMB+RMB/MMB pan, wheel increments, RMB+wheel bounded speed, and RMB+W/S/A/D/E/Q plus arrow/Page aliases fly. Autodesk sources establish the Fusion shortcut/pivot intent; the cursor ray, transform-extent fallback, no-crossing clamp, offset-basis rotation, and deferred capture epoch are the engine implementation approximation. F is not bounds-aware Fit and MMB-double-click Fit remains deferred until real bounds/picking exist. Navigation is never serialized into `.spiralproject`; it uses the documented transactional workspace-global editor settings file. Movement uses the Application timestep and double-precision world coordinates, writes the active Scene main camera through public Scene APIs, preserves canonical sector/local authority, and marks focus relocation discontinuous before immutable snapshot publication. Picking/multi-select, bounds framing/Zoom to Fit, gizmos, orthographic modes, camera piloting, and configurable bindings remain deferred. `ViewportNavigationSmokeV5` provides the focused deterministic settings, callback-order, camera/pivot, cursor-restore, DVec3, snapshot, and discontinuity evidence; on 2026-07-16 the user completed the bounded real Windows D3D12 Fusion/Unreal movement and cursor-capture interaction gate over the focused viewport without reporting a failure.
- [x] **Phase 3 production-pacing measurement prerequisite — Windows PresentMon correlation:** `Scripts/CapturePresentMonCorrelation.ps1` validates an explicit PresentMon 1.10.0 path/hash, launches the benchmark editor into its exact PID/path/run/QPC readiness gate, starts a unique exact-PID/QPC ETW session, requires a 1500 ms ready-and-alive settle before release, and retains separate stable engine/collector raw streams plus a deterministic fail-closed join report and atomic receipt. Independent bounded jobs clean every owned process; an exact-name teardown removes only the invocation's own ETW session when necessary, and cleanup failure is not reported as success. The fake-process matrix covers ordering, identity, collision/version/header/liveness/timeouts, join suppression, exact-session teardown success/failure/survival, and descendants. The final non-elevated local D3D12 run used PresentMon 1.10.0, retained 512 engine frames and 541 exact-PID collector rows, joined 512 pairs with 29 warm-up and zero trailing rows at 158-1926 QPC ticks, preserved the collector hash through cleanup, exited both children with code 0, and verified the unique ETW session absent. The receipt keeps monitor/VRR unknown and RTSS/FES/input/GPU-headroom unavailable. This qualifies external present/display correlation only; it neither mutates RTSS/driver state nor selects a production candidate.
- [x] **Phase 3 GPU timing prerequisite P1 — backend-neutral query lifecycle:** `TimestampQueryPool` is an engine-owned `Engine::RHI` state machine with validated timestamp type/nonzero bounded count, exact-device ownership, overflow-safe index/range checks, and reset/write/resolve ordering. A failed operation poisons its recording, failed recording/submission publishes nothing, an accepted exact completion token publishes one generation as a nonblocking not-ready result, completion publishes ready values or truthful disjoint status, stale/duplicate-token/duplicate-completion reads or operations reject, reuse is blocked while the generation is unresolved, and history retains at most four generations. One logical pool is caller-serialized with one unresolved generation; parallel command contexts use distinct bounded pools. The zero-warning Debug build and 65/65 `EngineTests` pass creation, ownership, invalid-order/rollback, accepted publication, no-CPU-wait reuse, stale reads, cross-pool token validity, same-pool token rejection, ready/disjoint completion, and bounded history. D3D12/Vulkan query creation and command methods remain explicitly unavailable, so this checks contract evidence only—not native timestamps, renderer timing, or GPU headroom.
- [x] **Phase 3 GPU timing prerequisite P2A1 — native-query recording transaction and post-submit retirement:** `TimestampQueryTransaction` advances the P1 logical operation before invoking a backend callback, poisons native callback failure, reserves bounded retirement capacity before submit, abandons a fully valid recording without publication when submission fails, and publishes the P1 generation plus opaque backend-private shared state only for an accepted exact-device completion token. The device-owned retirement queue retains that state after accepted publication and public-pool destruction, rejects duplicate-state/cross-device/stale/incomplete retirement and reuse, releases only on the exact terminal token, and caps reserved plus unresolved records at four without waiting. The zero-warning Debug build and 66/66 `EngineTests` prove validation-before-callback, native/submit rollback, exact-token publication, completion-gated result/state reuse, post-submit destruction retention, and bounded retirement. Both backend query paths and timestamp capabilities remain unavailable. This does not yet retain the logical pool through pre-submit wrapper destruction or preflight a complete publication before native submit; P2A2 owns those remaining seams.
- [x] **Phase 3 GPU timing prerequisite P2A2 — pre-submit ownership and publication preflight:** the transaction and its recording share the logical pool state, and the retirement queue retains that pool plus backend-private state, so destroying the public wrapper before submit or completion cannot dangle either authority. `PrepareForSubmit` non-mutatingly validates a complete reset/write/resolve set, current generation, and exact pool/state reservation, then freezes further recording before any native-submit callback. Abandoned prepared work publishes nothing and releases its reservation. Publication separately rejects invalid/cross-device and same-pool duplicate tokens while accepting one submission token across distinct pools; per-state completion supports different result sets for those pools, and token retirement rejects until every matching generation is terminal. The final zero-warning Debug build, 66/66 `EngineTests`, style, and diff checks prove pre-submit destruction, preflight rejection/freeze, failed-submit rollback, cross-pool token validity, same-pool duplicate rejection, terminal completion, and bounded release. D3D12/Vulkan query paths and capabilities remain unavailable, so this is shared ownership/preflight evidence only.
- [x] **Phase 3 GPU timing prerequisite P2B — D3D12 native query translation:** the private D3D12 RHI bridge owns timestamp query heaps and readback buffers, exposes the selected graphics queue scale as nanoseconds per tick, translates reset/write/resolve only through an accepted P2A transaction, publishes native state only with the exact submission token, and maps/completes/retire results only after that token's fence is terminal. Unsupported/foreign pools and non-graphics command lists reject; readback/map failure terminates the logical generation as disjoint/failed rather than fabricating a duration. The device capability is advertised/enabled/implemented only with a nonzero selected-graphics-queue frequency, while `Phase3FrameTimingV1` deliberately remains on CPU timing until P3 implements the renderer consumer. The zero-warning Debug build, 66/66 deterministic tests, and bounded local RTX 3080 Ti `Scripts/TestD3D12Timestamps.ps1` run prove native allocation, positive timestamp period, write/resolve, pre-completion not-ready publication, exact-token readback, completion-gated reuse, and public-wrapper destruction retention. This is D3D12 native RHI evidence only, not frame/pass instrumentation or GPU headroom.
- [x] **Phase 3 GPU timing prerequisite P2C — Vulkan native query translation:** the private Vulkan RHI wrapper owns a timestamp `VkQueryPool` beside the existing NVRHI-created device, obtains the live NVRHI command buffer only while recording, emits core command-buffer reset/write operations on the selected effective graphics queue, and treats resolve as the P1 publication boundary because Vulkan reads query pools directly. Exact NVRHI submission completion gates `vkGetQueryPoolResults` without `WAIT`; `VK_NOT_READY` remains nonterminal, errors publish disjoint/failure, and successful 64-bit values are masked to the graphics family's `timestampValidBits` before exact-token retirement. Capability usability requires nonzero valid bits and physical `timestampPeriod`; `Phase3FrameTimingV1` remains on CPU until P3. The zero-warning Debug build, 66/66 tests, and bounded local RTX 3080 Ti `Scripts/TestVulkanTimestamps.ps1` run prove allocation, positive period, write/resolve, pre-completion not-ready publication, readback, reuse, destruction retention, clean exit, and no surviving Editor. This is Vulkan native RHI evidence only, not frame/pass instrumentation or GPU headroom.
- [x] **Phase 3 GPU timing prerequisite P2D — asynchronous production RenderGraph frame retirement boundary:** `SubmittedRenderGraphFrameOwner` retains at most four submitted production frames with their authoritative application frame ID, heap-owned graph/recording contexts, stable accepted-prefix pass labels, every accepted exact completion token, and type-erased shared payloads for pass-captured resources outside the graph. Polling is nonblocking; only frames whose complete token set is terminal are released, invalid/failed tokens remain retained and fail truthfully, and duplicate frame/token state or capacity exhaustion rejects without waiting or dropping accepted work. D3D12 retains mapped frame-slot constants and rejects reuse while their prior frame is unretired; Vulkan retains its per-frame constant buffers and defers output replacement while an exact graph token is incomplete. Both keep the final Graphics output handoff ordered before the existing native presentation path and add no raw API, device, or queue. The final zero-warning Debug build and 67/67 `EngineTests` prove graph/payload lifetime, exact-token/pass-label identity, accepted-prefix failure retention, bounded capacity, and zero wait calls. Local RTX 3080 Ti `TestRender.ps1` and `TestVulkan.ps1` pass consecutive production frames with `ProductionRenderGraphRetirementV1 ... passes=3 cpuWaitBetween=no`; shutdown alone establishes device idle before releasing any final unretired owner state. This prerequisite adds no timestamp scope or duration; P3 consumes the boundary.
- [x] **Phase 3 GPU timing prerequisite P3A — RenderGraph timestamp scopes and bounded asynchronous query-slot retirement:** each accepted effective-Graphics production pass owns a distinct two-query pool/transaction ordered as reset, start timestamp, graph barriers/acquires and pass work, releases, end timestamp, and resolve in that pass's command list. The retained P2D frame owns the pool/generation beside its authoritative frame ID, ordered label, and exact token until nonblocking terminal collection; invalid/failed tokens publish truthful terminal raw status while remaining retained under the P2D failure contract. The RHI native-retirement bound is 12 states for four three-pass frames, and unsupported independent queues reject before recording. The final zero-warning Debug build and 68/68 `EngineTests` prove caller/worker operation order, exact identities/generations, accepted-prefix disjoint failure, 12-state capacity, cross-queue rejection, and zero wait calls. Local RTX 3080 Ti `TestRender.ps1` and `TestVulkan.ps1` require multiple `RenderGraphTimestampScopesV1 ... scopes=3 raw=ready cpuWaitBetween=no result=pass` retirements while retaining the exact-byte comparator. This publishes raw completion-gated endpoints only; P3B/P3C still own duration/Profiler/capability/benchmark/headroom claims.
- [x] **Phase 3 GPU timing prerequisite P3B — Renderer/Profiler asynchronous GPU duration publication and CPU fallback:** retired P3A scopes are converted only when every scope preserves one exact frame, unique pass label, valid token, positive finite/equal period, matching start/end generation, and effective Graphics queue. Named-pass durations use their own end-minus-start ticks; whole-frame GPU time is final-pass end minus first-pass start and is never a sum or a cross-queue inference. A bounded renderer history is amended only by exact frame ID; unavailable/evicted records are ignored truthfully rather than attached to the current frame, and nonterminal/unavailable/disjoint states plus CPU fallback remain explicit. The Profiler reads the latest completed timing frame, and `Phase3FrameTimingV1` promotes to exercised `GpuTimestamps` only after a real ready publication. The shared timestamp-retirement queue is now mutex-serialized because the three graph recording workers reserve native query state concurrently. The final zero-warning Debug build, 70/70 `EngineTests`, and clean local RTX 3080 Ti D3D12/Vulkan headed scripts prove duration math, identity/status rejection, concurrent 12-slot reservation, multiple real ready publications, capability promotion, and zero surviving Editors. This does not amend benchmark artifacts or compute headroom; P3C owns that work.
- [x] **Phase 3 GPU timing prerequisite P3C — matching-frame benchmark GPU headroom:** `FramePacingBenchmarkCapture` amends only a retained frame whose ID exactly matches the P3B publication, republishes its immutable snapshot, and derives headroom in milliseconds only as `1000 / exact-frame effective Smooth target FPS - exact-frame ready GPU interval`. Schema 3 added explicit GPU timing status, duration, and headroom to stable CSV/JSON; Responsive frames, stale/evicted/mismatched IDs, non-ready/disjoint results, invalid durations, and missing/nonpositive targets retain unavailable duration/headroom as applicable. PresentMon schema-2/3 inputs remain accepted, while the later source-alignment prerequisite extends new live artifacts to schema 4. The final zero-warning Debug build and 70/70 `EngineTests` prove exact amendment, eviction/mismatch rejection, nonpositive-target and status behavior, formula, and export fixtures. Full local RTX 3080 Ti D3D12/Vulkan 60/120 FPS Responsive/InterFrame/SubmissionGate matrices retained 512 frames per condition, at least one ready GPU duration per artifact, exact formula checks for every eligible row, unavailable Responsive headroom, and zero surviving Editors; focused PresentMon parser/supervisor regressions also passed. This adds no display/input evidence and does not select a pacing behavior.
- [x] **Phase 3 test-loop prerequisite T1 — selectable tiers and measured feedback:** `EngineTests` classifies 60 in-memory Fast cases and the complete 75-case Integration registry without changing case semantics; exact/filter/list selection retains deterministic order, each result and total publish measured duration, Fast/Integration default budgets are 60/300 seconds, failures print exact reruns, invalid/zero/conflicting selectors fail, and schema-1 JSON preserves selection/tier/order/status/budget/seed. Both platform scripts exercise the already-built executable; local MSVC Debug passed the zero-warning build and `EngineTestRunnerV1 result=pass total=75 fast=60 integration=15`, while CI uses the explicit Integration tier.
- [x] **Phase 3 test-loop prerequisite T2 — reproducible generated/property testing:** dependency-free `GeneratedTest.h` supplies the stable default or caller seed, boundary-weighted choices, serialized traces, exact replay, chunk/value minimization, and atomically published counterexample JSON with original/minimized traces and rerun command. The Fast tier runs 512-case world-grid normalization against an independent centered-half-open carry model and 512 stateful RenderGraph access sequences against an independent RAW/WAR/WAW dependency model; an Integration case proves failure-artifact contents. Local MSVC Debug and the runner contract passed seed-42 exact replay plus malformed/non-exact replay rejection.
- [x] **Phase 3 test-loop prerequisite T3 — structured fuzz replay and sanitizer lane:** checked `Tests/Corpus/Fuzz` cases and the complete Integration tier replay structure-producing Scene and portable-shader inputs through valid generation, field corruption, truncation, version edits, bit flips, and crossover; rejected loads preserve caller-owned destinations and accepted loads must pass canonical/semantic validation. `EngineFuzzTests` adds deterministic bounded campaigns and the same structure-aware libFuzzer entry point; ignored raw failure inputs/manifests retain exact reruns, and `Docs/VERIFICATION.md` records replay and libFuzzer minimization commands. Linux Clang ASan+UBSan instruments only project Engine/test/fuzz code, links explicitly uninstrumented vendor libraries, enables symbolized fail-fast/leak diagnostics, and runs the full Integration tier plus 512 libFuzzer inputs. Local MSVC passed 75/75 through the runner and a 256-input seed-42 campaign; hosted run `29651182554` passed the sanitizer/fuzz lane and the complete Windows/Ubuntu/macOS matrix. TSan is explicitly deferred until its concurrency/toolchain/suppression boundary is independently qualified; no random-byte-only, coverage-percentage, race-freedom, vendor-memory-safety, backend, or device claim is inferred.
- [x] **Phase 3 Smooth target application prerequisite:** each experimental control point retains its last actual release, target, and next deadline. Re-publishing an identical policy preserves phase; a target or candidate change invalidates the old deadline, rebases from the last valid release at the next applicable control point, waits there when the new deadline is still ahead, or truthfully records a late frame and rebases from observation. The per-frame resolved-policy snapshot prevents a mid-frame edit from applying both candidates. The zero-warning Debug build and 75/75 tests prove faster/slower target changes, candidate switch/back, Responsive reset, and identical-policy phase retention. Local RTX 3080 Ti D3D12/Vulkan same-process 5-to-10 and 5-to-1000 FPS runs prove the new target reaches the next InterFrame boundary and remains active; the original 5 FPS native control-point smokes remain. Hosted WARP reproduced the live 5-to-10 change but exposed that the separate effective-limiter label was not frame-source aligned; that category claim moves to the following prerequisite without withdrawing target application evidence.
- [x] **Phase 3 source-aligned effective-limiter prerequisite:** each completed `FrameStart[N] - FrameStart[N-1]` interval retains its predecessor and is classified only from the applied InterFrame release/frozen policy on `N` or SubmissionGate/CPU/GPU/exact-present evidence and frozen policy on `N-1`. Every qualified result publishes its source frame; first/gapped/evicted/missing evidence, non-exact presentation, and multiple qualifying sources remain `Unresolved`. Delayed exact GPU `N-1` reclassifies only retained cadence `N`, including benchmark schema-4 amendment; D3D12/Vulkan present attempts publish immutable application-frame identity, and Vulkan clears prior success before a new attempt. A relative/bounded requested-cadence band admits measured low-rate native scheduling jitter only when the exact pacing wait is the sole source. The zero-warning Debug build, 75/75 tests, schema benchmark, and bounded local RTX 3080 Ti D3D12/Vulkan target-change matrices prove both pacing candidates, current-work non-borrowing, conflicts, identity mismatch, 110.84/100 ms low-rate classification, delayed GPU amendment, source-frame telemetry, and cleanup. This is engine/present-source diagnosis only; display cadence, click-to-photon latency, and production candidate selection remain unclaimed.
- [x] **Phase 3 input-sampling identity prerequisite:** `Window::PollEvents` now runs exactly once for an active frame after renderer timing frame `N` opens and D3D12's mandatory pre-frame wait completes, but before immutable `Frame.PublishInput` and `InputSimulation`; minimized windows still poll without fabricating a renderer sample. The returned `RendererInputSample` carries exact frame ID, finite nonnegative steady-clock offset, and QPC tick when attachment timing is active through the immutable frame input and lifecycle trace. A pure atomic validator rejects duplicate, wrong-frame, missing-start, after-simulation, invalid, and regressing samples without mutation. The zero-warning Debug build, 75/75 tests, code style, D3D12/Vulkan headed lifecycle smokes, and both 512-frame 60 FPS schema-4 benchmark matrices prove `FrameStart -> InputSample -> InputSimulation -> RenderSubmission -> Present` ordering while mandatory waits and input/display status remain separate/unavailable. This establishes source identity only; it publishes no latency interval, input injection, input-to-display, or click-to-photon claim.
- [x] **Phase 3 pacing latency-source prerequisite:** the exact `RendererInputSample` frame is preserved through simulation, native submit, `Present`, and later GPU-completion identity. A failure-atomic pure resolver publishes finite nonnegative same-frame input-to-simulation, input-to-submit, and input-to-`PresentEnd` intervals progressively and rejects missing/duplicate/wrong-order/wrong-frame evidence without borrowing another record. Profiler, native telemetry, and schema-5+ CSV/JSON expose the source frame and three ordered engine-stage intervals; input-to-display, click-to-photon, and generic end-to-end input latency remain explicitly unavailable. PresentMon join/supervision accepts schemas 2-6 without treating these or later deadline-wait fields as display evidence. The zero-warning Debug build, 75/75 tests, code style, schema-5 parser regression, D3D12/Vulkan headed smokes, and both 512-frame 60 FPS benchmark matrices prove exact source/order/formulas and mandatory-wait separation. This does not measure peripheral, GPU-completion, compositor, display, or photon latency.
- [x] **Phase 3 external reference-condition prerequisite — RTSS `ASYNC` protocol:** the 2026-07-18 read-only audit established that installed `RTSS.exe` 7.3.5.28001 (valid Authenticode signer Alexey Nicolaychuk; SHA-256 `786D9C786C085D4BF14C6E2F08AB1779E01741CEEEB367F8C11DB4AB99F9F68D`; no RTSS process during audit) exposes no documented non-UI transactional encoding for exact-Editor `ASYNC`. Its bundled official SDK profile interface exports `Enum`/`Load`/`Get`/`Set`/`Save`/`Update` and documents `FramerateLimit`, but no ASYNC/limiter-mode selector; bundled ReadMe page 6 distinguishes the default async limiter, FES/back-edge, and Reflex (with Reflex fallback to default async) without documenting a property encoding. `Save`/`Update` are not documented as atomic/crash-recoverable and `UpdateProfiles` reloads all running 3D apps; shared memory is telemetry, not control. Therefore RTSS `ASYNC` is unavailable to automation: do not guess config formats or use UI automation. The bounded manual gate requires an operator to identify the exact `Editor.exe` path/profile and RTSS version/hash; capture before state for limiter mode, target, FES/scanline/frame-generation, and global/profile scope; change only that exact Editor profile in RTSS UI to the requested async condition; capture after state; run one bounded benchmark; immediately restore the exact prior values even after failure/timeout; capture restored state; and sign a receipt. Global/other-profile/driver/FES/frame-generation implicit mutation is prohibited. This closes only the protocol/manual-gate prerequisite; it is not evidence that RTSS ASYNC ran, improved results, or won the production Smooth selection.
- [x] **Phase 3 production Smooth PresentMon collector-finalization prerequisite:** after the exact released Editor exits successfully, the supervisor now requires the owned exact-PID PresentMon collector to exit naturally with code zero, leave no owned descendants, and relinquish its exact named ETW session/output before any raw CSV read, stability check, hash, header validation, or join. A lingering, nonzero, timed-out, descendant-surviving, or session-surviving collector fails closed with no report/join and a cleanup-verified receipt. Deterministic fake coverage holds the CSV with exclusive sharing after Editor exit and proves finalization timeout, null hashes/report, `joined=false`, and verified cleanup; two consecutive complete supervisor matrices plus the parser, bounded-process, code-style, PowerShell-parse, and diff gates passed. Exact PID/QPC identity, immutable raw/hash checks, and one-to-one join requirements remain unchanged. This fixes capture reliability only; it is not new native pacing/display evidence.
- [x] **Phase 3 production Smooth deadline-precision prerequisite:** the production `SystemFramePacingClock` now delegates only monotonic deadline waiting to Platform. Windows owns one unnamed non-inheritable least-privilege `CREATE_WAITABLE_TIMER_HIGH_RESOLUTION` handle per clock, re-arms after an early wake, waits to a reported at-most-0.5 ms final-tail boundary, and reports timer/portable wait, active-tail budget/actual, fallback reason, process-CPU/wall proxy, requested deadline, actual release, and overshoot in schema-6 artifacts. Unsupported creation or arm/wait failure falls back visibly to portable `steady_clock` waiting; Renderer policy, candidate control points, phase anchoring, mandatory backend waits, and frame retention remain unchanged. Deterministic early-wake/failure/cleanup coverage, the zero-warning Debug build, 76/76 tests, parser/supervisor matrices, and local RTX 3080 Ti D3D12/Vulkan 60/120 Responsive/InterFrame/SubmissionGate matrices passed. All applied Windows waits identified the high-resolution timer with zero fallback and tail budgets no greater than 0.5 ms; D3D12 waited-release overshoot fell from the prior roughly 14.3 ms p50/15.5 ms p99 to 0.0010-0.0014 ms p50 and 0.0195-0.3891 ms p99 across the four waited conditions. At 120 FPS the workload often already exceeded the 8.33 ms budget, so missed target cadence remains truthful workload evidence rather than a timer-precision failure. This establishes measured deadline behavior only, not display cadence or a production winner.
- [x] **Phase 3 production Smooth automated native-evidence prerequisite:** ignored bundle `output/production-smooth-schema6-20260718` freezes all twelve schema-6 60/120 FPS Responsive/InterFrame/SubmissionGate D3D12/Vulkan engine matrices and separate condition artifacts. Six fresh exact-PID D3D12 PresentMon 1.10.0 captures each retained 512/512 one-to-one pairs, 29 warm-up rows, zero trailing rows and drops, immutable raw/report hashes, natural zero exits, and verified process/session cleanup. D3D12 60 FPS InterFrame produced engine/present/display-change p50 of 16.668/16.629/16.667 ms and p99 of 16.718/18.831/22.240 ms; 120 FPS InterFrame remained workload-limited at 11.160/11.208/11.111 ms p50. SubmissionGate retained large 51.706 ms (60 FPS) and 96.296 ms (120 FPS) engine p99 spikes rather than hiding them. The Vulkan 60-responsive attachment attempt failed closed with no report/join when frame 31 had two causal collector candidates; both children still exited zero naturally, raw hashes were retained, and cleanup was verified. This provisional bundle establishes exact automated engine/PresentMon evidence only; physical display cadence, monitor/VRR state, input/photon latency, RTSS execution/configuration, and a production winner remain unclaimed.
- [x] **Phase 3 production Smooth external/reference manual-evidence prerequisite:** the operator completed and signed the exact-Editor RTSS 7.3.5 `ASYNC` gate at 60 and 120 FPS. Captures prove the initial/after-each-run restored state contained no Editor profile; global limiter mode `Async`, passive waiting on, benchmark mode off, and RTSS frame-start statistics remained unchanged. Each temporary exact `Editor.exe` profile used Low detection, the named limit, Scanline Sync `0`, OSD/statistics off, and no stealth/custom-D3D mode; FES, Reflex, frame generation, Global, other profiles, and driver state were not changed. Four bounded Responsive D3D12/Vulkan PresentMon runs each retained/joined 512 frames with 29 warm-up, zero trailing rows, natural zero exits, stable raw hashes, and verified cleanup. D3D12 engine p50/p95/p99 was 16.684/18.063/19.314 ms at 60 and 11.402/17.487/25.615 ms at 120; Vulkan retained 16.681/42.491/63.161 ms and 9.047/109.028/125.351 ms, so the external reference's tail spikes remain visible. Ignored bundle `output/production-smooth-rtss-async-passive-20260718` contains the raw captures and screenshot-hashed receipt `eaa0e1d10ca39a540d89af9cbc89a086941ae4a696c4ac388c64de672f85eb5e`. This qualifies only the signed RTSS Async/passive-waiting reference condition; it makes no physical-display, VRR, input-to-photon, or production-selection claim.
- [x] **Phase 3 production Smooth end-to-end input/physical-display evidence decision prerequisite:** selection evidence is now admitted by source class, not inferred from engine timestamps: PresentMon/native present timing is accepted only for OS/presentation scheduling correlation; physical-display and input evidence requires an optical hardware path (LDAT or an equivalent photodiode/high-speed camera) paired with a deterministic engine-owned high-contrast response marker and exact frame/input identity. A VRR condition additionally requires a receipt of the exact monitor/connection, OS refresh configuration, driver/display state, engine requested and actual presentation capability/mode, and optical display-cadence capture; API capability alone does not prove active VRR. No qualifying hardware/control path is currently available, so those values remain unavailable and production selection remains blocked. Shipped `Smooth Frametime` is engine-owned and must have no RTSS/MSI Afterburner runtime, configuration, profile, or shared-memory dependency; those tools remain optional lab/reference conditions only. The current engine `InterFrame` control point plus Platform deadline clock already supplies the internal-cap mechanism, but no production winner is selected and no renderer behavior changed. This decision records the source/primary-document audit only; the two following prerequisites were exposed before the existing comparison line.
- [x] **Phase 3 production Smooth presentation-policy prerequisite:** project format 3 persists a backend-neutral default `Synchronized` or opt-in `TearingAllowed` request, migrates v1/v2 to synchronized, rejects malformed values transactionally, and keeps the command-line override session-only. D3D12 preserves the latency-one waitable flip chain and applies policy replacement before that frame's mandatory wait: synchronized uses `Present(1, 0)`, while factory-supported/window-eligible tearing uses an allow-tearing chain plus `Present(0, DXGI_PRESENT_ALLOW_TEARING)`; exclusive-fullscreen tearing is not claimed. Vulkan resolves FIFO or supported IMMEDIATE before acquire/fence/submit and never substitutes MAILBOX/FIFO_RELAXED. Unsupported requests commit their fallback once without per-frame recreation. Both backends publish requested/capability/actual/fallback/semantic-sync/tearing/native-generation/effective-frame plus the matching last successful-present generation/frame, restore prior valid presentation or fail fatally, and preserve resize/ImGui/no-drop/frame identity. Schema-7 benchmark/readiness and PresentMon supervision consume only these engine-owned conditions and keep VRR active unavailable. Local zero-warning Debug build, 78/78 `EngineTests`, deterministic PresentMon join/supervisor tests, 60/120 FPS twelve-condition schema-7 D3D12/Vulkan matrices, `TestRender.ps1`, `TestVulkan.ps1`, and code style passed on the RTX 3080 Ti. This exposes policy and actual mode; it does not claim the engine can force or observe driver/display VRR.
- [ ] **Phase 3 production Smooth optical/input-correlation prerequisite:** add an engine-owned deterministic high-contrast response marker and exact input/frame identity, plus a bounded receipt and correlation path for LDAT or an equivalent photodiode/high-speed-camera optical measurement. The receipt must bind device/method/calibration, marker/input/frame identities, monitor/connection/OS refresh and driver/display state, requested/actual presentation mode, raw optical observations, timeout/failure cleanup, and unavailable fields. It must not use PresentMon, engine `PresentEnd`, API capability, or an overlay as a surrogate for panel cadence, VRR activation, peripheral input, or click-to-photon latency. This prerequisite is measurement instrumentation only: it adds no Reflex, FES, RTSS, Afterburner, driver mutation, or default-pacing behavior.
- [ ] Measure, compare, and select the production `Smooth Frametime` behavior at maintainable target cadences through DXGI waitable-swapchain and Vulkan presentation paths, after the preceding selectable-presentation and optical/input prerequisites have evidence. Compare the engine inter-frame candidate, the separately instrumented submission gate, external RTSS `ASYNC`, and any explicitly named FES/frame-generation experiment without silently substituting a delay immediately before the current `Present`. Preserve mandatory acquire/fence correctness waits and verify Responsive/each candidate with shared frame IDs, p50/p95/p99 start-to-start/display cadence, 1%/0.1% lows, CPU active versus intentional wait, deadline misses/overshoot, render submit, `Present`, GPU-complete, display/replacement/drop where available, GPU headroom, and the admitted input-latency path. Do not select a default from the current 5 FPS control-point smoke, engine/overlay graph, PresentMon alone, or a VRR capability bit alone.
