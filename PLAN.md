...
- [ ] **Phase 3 GPU timing prerequisite P2A — native-query transaction and retirement seam:** connect an accepted P1 recording to backend-private native state without exposing native handles above `Engine::RHI`: validate the logical operation before native mutation, publish no generation on failed recording/submission, publish the exact submission token only after accepted submit, retain shared native allocation/result state after public-pool destruction until every referencing command list/submission retires, and reject reuse/destruction races without a CPU wait. Deterministic fake-native tests must prove rollback, exact-token publication, public-wrapper destruction before completion, completion-gated result publication/reuse, and bounded retirement. This is shared lifetime/transaction evidence only; capability remains unusable and no D3D12/Vulkan timestamp claim is made.
