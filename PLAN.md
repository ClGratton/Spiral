...
- Vulkan bootstrap owns the native instance/device/queue/surface and wraps the resulting `nvrhi::DeviceHandle`. The first Vulkan RHI foundation is deliberately resource/clear/readback-only: NVRHI-backed `Engine::RHI` buffers, RGBA8/depth textures, output clear, explicit transitions, command recording/submission, deterministic buffer update, staging readback, debug markers, and GPU-safe retirement must be exercised through deterministic offscreen clear/readback. It neither consumes SPIR-V nor creates a graphics pipeline/draw, Scene viewport, or ImGui texture bridge. Those are separately ordered next; Vulkan must not copy the D3D12 native-command workaround.
...
- [ ] NVRHI Vulkan `Engine::RHI::Device` core after real device creation: buffers, RGBA8 color/depth textures, output clear, explicit transitions, command begin/end/submission, deterministic buffer upload/update, staging texture readback, debug names/markers, and GPU-safe retirement. Exercise deterministic offscreen clear/readback through NVRHI and verify dimensions, row layout, sampled pixels. Local Windows `TestVulkan.ps1` evidence is local only; replacement exact-head hosted runtime evidence must come from the Linux lavapipe and macOS Apple-Paravirtual/MoltenVK Vulkan smoke jobs (the hosted Windows job is D3D12-only). No shader/pipeline/draw, Scene, or ImGui claim.
- [ ] SPIR-V shader consumption, reflected bindings/input layout, graphics pipeline/framebuffer state, constant-buffer update, and deterministic indexed offscreen draw/readback using the exercised Vulkan RHI core primitives. No Scene snapshot or ImGui output handoff claim.
- [ ] Vulkan scene viewport integration using the exercised Vulkan RHI primitives, including Scene snapshot consumption and the narrow completed-NVRHI-output-to-native-presentation/ImGui handoff; keep raw Vulkan confined to bootstrap, WSI/presentation, and ImGui.
