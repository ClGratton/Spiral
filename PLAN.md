...
- [x] **Phase 3 presentation-pacing benchmark-capture prerequisite:** renderer-owned bounded immutable snapshots retain raw frame-ID-keyed lifecycle/wait/completion observations and derive nearest-rank p50/p95/p99 plus `1000 / p99` 1%-low and `1000 / p99.9` 0.1%-low start-to-start metrics, deadline-miss count, and p99 overshoot without replacing raw spikes. Schema-2 CSV/JSON artifacts carry backend/adapter/requested and effective policy target/candidate, run/process/path/QPC identity, and runner-supplied presentation/sync/VRR/tearing condition strings, whose honest default is `unknown`; display, replacement/drop, input, and GPU-headroom remain explicit unavailable fields. `EngineTests` (64/64) cover bounded eviction, continuity, raw spikes, aggregation, p99.9, escaping, lifecycle/wait/completion QPC fields, and attachment readiness/release validation. Local RTX 3080 Ti D3D12/Vulkan runs passed all twelve Responsive/InterFrame/SubmissionGate conditions at 60/120 FPS with a declared 30-frame warm-up and exactly 512 retained frames per artifact. This qualifies capture fidelity only; it neither claims display/input evidence nor selects a production candidate.
...
- [ ] **Phase 3 GPU timing prerequisite P3C — matching-frame benchmark GPU headroom:** extend benchmark retention and schema so a ready P3B GPU duration amends only its exact retained frame, and publish headroom only as exact effective target budget minus that matching-frame GPU interval. Stale, evicted, unavailable, pending, disjoint, nonpositive-target, or mismatched results remain explicitly unavailable. Add deterministic CSV/JSON fixtures and bounded D3D12/Vulkan pacing captures. P3A-P3C remain mandatory before production pacing selection and Phase 3E Forward+/clustered lighting, PBR, lights, shadows, or sky/atmosphere.
