...
- [ ] **Phase 3 production Smooth presentation-policy prerequisite:** introduce backend-neutral selection and actual-mode diagnostics for synchronized versus VRR-compatible/tearing presentation, preserving separate capability, requested policy, and actual selected mode. D3D12's current `Present(1, 0)`/waitable-swapchain and Vulkan's current FIFO mode are fixed bootstrap choices, not proof of a selectable VRR path. A future D3D12 VRR-compatible mode must follow the documented flip-model/allow-tearing creation and `Present(0, DXGI_PRESENT_ALLOW_TEARING)` constraints when capability and window state permit; Vulkan must expose its own supported present-mode policy without treating mailbox/immediate replacement as hidden smoothing. The application must not claim it can force driver/display VRR. This prerequisite must retain mandatory acquire/fence/latency waits, no-drop behavior, and exact frame identity, and publish unsupported/fallback/actual-mode diagnostics before any candidate comparison uses a VRR condition.
