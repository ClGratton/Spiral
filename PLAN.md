...
- Scene format version 4 makes canonical signed-sector/local position the private writable `Transform` authority and persists the immutable Scene `WorldGridPolicy`; it writes no duplicate `MainCamera.Transform` and migrates version 1-3 absolute-double scenes through the default policy with selected-entity camera-transform precedence. Exact policy/sector-local persistence, huge sectors, precision-safe single-axis mutation, legacy migration/default policy/entity-main-camera precedence, and invalid/noncanonical rejection with an unchanged destination are covered by Windows/MSVC and MinGW Debug `EngineTests`; real Editor headless save/reload, scene-authoring save/reopen, and undo/redo smokes also pass on the MSVC build. Each immutable snapshot epoch carries the complete stable-ID editor-viewport `CameraView`, including double world position, translated origin, origin sector, and temporal-history invalidation. The view publisher owns its tracker state (implemented in the Scene module), keeping sector-snapped state independent per stable view ID, retaining each axis through the configured hysteresis band, selecting direct destination sectors on teleport, and leaving exact-camera translation as the default. Equal and adjacent extreme signed-sector comparisons preserve local hysteresis detail through derived local-boundary tests without double-precision sector arithmetic; non-adjacent sector changes rebase directly. Known Editor scene replacement/load/restore and discontinuous camera-state synchronization set a one-shot flag that is consumed only by a valid published view. Windows/MSVC `EngineTests` cover exact hysteresis boundaries, no-flap, teleports, invalid-request transactional state, extreme signed-sector adjacency, independent retained view epochs, and exact-camera behavior; the real Editor snapshot smoke passes in parallel and deterministic frame-task modes. Backend-neutral raster preparation still composes approximate absolute doubles before subtracting mesh and camera positions against that origin, then the D3D12 viewport issues one built-in prototype-geometry draw per visible mesh record with per-frame/per-draw constants. Inspector transform edits therefore enter the following snapshot and move the current D3D12 draw. Canonical sector/local snapshot/raster propagation across huge sectors remains pending; real mesh/material resources, Vulkan scene raster, culling, coordinate debug views, physics, and ray/TLAS/query consumers remain pending.
...
- [ ] Propagate canonical sector/local transforms through snapshot/raster preparation and repeat comparative D3D12 captures across real sector transitions before real resources, culling, coordinate debug views, physics, and ray/TLAS/query consumers rely on them.
