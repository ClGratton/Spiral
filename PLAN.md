...
- [ ] Persist canonical sector/local Scene transforms and their immutable world-grid policy through scene format version 4 while retaining version 1-3 loads, without parallel transform authorities.
- [ ] Implement stable-ID per-view sector-snapped origin tracking with hysteresis and teleport handling, publish independent immutable view epochs, and test no-flap and multi-view retention while preserving exact-camera translation as the default.
