...
- [ ] **Phase 3 production Smooth automated native-evidence prerequisite:** run the current versioned 60/120 FPS engine matrices for Responsive, InterFrame, and SubmissionGate on D3D12 and Vulkan; add exact-PID PresentMon correlation for D3D12 and a Vulkan PresentMon attachment attempt that fails closed when the required exact-process evidence is unavailable or ambiguous. Preserve immutable raw streams, joined reports, and condition manifests for every accepted or failed condition. This provisional bundle may close independently, but it does not establish physical display cadence, monitor/VRR state, input or photon latency, RTSS execution/configuration, or a production winner.
