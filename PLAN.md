...
- [ ] **Phase 3 production Smooth external/reference manual-evidence prerequisite:** execute the accepted exact-Editor RTSS `ASYNC` manual gate for each admitted reference condition: operator-signed before state, exact apply evidence, one bounded run, exact restore evidence even after failure/timeout, and a signed receipt. FES or frame-generation is included only when named explicitly as a separate condition with its own before/applied/restored state and evidence; neither may be mutated or inferred implicitly. This prerequisite is reference-condition evidence only and does not select the production default.
