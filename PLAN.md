...
- [x] **Phase 3 presentation-pacing benchmark-capture prerequisite:** renderer-owned bounded immutable snapshots retain raw frame-ID-keyed lifecycle/wait/completion observations and derive nearest-rank p50/p95/p99 plus `1000 / p99` 1%-low and `1000 / p99.9` 0.1%-low start-to-start metrics, deadline-miss count, and p99 overshoot without replacing raw spikes. Stable schema-1 CSV/JSON artifacts carry backend/adapter/requested and effective policy target/candidate plus runner-supplied presentation/sync/VRR/tearing condition strings, whose honest default is `unknown`; display, replacement/drop, input, and GPU-headroom remain explicit unavailable fields. `EngineTests` (63/63) cover bounded eviction, continuity, raw spikes, aggregation, p99.9, escaping, and lifecycle/wait/completion artifact fields. Local RTX 3080 Ti D3D12/Vulkan runs passed all twelve Responsive/InterFrame/SubmissionGate conditions at 60/120 FPS with a declared 30-frame warm-up and exactly 512 retained frames per artifact. This qualifies capture fidelity only; it neither claims display/input evidence nor selects a production candidate.
- [ ] **Phase 3 external-capture attachment prerequisite — benchmark readiness and Windows QPC identity:** after renderer/window initialization, publish a unique run ID, exact editor PID/canonical executable path, QueryPerformanceCounter frequency/current tick, artifact path, and requested condition metadata; wait on a bounded explicit supervisor release before the declared warm-up and retained capture begin. The runner must prove no measurement rows precede release, exact-run/process identity, timeout/cancellation failure, and verified editor cleanup. This qualifies the attachment boundary only; it does not require or claim a working external collector.
